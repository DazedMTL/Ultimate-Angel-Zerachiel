...
(function ($SkillShop) {
  ...
  Utils.processParameters = function () {
    ...
    result["Default Price"] = Number(parameters["Default Price"] || 0);

    result["Gold Cost Text"] = String(
      parameters["Gold Cost Text"] || "Gold Cost"
    );

    result["Item Cost Text"] = String(
      parameters["Item Cost Text"] || "Requires"
    );

    result["Buy Command"] = String(parameters["Buy Command"] || "Learn");

    result["Cancel Command"] = String(parameters["Cancel Command"] || "Leave");

    result["Text Alignment"] = String(parameters["Text Alignment"] || "Leave");
  };
  ...
  Game_Actor.prototype._canBuySkillCustom = function (skill) {
    ...
    for (var i = 0; i < customRequires.length; i++) {
      try {
        ...
        throw new Error(
          "[YED_SkillShop] There are problems with custom requirements for skill ID" +
            skill.id
        );
      }
  ...
  Game_Actor.prototype._paySkillBuyCostCustom = function (skill) {
    ...
    for (var i = 0; i < customCosts.length; i++) {
      try {
        ...
        console.log(err.message);
        throw new Error(
          "[YED_SkillShop] There are problems with custom cost for skill ID" +
            skill.id
        );
      }
    }
...
(function ($SkillShop) {
   ...
   */
  SkillShopActors.prototype = Object.create(Window_Selectable.prototype);
   ...
   * Draw face mini.
   */
  SkillShopActors.prototype.drawFace = function (
    faceName,
    faceIndex,
    x,
    y,
...
(function ($SkillShop) {
   ...
   * Inherits from Window_Base
   */
  SkillShopSkills.prototype = Object.create(Window_Selectable.prototype);
