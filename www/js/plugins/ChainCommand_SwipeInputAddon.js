...
if (Imported["SumRndmDde Swipe Input"] && Imported.ChainCommand) {
  (function () {
    var validKeys =
      PluginManager.parameters("ChainCommand")["Valid Keys"].split(" ");

    Scene_ChainCommand.prototype.processInput = function () {
      for (var i = 0; i < validKeys.length; i++) {
        if (
          Input.isTriggered(validKeys[i]) ||
          (["up", "left", "right", "down"].contains(validKeys[i]) &&
            SwipeInput.isTriggered(validKeys[i]))
        ) {
