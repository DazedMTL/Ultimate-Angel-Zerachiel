...
//■ ピクチャ表示処理 ■■■■■
Game_Interpreter.prototype.SetSprite = function (
  _No,
  _Path,
  _x,
  _y,
...
//スプライトの表示::中心
Game_Interpreter.prototype.SetSpriteC = function (
  _No,
  _Path,
  _x,
  _y,
...
//スプライトの表示::右上
Game_Interpreter.prototype.SetSpriteR = function (
  _No,
  _Path,
  _x,
  _y,
...
//スプライトのフェードイン
Game_Interpreter.prototype.SetSpriteFIn = function (
  _No,
  _Path,
  _x,
  _y,
...
Game_Interpreter.prototype.SetSpriteFInC = function (
  _No,
  _Path,
  _x,
  _y,
...
Game_Interpreter.prototype.SetSpriteFInR = function (
  _No,
  _Path,
  _x,
  _y,
...
function overSpriteClickable(_PicNo, _CMapArr) {
  if (this.overPointerSpCk(_PicNo)) {
    ...
    for (var i = 0; i <= _CMapArr.length - 1; i++) {
      if (
        _TcPosX >= _CMapArr[i][0] + _CkSprite._x - _CkZureX &&
...
Game_Interpreter.prototype.CngSprite = function (_No, _Path) {
  ...
  for (var i = 0; i <= _N_Sprite.children.length - 1; i++) {
    if (
      _N_Sprite.children[i].No == _No &&
      _N_Sprite.children[i].dLayer == N_Sprite_DLayer
    ) {
      var _FolPath = _FolPathGet(_Path);
      var _FileName = _FileNameGet(_Path);
...
var DelSprite = function (_No, _SpStrFlg) {
  //thisを付けなくても良いバージョン
  var _N_Sprite = SelSprite(_No); //選択Spriteを取得
  for (var i = 0; i <= _N_Sprite.children.length - 1; i++) {
    if (
...
Game_Interpreter.prototype.MoveSprite = function (
  _No,
  _x,
  _y,
  _time,
...
//移動位置を追加する
Game_Interpreter.prototype.MoveSpriteAdd = function (
  _No,
  addx,
  addy,
  _time,
...
Game_Interpreter.prototype.CmpSprite_Setting = function (
  _No,
  _CmpSpriteArr,
  _xPos,
  _yPos,
...
Game_Interpreter.prototype.CmpSprite_SettingC = function (
  _No,
  _CmpSpriteArr,
  _xPos,
  _yPos,
...
Game_Interpreter.prototype.CmpSprite_SettingR = function (
  _No,
  _CmpSpriteArr,
  _xPos,
  _yPos,
...
Game_Interpreter.prototype.CmpSprite_Setting_Update = function () {
  for (var i = 0; i <= _CmpSprite_Setting_Keep.length - 1; i++) {
    //描画に成功した場合はTrueが入る
    if (
      this.CmpSprite_Setting_Draw(
        _CmpSprite_Setting_Keep[i]._No,
        _CmpSprite_Setting_Keep[i]._CmpSpriteArr
      )
    ) {
...
Game_Interpreter.prototype.CmpSprite_Setting_Draw = function (
  _No,
  _CmpSpriteArr
) {
  for (var i = 1; i <= _CmpSpriteArr.length - 1; i++) {
    if (!_CmpSpriteArr[i]._Sprite.bitmap.isReady()) {
      //準備ができていない項目が存在する
      return false;
    }
  }
...
var SpriteSkipCk = function (DrawStr) {
  ...
  for (var i = 0; i <= _SpriteStrArr.length - 1; i++) {
    if (_SpriteStrArr[i]._No == DrawStr._No) {
      ...
      if (
        _SpriteStrArr[i]._Text != DrawStr._Text ||
...
var SpriteStrDataGet = function (_PNo) {
  if (GetSpriteData(_PNo) != null) {
    if (GetSpriteData(_PNo).dTextInfo != null) {
      GetSpriteData(_PNo).dTextInfo.originalValue = GetSpriteData(
        _PNo
      ).dTextInfo.originalValue.replace("\n", "");
    }
    return GetSpriteData(_PNo).dTextInfo;
  }
};
var _SetdTextInfo = null;
Game_Interpreter.prototype.SpriteStr = function (
  _No,
  _Text,
  _Size,
  _x,
  ...
  if (SpriteStrDataGet(_No) != null) {
    ...
    if (
      SpriteStrDataGet(_No).originalValue == _Text &&
...
Game_Interpreter.prototype.SpriteStrC = function (
  _No,
  _Text,
  _Size,
  _x,
...
Game_Interpreter.prototype.SpriteStrR = function (
  _No,
  _Text,
  _Size,
  _x,
...
//テキストポップアップ
Game_Interpreter.prototype.SpriteStrPop = function (
  _No,
  _Text,
  _Size,
  _x,
...
Game_Interpreter.prototype.CngSpriteStr = function (_No, _Text, _Size) {
  ...
  if (SpriteStrDataGet(_No) != null) {
    ...
    var _Size =
      typeof _Size !== "undefined" ? _Size : SpriteStrDataGet(_No).size;
    ...
    if (
      ...
      _NSprite_AncNo = 0;
      if (_StData._anchor._x == 0.5 && _StData._anchor._y == 0.5)
        _NSprite_AncNo = 1;
      if (_StData._anchor._x == 1 && _StData._anchor._y == 0)
        _NSprite_AncNo = 2;
      this.SpriteStr(
        _No,
        _Text,
...
Game_Interpreter.prototype.PlaySpriteAnime = function (
  _No,
  _Path,
  _CellSu,
  _CellFlm,
...
Game_Interpreter.prototype.PlaySpriteAnimeC = function (
  _No,
  _Path,
  _CellSu,
  _CellFlm,
...
Game_Interpreter.prototype.PlaySpriteAnimeR = function (
  _No,
  _Path,
  _CellSu,
  _CellFlm,
...
//■ 特定セルの表示 ■■■■■
Game_Interpreter.prototype.SetSpriteCell = function (
  _No,
  _Path,
  _CellSu,
  _CellNo,
...
Game_Interpreter.prototype.SetSpriteCellC = function (
  _No,
  _Path,
  _CellSu,
  _CellNo,
...
Game_Interpreter.prototype.SetSpriteCellR = function (
  _No,
  _Path,
  _CellSu,
  _CellNo,
...
Game_Interpreter.prototype.KeepSetSpriteStr = function (
  _No,
  _Data,
  _x,
  _y,
...
Game_Interpreter.prototype.SpriteInObj = function (
  _No,
  _Sprite,
  _x,
  _y,
...
NSprite.prototype.move = function (
  x,
  y,
  scaleX,
  scaleY,
