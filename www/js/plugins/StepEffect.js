...
(function () {
  ...
  DataManager.initStepEffects = function (note) {
    ...
    for (var i = 0, max = texts.length; i < max; i++) {
      if (
        texts[i].match(
          /<(?:ステップ|Step)((?:エフェクト|Effect|サウンド|Sound))(\d+):(.+)>/
        )
      ) {
        ...
        texts[i].match(
          /<(?:ステップ|Step)((?:エフェクト|Effect|サウンド|Sound))(?:設定|Setting)(\d+):(.+)>/
        )
      ) {
        var tag = Number(RegExp.$2);
        if (!stepEffects[tag]) {
  ...
  Sprite_StepEffect.prototype.initialize = function (
    fileName,
    setting,
    x,
    y,
    ...
    var scale = sc ? sc : 100;
    var wet = fn.filter(function (s) {
      return s.contains("wet");
    })[0];
    var as = fn.filter(function (s) {
      return s.contains("animeSpeed");
    })[0];
  ...
  Sprite_StepEffect.prototype.update = function () {
    ...
    if (!this.bitmap) {
      ...
      if (this._fitStep) {
        cx =
          (this._dir === 2 ? (this._stepCount % 2) * stepSize : 0) -
          stepSize / 2;
        cx =
          this._dir === 8
            ? stepSize / 2 - (this._stepCount % 2) * stepSize
            : cx;
        cy =
          (this._dir === 4 ? (this._stepCount % 2) * stepSize : 0) -
          stepSize / 2;
        cy =
          this._dir === 6
            ? stepSize / 2 - (this._stepCount % 2) * stepSize
  ...
  Game_Map.prototype.showStepEffect = function (
    tag,
    x,
    y,
    d,
    ...
    if (tileset) {
      ...
      var soundEnable =
        character._moveCount % (stepSpeed * ary[character.realMoveSpeed()]) ===
        0;
      if (soundEnable && effect.se && effect.se !== "null") {
        ...
        var p = s[1].split("-");
        v = v[1]
          ? Math.randomInt(Math.abs(Number(v[1]) - Number(v[0]))) + Number(v[0])
          : Number(v[0]);
        p = p[1]
          ? Math.randomInt(Math.abs(Number(p[1]) - Number(p[0]))) + Number(p[0])
          : Number(p[0]);
        var distance =
          Math.abs(character._realX - $gamePlayer._realX) +
          Math.abs(character._realY - $gamePlayer._realY);
      ...
      if (effect.anime && effect.anime !== "null") {
        SceneManager._scene.setStepEffectAnime(
          effect.anime,
          effect.animeSetting,
          x,
          y,
  ...
  Game_Map.prototype.stepEffects = function (tileset) {
    if (!tileset._stepEffects)
      tileset._stepEffects = DataManager.initStepEffects(tileset.note);
  ...
  Game_Event.prototype.stepEffects = function () {
    ...
    this.event().pages[0].list.forEach(
      function (l) {
  ...
  Scene_Map.prototype.updateStepSprites = function () {
    ...
    this._stepSprites = this._stepSprites.filter(function (s) {
      return !!s;
    });
  };

  Scene_Map.prototype.setStepEffectAnime = function (
    name,
    setting,
    x,
    y,
