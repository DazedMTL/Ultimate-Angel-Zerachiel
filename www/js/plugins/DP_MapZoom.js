...
(function () {
  ...
  var user_fix_picture = Boolean(
    parameters["Picture Size Fixation"] === "true" || false
  );
  ...
  var dp_renderSize = {
    ...
    onChange: function (scale) {
      ...
      //マップサイズ変更
      spriteset._tilemap.width = Math.ceil(
        (Graphics.width + spriteset._tilemap._margin * 2) / scale
      );
      spriteset._tilemap.height = Math.ceil(
        (Graphics.height + spriteset._tilemap._margin * 2) / scale
      );
  ...
  camera.animation = (function () {
    ...
    var r = {
      ...
      update: function () {
        ...
        _pan_prev.x =
          (_pan_prev.x * (_duration - 1) + _pan_target.x) / _duration;
        _pan_prev.y =
          (_pan_prev.y * (_duration - 1) + _pan_target.y) / _duration;

        $gameScreen.setZoom(
          0,
          0,
          ($gameScreen.zoomScale() * (_duration - 1) + _target) / _duration
        );
        camera.center(_pan_prev.x, _pan_prev.y);
  ...
  (function () {
    ...
    Sprite_Picture.prototype.updatePosition = function () {
      ...
      this.x = Math.floor(
        (picture.x() + map_s.x) * (1 / $gameScreen.zoomScale())
      );
      this.y = Math.floor(
        (picture.y() + map_s.y) * (1 / $gameScreen.zoomScale())
      );
    };
  })();
