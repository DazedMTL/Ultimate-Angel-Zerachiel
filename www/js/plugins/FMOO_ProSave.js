...
LZString.compressToBase64 = function (data) {
  ...
  if (StorageManager.isLocalMode() && Fmoo.useZlib == 1) {
    ...
    return (
      _fmoo_SavePrefix +
      zlib.deflateRawSync(new Buffer(data)).toString("base64")
    );
  }
...
LZString.decompressFromBase64 = function (data) {
  console.log(
    "FMOO_ProSave.decompressFromBase64",
    StorageManager.isLocalMode(),
    data.startsWith(_fmoo_SavePrefix)
  );
  if (StorageManager.isLocalMode() && data.startsWith(_fmoo_SavePrefix)) {
    var zlib = require("zlib");
    return zlib
      .inflateRawSync(new Buffer(data.slice(_fmoo_SavePrefix.length), "base64"))
      .toString();
  }
