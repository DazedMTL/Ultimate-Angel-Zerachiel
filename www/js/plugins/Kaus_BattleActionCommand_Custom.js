...
Imported.Kaus_BattleActionCommand = 1.00;


const parameters = PluginManager.parameters('Kaus_BattleActionCommand');
const KauspartycommandExpulsion = (parameters['partycommandExpulsion'] === 'true');
...
Scene_Battle.prototype.createTargetGraphics = function () {
    if (!this._target) {
        ...
        this._target.bitmap = ImageManager.loadBitmap('img/battlehud/', 'BattleCursor');
        this._targetEnemy.bitmap = ImageManager.loadBitmap('img/battlehud/', 'SelectTargetL');
        this._targetAlly.bitmap = ImageManager.loadBitmap('img/battlehud/', 'SelectTargetR');
...
Scene_Battle.prototype.createCommandGraphics = function () {
    if (!this._commandOrb) {
        ...
        this._commandOrb.bitmap = ImageManager.loadBitmap('img/battlehud/', 'commandOrb_Base');
        this._commandLS.bitmap = ImageManager.loadBitmap('img/battlehud/', 'commandlifestream');
        this._commandFrame.bitmap = ImageManager.loadBitmap('img/battlehud/', 'commandOrb_Frame');
        this._commandIcon.bitmap = ImageManager.loadBitmap('img/battlehud/', 'commandOrb_Items');
        this._commandIcon.bitmap = ImageManager.loadBitmap('img/battlehud/', 'commandOrb_Guard');
        this._commandIcon.bitmap = ImageManager.loadBitmap('img/battlehud/', 'commandOrb_Tech');
        this._commandIcon.bitmap = ImageManager.loadBitmap('img/battlehud/', 'commandOrb_Attack');
        this._commandHL.bitmap = ImageManager.loadBitmap('img/battlehud/', 'commandHL');
        this._commandAttack.bitmap = ImageManager.loadBitmap('img/battlehud/', 'commandTextAttack');
        this._commandTech.bitmap = ImageManager.loadBitmap('img/battlehud/', 'commandTextTech');
        this._commandGuard.bitmap = ImageManager.loadBitmap('img/battlehud/', 'commandTextGuard');
        this._commandItems.bitmap = ImageManager.loadBitmap('img/battlehud/', 'commandTextItems');
    ...
    this._commandOrb.opacity = this._commandHL.opacity = this._commandAttack.opacity = this._commandGuard.opacity = this._commandTech.opacity = this._commandItems.opacity = this._commandIcon.opacity = this._commandFrame.opacity = this._commandLS.opacity = 0;
...
Scene_Battle.prototype.updateCommandGraphics = function () {
    ...
    const inputobjcond = {
        targetwin: checkwin,
        condposarr: [[posat.x, posat.y, posat.width, posat.height],
    ...
    if (SceneManager.Kausinputok(inputobjcond) || SceneManager.Kausinputcancel()) {
        this.removeCommandGraphics();
        return;
    }
    if ((preindex !== checkwin.index()) || Input.isRepeated('up') || Input.isRepeated('down')) {
        this._commandHL.x = 45;
        this._commandIcon.opacity = 0;
    }

    //Orb Animation
    if (this._commandOrb.opacity != 255) { this._commandOrb.opacity += 40; this._commandFrame.opacity += 40; this._commandLS.opacity += 40; }
    if (this._commandOrb.y != 520 + 96) { this._commandOrb.y -= 20; this._commandFrame.y -= 20; }
    this._commandOrb.rotation += 0.002;
    //Icon Animation
    this._commandLS.blendMode = PIXI.BLEND_MODES.SCREEN;
    if (this._commandIcon.opacity != 255 & this._commandOrb.y == 520 + 96) this._commandIcon.opacity += 5;
    this._commandLS.origin.x += 0.5;
    //Command Text Intro
    if (this._commandAttack.opacity != 255) this._commandAttack.opacity = this._commandGuard.opacity = this._commandTech.opacity = this._commandItems.opacity += 10;
    ...
    //Highlight X Insist
    if (this._commandHL.opacity != 255 & this._commandOrb.y == 520 + 96) this._commandHL.opacity += 20;
    if (this._commandHL.x != 155 & this._commandOrb.y == 520 + 96) this._commandHL.x += 10;
...
Scene_Battle.prototype.createDestinyGraphics = function () {
    if (!this._fadeBlackRIGHT) {
        ...
        this._fadeBlackRIGHT.bitmap = ImageManager.loadBitmap('img/battlehud/', 'BattleFate_FadeRight');
        this._fadeBlackLEFT.bitmap = ImageManager.loadBitmap('img/battlehud/', 'BattleFate_FadeLeft');
        this._DestinyFight.bitmap = ImageManager.loadBitmap('img/battlehud/', 'BattleFate_Fight');
        this._DestinyEscape.bitmap = ImageManager.loadBitmap('img/battlehud/', 'BattleFate_Escape');
        this._Destiny.bitmap = ImageManager.loadBitmap('img/battlehud/', 'BattleFate_Destiny');
        this._DestinyHL.bitmap = ImageManager.loadBitmap('img/battlehud/', 'BattleFate_HL');
...
Scene_Battle.prototype.updateDestinyGraphics = function () {
    ...
    const inputobjcond = { targetwin: checkwin, condposarr: [[posdesf.x, posdesf.y, posdesf.width, posdesf.height], [posdese.x, posdese.y, posdese.width, posdese.height]] };
    const preindex = checkwin.index();
    if (SceneManager.Kausinputok(inputobjcond)) {
        this.removeDestinyGraphics();
        return;
    }
    if ((preindex !== checkwin.index()) || Input.isRepeated('left') || Input.isRepeated('right')) {
        this._Kausmovement = 0;
        this._KaustoggleArrow = false;
        this._KaustoggleHL = false;
        this._DestinyHL.opacity = 0;
    ...
    switch (checkwin.index()) {
        case 0:
            this._DestinyArrow.bitmap = ImageManager.loadBitmap('img/battlehud/', 'BattleFate_ArrowR')
            this._DestinyHL.move(172, 170);
            if (this._DestinyArrow.x != this._KausArrowData[0] + 10 & !this._KaustoggleArrow) this._Kausmovement += 0.5;
            else this._KaustoggleArrow = true;
            if (this._DestinyArrow.x != this._KausArrowData[0] & this._KaustoggleArrow) this._Kausmovement -= 0.5;
            ...
            break;
        case 1:
            this._DestinyArrow.bitmap = ImageManager.loadBitmap('img/battlehud/', 'BattleFate_ArrowL')
            this._DestinyHL.move(558, 170);
            if (this._DestinyArrow.x != this._KausArrowData[1] - 10 & !this._KaustoggleArrow) this._Kausmovement -= 0.5;
            else this._KaustoggleArrow = true;
            if (this._DestinyArrow.x != this._KausArrowData[1] & this._KaustoggleArrow) this._Kausmovement += 0.5;
            else this._KaustoggleArrow = false;
