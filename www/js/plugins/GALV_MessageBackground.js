...
(function () {
  Galv.MBG.v = Number(
    PluginManager.parameters("Galv_MessageBackground")["Image Variable ID"]
  );
  Galv.MBG.s = Number(
    PluginManager.parameters("Galv_MessageBackground")["Disable Switch ID"]
  );
  ...
  Galv.MBG.Window_Message_setBackgroundType =
    Window_Message.prototype.setBackgroundType;
  ...
  Galv.MBG.Scene_Battle_createWindowLayer =
...
Sprite_GalvMsgBg.prototype.controlBitmap = function () {
  if (
    $gameSwitches.value(Galv.MBG.s) ||
    Galv.MBG.window.openness <= 0 ||
    Galv.MBG.disable
  ) {
    this.opacity = 0;
    this.maxopac = 255;
