...
(function () {
	...
	var villaA_StandPicture_Size = String(parameters['立ち絵サイズ']).split(",").map(function (ele) { return Number(ele) }) || [546, 624];
	var villaA_battle_StandAnimationPositionX = Number(parameters['戦闘中アクターへのアニメーションx座標']);
	var villaA_battle_StandAnimationPositionY = Number(parameters['戦闘中アクターへのアニメーションy座標']);
	var villaA_battle_StandPositionX = Number(parameters['戦闘中立ち絵x座標']);
	var villaA_battle_StandPositionY = Number(parameters['戦闘中立ち絵y座標']);
	var villaA_menu_StandPositionX = Number(parameters['メニュー画面立ち絵x座標']);
	var villaA_menu_StandPositionY = Number(parameters['メニュー画面立ち絵y座標']);
	var villaA_battle_stand_ValidStateArray = String(parameters['異常状態優先度'] || "毒,暗闇,睡眠").split(",");
	...
	Scene_Boot.prototype.checkPluginParameters = function () {
		...
		villaA_battle_stand_ValidStateArray.forEach(function (stateName) {
			if (stateNameArray.indexOf(stateName) < 0) { throw new Error(stateName + "という名前のステートがデータベースにありません。プラグインパラメータに間違いがないか確認してください。") }
		});
	}
	...
	ImageManager.loadBattleStandImage = function (filename, crisis, hue) {
		if (StorageManager.CheckImgFilePath(filename)) {
			...
			if (StorageManager.CheckImgFilePath(escapeFileName)) {
				return this.loadBitmap('img/pictures/stand_images/', escapeFileName, hue, true);
			} else {
				throw new Error("通常状態のＨＰ1/4未満、半分未満、半分以上の立ち絵が見つかりません。wait_nomal.png、wait_near_pinch.png、wait_pinch.pngをstand_imagesフォルダ内にご用意ください。")
			}
		}
	};
	...
	Sprite_StandCharaPicture.prototype.updateStandPictureForMenu = function () {
		...
		this._standPicture.bitmap = ImageManager.loadBattleStandImage(pictureName, this.crisisSuffix());
	}

	Sprite_StandCharaPicture.prototype.updateStandPictureForBattle = function (motionType) {
		if (!this._standPicture) return;
		var crisis = this.crisisSuffix();
		var actor = $gameParty.leader();
		if (motionType == 'walk') motionType = 'wait';
		if (this.hasCheckedState() && motionType != "damage" && motionType != "victory") {
			motionType = "abnormal";
		}
		if (motionType == "abnormal" || motionType == "sleep") {
			...
			if (abnomalDescribe) {
				...
				this._standPicture.bitmap = ImageManager.loadBattleStandImage(pictureName, crisis);
		...
		if (motionType == "damage" || motionType == "wait" || motionType == "victory") {
	...
	Sprite_StandCharaPicture.prototype.hasCheckedState = function () {
		var states = $gameParty.leader().states().map(function (state) {
	...
	Sprite_StandCharaPicture.prototype.setAbnomalDescribe = function () {
		var tesArr = $gameParty.leader().states().map(function (state) { return state.name });
		var changeState = $gameParty.leader().states().map(function (state) {
			return villaA_battle_stand_ValidStateArray.indexOf(state.name)
		});
	...
	Spriteset_Battle.prototype.settingDamagePopupLayer = function () {
		this.battlerSprites().forEach(function (sprite) {
	...
	var _alias_villaA_battle_stand_Sprite_Battler_setupDamagePopup = Sprite_Battler.prototype.setupDamagePopup
	Sprite_Battler.prototype.setupDamagePopup = function () {
		if (this._battler.isDamagePopupRequested()) {
			if (this._battler.isSpriteVisible()) {
				var sprite = new Sprite_Damage();
				if (this._battler.isActor()) {
					sprite.x = villaA_battle_StandPositionX + villaA_StandPicture_Size[0] / 2;
					sprite.y = villaA_battle_StandPositionY + villaA_StandPicture_Size[1] / 2;
				} else {
	...
	Sprite_Actor.prototype.setActorHome = function (index) {
		this.setHome(villaA_battle_StandAnimationPositionX, villaA_battle_StandAnimationPositionY);
	};
	...
	var _alias_villaA_battle_stand_Sprite_Actor_startMotion = Sprite_Actor.prototype.startMotion;
	...
	var _alias_villaA_battle_stand_Scene_Menu_create = Scene_Menu.prototype.create
