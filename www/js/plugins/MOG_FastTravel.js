...
Moghunter.fastTravel_lastIndex = String(
  Moghunter.parameters["Remember Last MapID"] || "true"
);
Moghunter.fastTravel_playBgm = String(
  Moghunter.parameters["Play Bgm"] || "false"
);
Moghunter.fastTravel_BgmFileName = String(
  Moghunter.parameters["Bgm File Name"] || "Theme2"
);
Moghunter.fastTravel_fadeBgm = String(
  Moghunter.parameters["Fade Bgm"] || "true"
);
Moghunter.fastTravel_MapMoveSpeed = Number(
  Moghunter.parameters["Map Move Speed"] || 30
);
Moghunter.fastTravel_fadeSpeed = Number(
  Moghunter.parameters["Fade Speed"] || 30
);
Moghunter.fastTravel_zoomAnime = String(
  Moghunter.parameters["Zoom Animation"] || "true"
);
Moghunter.fastTravel_rotationAnime = String(
  Moghunter.parameters["Rotation Animation"] || "true"
);
Moghunter.fastTravel_CenterX = Number(
  Moghunter.parameters["Center X-axis"] || 0
);
Moghunter.fastTravel_CenterY = Number(
  Moghunter.parameters["Center Y-axis"] || 0
);
Moghunter.fastTravel_ListWidth = Number(
  Moghunter.parameters["List Width"] || 300
);
Moghunter.fastTravel_ListHeight = Number(
  Moghunter.parameters["List Height"] || 290
);
...
Moghunter.fastTravel_ListSlide = String(
  Moghunter.parameters["Slide List"] || "true"
);
Moghunter.fastTravel_ListLayoutX = Number(
  Moghunter.parameters["List Layout X-axis"] || 0
);
Moghunter.fastTravel_ListLayoutY = Number(
  Moghunter.parameters["List Layout Y-axis"] || -53
);
Moghunter.fastTravel_prevSlide = String(
  Moghunter.parameters["Slide Description"] || "true"
);
Moghunter.fastTravel_Prev_X = Number(
  Moghunter.parameters["Description X-axis"] || 160
);
Moghunter.fastTravel_Prev_Y = Number(
  Moghunter.parameters["Description Y-axis"] || 300
);
Moghunter.fastTravel_ComA_X = Number(
  Moghunter.parameters["Com Town X-axis"] || 470
);
Moghunter.fastTravel_ComA_Y = Number(
  Moghunter.parameters["Com Town Y-axis"] || 7
);
Moghunter.fastTravel_ComB_X = Number(
  Moghunter.parameters["Com Dungeon X-axis"] || 575
);
Moghunter.fastTravel_ComB_Y = Number(
  Moghunter.parameters["Com Dungeon Y-axis"] || 7
);
Moghunter.fastTravel_ComC_X = Number(
  Moghunter.parameters["Com Other X-axis"] || 710
);
Moghunter.fastTravel_ComC_Y = Number(
  Moghunter.parameters["Com Other Y-axis"] || 7
);
Moghunter.fastTravel_CursorX = Number(
  Moghunter.parameters["Com Cursor X-Axis"] || 0
);
Moghunter.fastTravel_CursorY = Number(
  Moghunter.parameters["Com Cursor Y-Axis"] || 0
);
Moghunter.fastTravel_CursorFloat = String(
  Moghunter.parameters["Com Cursor Animation"] || "true"
);
Moghunter.fastTravel_CursorBX = Number(
  Moghunter.parameters["Next Cursor X-Axis"] || 0
);
Moghunter.fastTravel_CursorBY = Number(
  Moghunter.parameters["Next Cursor Y-Axis"] || 0
);
Moghunter.fastTravel_CursorBFloat = String(
  Moghunter.parameters["Next Cursor Animation"] || "true"
);
Moghunter.fastTravel_Comp_Show = String(
  Moghunter.parameters["Show Completion"] || "true"
);
Moghunter.fastTravel_Comp_X = Number(
  Moghunter.parameters["Completion X-Axis"] || 590
);
Moghunter.fastTravel_Comp_Y = Number(
  Moghunter.parameters["Completion Y-Axis"] || 513
);
Moghunter.fastTravel_Comp_FontSize = Number(
  Moghunter.parameters["Completion Font Size"] || 22
);
Moghunter.fastTravelNewColor = Number(
  Moghunter.parameters["New Map Color"] || 6
);
Moghunter.fastTravel_directionCancel = String(
  Moghunter.parameters["Turn Back After Cancel"] || "true"
);
//=============================================================================
// ** ImageManager
//=============================================================================
...
Game_Interpreter.prototype.pluginCommand = function (command, args) {
  ...
  if (command === "fast_travel") {
    ...
    for (var i = 0; i < Moghunter.fastTravel_Towns.length; i++) {
      this._pre_fastTravelTowns[i] = [
        false,
        Moghunter.fastTravel_Towns[i],
        false,
      ];
    }

    for (var i = 0; i < Moghunter.fastTravelDungeons.length; i++) {
      this._pre_fastTravelDungeons[i] = [
        false,
        Moghunter.fastTravelDungeons[i],
        false,
      ];
    }

    for (var i = 0; i < Moghunter.fastTravelOther.length; i++) {
      this._pre_fastTravelOther[i] = [
        false,
        Moghunter.fastTravelOther[i],
        false,
      ];
    }
    ...
    for (var i = 1; i < $gameSystem._fastTravelTowns.length; i++) {
      for (var j = 1; j < this._pre_fastTravelTowns.length; j++) {
        if (
          $gameSystem._fastTravelTowns[i][1].name ==
          this._pre_fastTravelTowns[i][1].name
        ) {
    ...
    for (var i = 1; i < $gameSystem._fastTravelDungeons.length; i++) {
      for (var j = 1; j < this._pre_fastTravelDungeons.length; j++) {
        if (
          $gameSystem._fastTravelDungeons[i][1].name ==
          this._pre_fastTravelDungeons[i][1].name
        ) {
          this._pre_fastTravelDungeons[i][0] =
            $gameSystem._fastTravelDungeons[i][0];
          this._pre_fastTravelDungeons[i][2] =
            $gameSystem._fastTravelDungeons[i][2];
        }
      }
    }
    ...
    for (var i = 1; i < $gameSystem._fastTravelOther.length; i++) {
      for (var j = 1; j < this._pre_fastTravelOther.length; j++) {
        if (
          $gameSystem._fastTravelOther[i][1].name ==
          this._pre_fastTravelOther[j][1].name
        ) {
    ...
    $gameSystem._fastTravelComp =
      this._pre_fastTravelTowns.length +
      this._pre_fastTravelDungeons.length +
      this._pre_fastTravelOther.length -
      3;
  }
};
...
Game_System.prototype.initialize = function () {
  ...
  this._fastTravelComp =
    Moghunter.fastTravel_Towns.length +
    Moghunter.fastTravelDungeons.length +
    Moghunter.fastTravelOther.length -
    3;
};
...
Window_TravelList.prototype.drawItem = function (i) {
  ...
  if (!this._data[i][2]) {
    this.changeTextColor(this.textColor(Moghunter.fastTravelNewColor));
  }
...
Window_TravelList.prototype.processOk = function () {
  ...
  this.deactivate();
  this.visible = false;
  if (String(Moghunter.fastTravel_fadeBgm) === "true") {
...
SceneFastTravel.prototype.initialize = function () {
  ...
  this._zoomFade =
    String(Moghunter.fastTravel_zoomAnime) === "true" ? true : false;
  this._rotationFade =
    String(Moghunter.fastTravel_rotationAnime) === "true" ? true : false;
...
SceneFastTravel.prototype.checkLastSelection = function () {
  ...
  for (var i = 0; i < dataInfo.length; i++) {
    var data = dataInfo[i][1];
    if (data.name === $gameSystem._fastTravelSelection[1].name) {
      index = i;
    }
  }
  if (index >= 0) {
    ...
    if (index >= this._windowList.maxItems()) {
      index = 0;
    }
...
SceneFastTravel.prototype.updateFadeIn = function () {
  ...
  if ($gameSystem._fastTravelSelected) {
    if (this._zoomFade && !this._snapShot) {
      this.createSnapShot();
    }
    if (this._fadeTime <= 0) {
      this._field.opacity -= 4;
    }
    if (this._snapShot) {
      this._snapShot.scale.x += 0.005;
      this._snapShot.scale.y = this._snapShot.scale.x;
      if (this._rotationFade) {
        this._snapShot.rotation += 0.01;
      }
...
SceneFastTravel.prototype.createCompleted = function () {
  ...
  var ct =
    this._DataTown.length + this._DataDungeon.length + this._DataOther.length;
  //  var perc = Math.floor(ct / $gameSystem._fastTravelComp * 100)
  //  var total = ct + " / " + $gameSystem._fastTravelComp
  var perc = Math.floor((ct / $gameSystem._fastTravelComp) * 100);
  var total = ct + " / " + $gameSystem._fastTravelComp;
  this._comp.bitmap.drawText(
    total + " (" + perc + "%" + ")",
    0,
    0,
    200,
  ...
  this._comp.visible =
    String(Moghunter.fastTravel_Comp_Show) === "true" ? true : false;
};
...
SceneFastTravel.prototype.createCursor = function () {
  ...
  this._cursorFloat =
    String(Moghunter.fastTravel_CursorFloat) === "true" ? true : false;
...
SceneFastTravel.prototype.updateCursor = function () {
  if (this._cursorFloat) {
    this.updateCursorFloat();
  }
  if (this._Dataindex === 0) {
    this._cursor.x =
      this._listName[0].x +
      this._listName[0].bitmap.width / 2 +
      Moghunter.fastTravel_CursorX;
    this._cursor.y =
      this._listName[0].y +
      this._listName[0].bitmap.height +
      this._cursorAni[1] +
      Moghunter.fastTravel_CursorY;
  } else if (this._Dataindex === 1) {
    this._cursor.x =
      this._listName[1].x +
      this._listName[1].bitmap.width / 2 +
      Moghunter.fastTravel_CursorX;
    this._cursor.y =
      this._listName[1].y +
      this._listName[1].bitmap.height +
      this._cursorAni[1] +
      Moghunter.fastTravel_CursorY;
  } else {
    this._cursor.x =
      this._listName[2].x +
      this._listName[2].bitmap.width / 2 +
      Moghunter.fastTravel_CursorX;
    this._cursor.y =
      this._listName[2].y +
      this._listName[2].bitmap.height +
      this._cursorAni[1] +
      Moghunter.fastTravel_CursorY;
  }
};
...
SceneFastTravel.prototype.createCursorNext = function () {
  ...
  this._cursorBFloat =
    String(Moghunter.fastTravel_CursorBFloat) === "true" ? true : false;
...
SceneFastTravel.prototype.updateCursorB = function () {
  if (this._cursorBFloat) {
    this.updateCursorBFloat();
  }
  this._cursorB.x = this._np[0] + Moghunter.fastTravel_CursorBX;
  this._cursorB.y =
    this._np[1] + this._cursorBAni[1] + Moghunter.fastTravel_CursorBY - 10;
...
SceneFastTravel.prototype.updatePoints = function () {
  if (this._pointNeedRefresh && this._pointImg.isReady()) {
    this.refreshPoint();
  }
};
...
SceneFastTravel.prototype.needRefreshPointData = function () {
  if (this._DataindexOld != this._Dataindex) {
    return true;
  }
  if (this._WidowindexOld != this._windowList._index) {
    return true;
  }
  return false;
};
...
SceneFastTravel.prototype.executeCancel = function () {
  ...
  this._phase = 2;
  this._windowList.deactivate();
  if (String(Moghunter.fastTravel_directionCancel) === "true") {
    this.setPlayerDirectionforCancel();
  }
};
...
SceneFastTravel.prototype.selectMap = function () {
  ...
  $gamePlayer.reserveTransfer(
    this._currentData.mapID,
    this._currentData.mapX,
    this._currentData.mapY,
    dir,
    false
  );
...
SceneFastTravel.prototype.updateMapPosition = function () {
  if (this.needRefreshPointData()) {
    this.refreshPointData();
  }
  this._Mapfield.x = this.mapMoveTo(
    this._Mapfield.x,
    -this._np[0] + this._mapPos[0]
  );
  this._Mapfield.y = this.mapMoveTo(
    this._Mapfield.y,
    -this._np[1] + this._mapPos[1]
  );
  if (
    this._Mapfield.x === -this._np[0] + this._mapPos[0] &&
    this._Mapfield.y === -this._np[1] + this._mapPos[1]
...
SceneFastTravel.prototype.mapMoveTo = function (value, real_value) {
  ...
  var dnspeed =
    3 + Math.abs(value - real_value) / Moghunter.fastTravel_MapMoveSpeed;
  if (value > real_value) {
    value -= dnspeed;
    if (value < real_value) {
      value = real_value;
...
SceneFastTravel.prototype.update = function () {
  ...
  if (this._needCenter) {
    if (this._map.bitmap.isReady()) {
      this.setCenterStart();
    }
    return;
  }
  if (this._phase === 1 && !$gameSystem._fastTravelSelected) {
    this.updateCommand();
  }
  if (this._phase > 0) {
  ...
  if (this._phase === 0) {
    ...
    (!this._mapMoving && $gameSystem._fastTravelSelected) ||
    this._phase === 2
  ) {
    this.updateFadeIn();
  }
