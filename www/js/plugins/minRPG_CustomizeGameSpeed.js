...
(function () {
  ...
  var getVarAsBool = function (param) {
    ...
    if (Number.isNaN(param) || param === Infinity || param === -Infinity)
      return false; // NaN(数えられない数)でも、±Infinity（∞）でも、falseだよ。
    // paramがString型の時
    if (param instanceof String) {
      // paramがString型なら、初めて、大文字に変換して、文字列として値を見るよ。
      var upperParam = param.toUpperCase(); // 大文字小文字を分けてみるのが面倒だから、大文字化しちゃうよ。
      isON = upperParam === "ON" || upperParam === "TRUE" || param === "1";
      if (isON === false) {
        // falseなら、"\V[n]"などが入っている可能性があるから、変数の値を見るよ。
  ...
  var getVarAsNumber = function (param) {
    ...
    // paramがNumber型の時
    if (Number.isNaN(param) || param === Infinity || param === -Infinity)
      return param; // NaN(数えられない数)でも、±Infinity（∞）は、そのまま返す。
    ...
    if (param instanceof String) {
      // paramがString型なら、初めて、大文字に変換して、文字列として値を見るよ。
      var upperParam = param.toUpperCase(); // 大文字小文字を分けてみるのが面倒だから、大文字化しちゃうよ。
      if (upperParam === "ON" || upperParam === "TRUE" || param === "1")
        return 1;
  ...
  var getVarAsString = function (param) {
    ...
    // paramがNumber型の時
    if (Number.isNaN(param) || param === Infinity || param === -Infinity)
      return ""; // NaN(数えられない数)でも、±Infinity（∞）は、""を返す。
    // paramがNumber型もしくはString型の数値のみの時
  ...
  var getParam_IncludeEscapeCharactors = function (
    type,
    paramNames,
    defaultParam
  ) {
  ...
  var getParamBool_IncludeEscapeCharactors = function (
    paramNames,
    defaultParam
  ) {
    var value = getParamOther(paramNames);
   ...
   * ・値に"V[n]"などの変数が入っている際は、そのままその文字列を返します。
   *  → 変数参照時にgetVarAsBool(param)と呼び出すと、"\V[n]"の値を任意の型に変換して、返せます。 */
  var getParamNumber_IncludeEscapeCharactors = function (
    paramNames,
    defaultParam,
    min,
    max
  ...
  var getParamNumber = function (paramNames, min, max) {
    ...
    ).clamp(min, max); // parseIntできない時はエラーになるので、valueにnullやundefinedや±Infinity（！）や文字列が入っていてもエラーになるよ。
  };
  // ■bool型のパラメータ
  ...
  var getCommandName = function (command) {
    // 例: プラグインコマンドが「initVariables 1 10」なら、command="initVariables"
  ...
  var getArgNumber = function (arg, min, max) {
    if (arguments.length < 2) min = -Infinity;
    if (arguments.length < 3) max = Infinity;
    return (parseInt(convertEscapeCharactersAndEval(arg), 10) || 0).clamp(
      min,
      max
    ); // parseIntできない時は0になるので、valueにnullやundefinedや±Infinity（！）やNaNや文字列が入っていても0になるよ。
  };
  ...
  // 【機能】スピード変更のON/OFF
  var _isOnChangeSpeed_inThisPlugin = getParamBoolean([
    "isNoChangeSpeed_inThisPlugin",
    "【機能】スピード変更",
  ]);
  // 【機能】各種パラメータの限界値変更を禁止するかのON/OFF
  var _isOnChangeParamLimit_inThisPlugin = getParamBoolean([
  ...
  // 【機能】戦闘を検知するかのON/OFF
  var _isOnControlBattleEncountAndWinLose = getParamBoolean([
    "isOnControlBattleEncountAndWinLose",
    "【機能】戦闘を検知するか",
  ]);
  // 【機能】かばう条件を変更するかのON/OFF
  var _isOnChangeDefault_Migawari = getParamBoolean([
    "isChangeDefault_Migawari",
    "【機能】身代わり条件を変えるか",
  ]);

  // このゲーム固有の、ツクールデフォルトスピードに対するスピード倍率
  var _MAPSPEED_DEFAULT = getParamNumber(
    ["MAPSPEED_DEFAULT", "標準イベントスピード"],
    1,
    100
  ); // 整数
  var _BATTLE_LOGSPEED_DEFAULT = getParamNumber(
    ["BATTLE_LOGSPEED_DEFAULT", "標準戦闘ログスピード"],
    1,
    100
  ); // 整数
  var _BATTLE_ANIMATIONSPEED_DEFAULT = getParamNumber(
    ["BATTLE_ANIMATIONSPEED_DEFAULT", "標準戦闘アニメーションスピード"],
    1,
    100
  ); // 整数
  var _BATTLE_ENEMYSPEED_DEFAULT = getParamNumber(
    ["BATTLE_ANIMATIONSPEED_DEFAULT", "標準戦闘敵スプライトスピード"],
    1,
    100
  ); // 整数
  var _BATTLE_MOTIONSPEED_DEFAULT = getParamNumber(
    ["BATTLE_MOTIONSPEED_DEFAULT", "標準戦闘武器モーションスピード"],
    1,
    100
  ); // 整数
  var _BATTLE_MOVESPEED_DEFAULT = getParamNumber(
    ["BATTLE_MOVESPEED_DEFAULT", "標準戦闘移動スピード"],
    1,
    100
  ); // 整数
  var _RAPID_MAPSPEED_DEFAULT = getParamNumber(
    ["RAPID_MAPSPEED_DEFAULT", "早送りイベントスピード"],
    1,
    100
  ); // 整数
  var _RAPID_BATTLE_LOGSPEED_DEFAULT = getParamNumber(
    ["RAPID_BATTLE_LOGSPEED_DEFAULT", "早送り戦闘ログスピード"],
    1,
    100
  ); // 整数
  var _RAPID_BATTLE_ANIMATIONSPEED_DEFAULT = getParamNumber(
    ["RAPID_BATTLE_ANIMATIONSPEED_DEFAULT", "早送り戦闘アニメーションスピード"],
    1,
    100
  ); // 整数
  var _RAPID_BATTLE_ENEMYSPEED_DEFAULT = getParamNumber(
    ["RAPID_BATTLE_ANIMATIONSPEED_DEFAULT", "早送り戦闘敵スプライトスピード"],
    1,
    100
  ); // 整数
  var _RAPID_BATTLE_MOTIONSPEED_DEFAULT = getParamNumber(
    ["RAPID_BATTLE_MOTIONSPEED_DEFAULT", "早送り戦闘武器モーションスピード"],
    1,
    100
  ); // 整数
  var _RAPID_BATTLE_MOVESPEED_DEFAULT = getParamNumber(
    ["RAPID_BATTLE_MOVESPEED_DEFAULT", "早送り戦闘移動スピード"],
    1,
    100
  ); // 整数
  var _SKIPSPEED_DEFAULT = getParamNumber(
    ["_SKIPSPEED_DEFAULT", "デフォルトスキップ倍数"],
    1,
    100
  ); // 整数

  // 可変な各種早送りスピードを格納する、スイッチや変数番号。 0は無視（何も実行しない）を意味する。ツクール内変数やスイッチの番号は、1～9999までのはず。
  var _VarNo_Rapid_BattleRate = getParamNumber(
    ["VarNo_Rapid_BattleRate", "可変戦闘早送り倍数を格納する変数番号"],
    0,
    9999
  );
  var _VarNo_Rapid_MapRate = getParamNumber(
    ["VarNo_Rapid_MapRate", "可変イベント早送り倍数を格納する変数番号"],
    0,
    9999
  );
  // 可変な各種スキップスピードを格納する、スイッチや変数番号
  var _SwitchNo_isSkip = getParamNumber(
    ["SwitchNo_isSkip", "スキップ中かを格納するスイッチ番号"],
    0,
    9999
  );
  var _SwitchNo_isCanNotSkip = getParamNumber(
    ["SwitchNo_isNoSkip", "スキップ禁止中かを格納するスイッチ番号"],
    0,
    9999
  );
  // 可変なゲームスピードを持つ変数番号
  var _VarNo_GameSpeed = getParamNumber(
    ["VarNo_BattleSpeed", "可変ゲームスピードを格納する変数番号"],
    0,
    9999
  );
  var _VarNo_MessageWaitEscapeCharacterSpeed = getParamNumber(
    [
  ...
  var _VarNo_SkipRate = getParamNumber(
    ["VarNo_SkipRate", "可変スキップ倍数を格納する変数番号"],
    0,
    9999
  );

  // 戦闘中かを格納するスイッチ番号
  var _SwitchNo_IsBattle = getParamNumber(
    ["SwitchNo_IsBattle", "戦闘中かを格納するスイッチ番号"],
    0,
    9999
  );
  ...
  var _SwitchNo_RandomEncount = getParamNumber(
    ["SwitchNo_RandomEncount", "ランダムエンカウント時にONになるスイッチ番号"],
    0,
    9999
  );
  // 戦闘全滅時、逃走or中断時、勝利時、にONになるスイッチ番号
  var _SwitchNo_BattleWin = getParamNumber(
    ["SwitchNo_BattleWin", "戦闘勝利時にONになるスイッチ番号"],
    0,
    9999
  );
  var _SwitchNo_BattleEscape = getParamNumber(
    ["SwitchNo_BattleEscape", "戦闘逃走時にONになるスイッチ番号"],
    0,
    9999
  );
  var _SwitchNo_BattleLose = getParamNumber(
    ["SwitchNo_BattleLose", "戦闘全滅時にONになるスイッチ番号"],
    0,
    9999
  );
  // 最後に戦闘した敵グループIDを格納する変数番号
  var _VarNo_LastBattleTroopID = getParamNumber(
    ["VarNo_LastBattleTroopID", "最後に戦闘した敵グループIDを格納する変数番号"],
    0,
    9999
  );

  // 各種、早送りボタンを機能させるかの有無
  var _isRapid_MapButton_ok = getParamBoolean([
    "isRapid_MapButton_ok",
    "決定ボタンでイベント早送り可能か",
  ]);
  var _isRapid_BattleButton_ok = getParamBoolean([
    "isRapid_BattleButton_ok",
    "決定ボタンで戦闘早送り可能か",
  ]);
  var _isRapid_MapButton_cancel = getParamBoolean([
    "isRapid_MapButton_cancel",
    "キャンセルボタンでイベント早送り可能か",
  ]);
  var _isRapid_BattleButton_cancel = getParamBoolean([
    "isRapid_BattleButton_cancel",
    "キャンセルボタンで戦闘早送り可能か",
  ]);
  var _isRapid_MapButton_shift = getParamBoolean([
    "isRapid_MapButton_shift",
    "ダッシュボタンでイベント早送り可能か",
  ]);
  var _isRapid_BattleButton_shift = getParamBoolean([
    "isRapid_BattleButton_shift",
    "ダッシュボタンで戦闘早送り可能か",
  ]);
  // 各種条件下で、スキップを機能させるかの有無
  var _isSkip_Button_ok = getParamBoolean([
    "isSkipButton_ok",
    "決定ボタンでスキップ可能か",
  ]);
  var _isSkip_Button_cancel = getParamBoolean([
    "isSkipButton_cancel",
    "キャンセルボタンでスキップ可能か",
  ]);
  var _isSkip_Button_shift = getParamBoolean([
    "isSkipButton_shift",
    "ダッシュボタンでスキップ可能か",
  ]);
  var _isSkip_OnMapEvent = getParamBoolean([
    "isSkip_OnMapEvent",
    "マップイベント中もスキップ可能か",
  ]);
  var _isSkip_OnMapMoving = getParamBoolean([
    "isSkip_OnMapMoving",
    "マップ移動中もスキップ可能か",
  ]);
  var _isSkip_OnMapNeutral = getParamBoolean([
    "isSkip_OnMapNeutral",
    "マップ無操作中もスキップ可能か",
  ]);
  var _isSkip_OnBattle = getParamBoolean([
    "isSkip_OnBattle",
    "戦闘中もスキップ可能か",
  ]);

  // 各種パラメータの限界値。最小値はMPだけ0、他は最小値1～最大値はNumber.MAX_VALUE(1.7976931348623157e+308)。Infinityにすると表示や計算時にエラーになるので注意。
  var _Limit_MaxHP = getParamNumber(
    ["MaxHPLimit", "最大HPの限界値"],
    1,
    Number.MAX_VALUE
  ); // HPは1以上。
  var _Limit_MaxMP = getParamNumber(
    ["MaxMPLimit", "最大MPの限界値"],
    0,
    Number.MAX_VALUE
  ); // MPだけ0以上。
  var _Limit_MaxAttack_AndOtherParam = getParamNumber(
    ["ATKetcLimit", "攻撃力などの限界値"],
    1,
    Number.MAX_VALUE
  ); // 攻撃力や守備力は0だと困るけど、大丈夫かな？ 念のため、1以上にしておこう。
  // 各種かばう条件
  var _Migawawri_Sareru_MaxHPPercent = getParamNumber(
    ["SubstiutedChara_MaxHPPercent", "かばわれる側の最大HP％0-100"],
    0,
    100
  );
  var _Migawawri_Sareru_MaxMPPercent = getParamNumber(
    ["SubstiutedChara_MaxMPPercent", "かばわれる側の最大MP％0-100"],
    0,
    100
  );
  var _Migawawri_Suru_MinHPPercent = getParamNumber(
    ["SubstiutingChara_MinHPPercent", "かばう側の最小HP％0-100"],
    0,
    100
  );
  var _Migawawri_Suru_MinMPPercent = getParamNumber(
    ["SubstiutingChara_MinMPPercent", "かばう側の最小MP％0-100"],
    0,
    100
  );
  var _Migawawri_Suru_ActPercent = getParamNumber(
    ["SubstiutingRate", "身代わり率％0-100"],
    0,
    100
  );
  // 最後にかばったキャラ・かばわれたキャラのIDを格納する変数番号
  var _VarNo_Migawari_Sareta_AcorID = getParamNumber(
    [
  ...
  var _VarNo_Migawari_Shita_AcorID = getParamNumber(
    [
  ...
  var _VarNo_Migawari_Sareta_EnemyID = getParamNumber(
    [
  ...
  var _VarNo_Migawari_Shita_EnemyID = getParamNumber(
    [
  ...
  var _CommonEventNo_Migawari_Success_Actor = getParamNumber(
    [
  ...
  var _CommonEventNo_Migawari_Success_Enemy = getParamNumber(
    [
  ...
  var _WaitPeriodDef = getParamNumber_IncludeEscapeCharactors(
    ["Default_Wait_Period", "「＼.」の待ちフレーム数"],
    15,
    0,
    150
  );
  var _WaitLineDef = getParamNumber_IncludeEscapeCharactors(
    ["Default_Wait_Line", "「＼|」の待ちフレーム数"],
    60,
    0,
    600
  );
  ...
  var getMessageWaitEscapeCharacterSpeed = function () {
    ...
    if (
      _varNo != null &&
      _varNo >= 1 &&
      _varNo < $dataSystem.variables.length
    ) {
      ...
      if (_speed === Infinity || _speed === -Infinity) {
        _speed = _MESSAGESPEED_DEFAULT;
      } // 無限だったら、デフォルトスピードで。
      if (_speed === 0) {
        _speed = _MESSAGESPEED_DEFAULT;
      } // 0が入っていたら、デフォルトスピードで。
    ...
    if (
      _VarNo_GameSpeed != null &&
      _VarNo_GameSpeed >= 1 &&
      _VarNo_GameSpeed < $dataSystem.variables.length
    ) {
      ...
      else if (_gameSpeed === Infinity || _gameSpeed === -Infinity) {
        _gameSpeedChangeRate = 1;
      } // 無限大なら、1で。
      else if (_gameSpeed === 0) {
        _gameSpeedChangeRate = 1;
      } // 0が入っていたら、1で。
      else if (_gameSpeed === 5) {
        _gameSpeedChangeRate = 1;
      } // 5が入っていたら、1で。
      else if (_gameSpeed === 6) {
        _gameSpeedChangeRate = 1.3;
      } else if (_gameSpeed === 7) {
        _gameSpeedChangeRate = 1.7;
      } else if (_gameSpeed === 9) {
        _gameSpeedChangeRate = 99;
      } // 9が入っていたら、9で。
      else if (_gameSpeed === 10) {
        _gameSpeedChangeRate = 99;
      } // 最速の10が入っていたら、99で。
      else if (_gameSpeed < 0) {
        _gameSpeedChangeRate = 0;
      } // 1未満なら0に。
      else if (_gameSpeed > 99) {
        _gameSpeedChangeRate = 999;
      } // 1000以上になっても意味ないと思う。
      // _speedをかける。
    ...
    if (isFastRapidButton()) {
      // 変数番号を格納している、パラメータに変な値が入っていないか、随時チェック。
      if (
        _VarNo_Rapid_MapRate != null &&
        _VarNo_Rapid_MapRate >= 1 &&
        _VarNo_Rapid_MapRate < $dataSystem.variables.length
      ) {
        _gameSpeedChangeRate = Number.parseInt(
          $gameVariables.value(_VarNo_Rapid_MapRate)
        );
        // 変数の値に変な値が入っていないか、随時チェック。
        if (_gameSpeedChangeRate === NaN) {
          _gameSpeedChangeRate = 1;
        } // NaNなら、1。
        else if (
          _gameSpeedChangeRate === Infinity ||
          _gameSpeedChangeRate === -Infinity
        ) {
          _gameSpeedChangeRate = 0;
        } // 無限大なら、1。
        else if (_gameSpeedChangeRate === 0) {
          _gameSpeedChangeRate = 1;
        } // 0が入っていたら、01。
        else if (_gameSpeedChangeRate < 0.5) {
          _gameSpeedChangeRate = 0.5;
        } // 0.5未満なら0.5に。
        else if (_gameSpeedChangeRate > 99) {
          _gameSpeedChangeRate = 99;
        } // 100以上になっても意味ないと思う。
      } else {
        _gameSpeedChangeRate = 1;
    ...
    if (_speed > 999) {
      _speed = 999;
    } // 1000以上になっても意味ないと思う。999ならすでに0フレームになります。
    if (_speed === Infinity || _speed === -Infinity) {
      _speed = 999;
    } // 無限大なら、999で。
    if (Number.isNaN(_speed)) {
      _speed = 999;
    } // 数えられない数なら、999で。これは念のため。
    return _speed;
  };
  /** スキップモードの倍率を返します。*/
  var getSkipRate = function () {
    ...
    // 変数番号を格納している、パラメータに変な値が入っていないか、随時チェック。変な値が入っていたら、デフォルトスピードで。
    if (
      _VarNo_SkipRate != null &&
      _VarNo_SkipRate >= 1 &&
      _VarNo_SkipRate < $dataSystem.variables.length
    ) {
      _skipRate = $gameVariables.value(_VarNo_SkipRate);
    }
    // 変数の値に変な値が入っていないか、随時チェック。
    if (_skipRate === Infinity || _skipRate === -Infinity) {
      _skipRate = _SKIPSPEED_DEFAULT;
    } // 無限だったら、デフォルトスピードで。
    if (_skipRate === 0) {
      _skipRate = _SKIPSPEED_DEFAULT;
    } // 0が入っていたら、デフォルトスピードで。
    ...
    if (_skipRate > 99) {
      _skipRate = 99;
    } // 100以上になっても意味ないと思う。100倍なんて目に見えないし、updateを100回呼んでも処理は高速化されない。
    return _skipRate;
  };
  ...
  var isFastRapidButton = function () {
    ...
    var _isSkip = false;
    if (
      (_isSkip_Button_shift === true && Input.isPressed("shift")) ||
  ...
  var isRapid_MapButtonOn = function () {
    ...
    if (_isRapid_MapButton_ok === true && Input.isPressed("ok")) {
      // 決定ボタン（ok/Enter/zキーなど）
      _isRaid = true;
    } else if (_isRapid_MapButton_shift === true && Input.isPressed("shift")) {
      // ダッシュボタン（Shiftキー）
      _isRaid = true;
    }
  ...
  var isRapid_BattleButtonOn = function () {
    ...
    // それぞれの早送りボタンが有効で、かつそのボタンの、いずれかが、「短く」押されていたら、true。
    if (_isRapid_BattleButton_ok === true && Input.isPressed("ok")) {
      // 決定ボタン（ok/Enter/zキーなど）
      _isRaid = true;
    }
    if (_isRapid_BattleButton_cancel === true && Input.isPressed("cancel")) {
      // 決定ボタン（xキー）
      _isRaid = true;
    } else if (
      _isRapid_BattleButton_shift === true &&
  ...
  Game_Interpreter.prototype.pluginCommandAddRead_minRPG_CustomizeGameSpeed =
  ...
  var _Game_Interpreter_pluginCommand =
    Game_Interpreter.prototype.pluginCommand; // (1)
  ...
  if (_isOnChangeParamLimit_inThisPlugin === true) {
    // パラメータ「【機能】ステータス限界値変更」がONなら
  ...
  if (_isOnChangeSpeed_inThisPlugin === true) {
    // パラメータ「【機能】スピード変更」がONなら
    // =============================================================================
    // ゲームスピード調整
    ...
    Window_BattleLog.prototype.updateWaitCount = function () {
      // 元メソッドの返り値は、this._waitCount>0ならtrue、それ以外ならfalseだよ。
      if (this._waitCount > 0) {
        ...
        if (
          ...
          if (_gameSpeed === Infinity || _gameSpeed === -Infinity) {
            _reduceCount = _BATTLE_LOGSPEED_DEFAULT;
          } // 無限大なら、デフォルトスピードで。
          else if (_gameSpeed === 0) {
            _reduceCount = _BATTLE_LOGSPEED_DEFAULT;
          } // 0が入っていたら、デフォルトスピード(2)で。
          else if (_gameSpeed === 1) {
            _reduceCount = _BATTLE_LOGSPEED_DEFAULT * 0.5;
          } else if (_gameSpeed === 2) {
            _reduceCount = _BATTLE_LOGSPEED_DEFAULT * 0.7;
          } else if (_gameSpeed === 3) {
            _reduceCount = _BATTLE_LOGSPEED_DEFAULT * 0.8;
          } else if (_gameSpeed === 4) {
            _reduceCount = _BATTLE_LOGSPEED_DEFAULT * 0.9;
          } else if (_gameSpeed === 5) {
            _reduceCount = _BATTLE_LOGSPEED_DEFAULT;
          } // 5が入っていたら、デフォルトスピード(2)で。
          else if (_gameSpeed === 6) {
            _reduceCount = _BATTLE_LOGSPEED_DEFAULT * 1.2;
          } // 6が入っていたら、デフォルトスピード*1.2で。
          else if (_gameSpeed === 7) {
            _reduceCount = _BATTLE_LOGSPEED_DEFAULT * 1.5;
          } // 6が入っていたら、デフォルトスピード*1.5で。
          else if (_gameSpeed === 10) {
            _reduceCount = 99;
          } // 最速の10が入っていたら、99で。
          else if (_gameSpeed < 1) {
        ...
        if (isFastRapidButton()) {
          // 元メソッドの早送り条件である this.isFastForward() とは、厳密には対応ボタンや長押し設定などが違うので、注意して。
          // 変数番号を格納している、パラメータに変な値が入っていないか、随時チェック。
          if (
            _VarNo_Rapid_BattleRate != null &&
            _VarNo_Rapid_BattleRate >= 1 &&
            _VarNo_Rapid_BattleRate < $dataSystem.variables.length
          ) {
            _reduceCount = Number.parseInt(
              $gameVariables.value(_VarNo_Rapid_BattleRate)
            );
            // 変数の値に変な値が入っていないか、随時チェック。
            if (_reduceCount === Infinity || _reduceCount === -Infinity) {
              _reduceCount = _RAPID_BATTLE_LOGSPEED_DEFAULT;
            } // 無限大なら、デフォルトスピードで。
            else if (_reduceCount === 0) {
              _reduceCount = _RAPID_BATTLE_LOGSPEED_DEFAULT;
            } // 0が入っていたら、デフォルトスピード(-2カウント)で。
            else if (_reduceCount < 1) {
              _reduceCount = 1;
            } // ここだけ、0は許可しない。1未満なら1に。ボタン長押しでゆっくりにしたい時に。
            else if (_reduceCount > 99) {
              _reduceCount = 99;
            } // これ以上の高速化は無意味。早くならない。
          } else {
            _reduceCount = _RAPID_BATTLE_LOGSPEED_DEFAULT;
        ...
        // 4.1. speedの例外チェック
        if (
          Number.isNaN(_reduceCount) ||
          _reduceCount == null ||
          _reduceCount === Infinity ||
          _reduceCount === -Infinity
        ) {
          if (_isDebugMode()) {
            console.log(
        ...
        if (
          Number.isNaN(this._waitCount) ||
          this._waitCount == null ||
          this._waitCount === Infinity ||
          this._waitCount === -Infinity
        ) {
          if (_isDebugMode()) {
            console.log(
    ...
    var _Scene_Map_encounterEffectSpeed =
      Scene_Map.prototype.encounterEffectSpeed;
    ...
    Sprite_Animation.prototype.update = function () {
      ...
      if (
        ...
        // 変数の値に変な値が入っていないか、随時チェック。
        if (_gameSpeed === Infinity || _gameSpeed === -Infinity) {
          _speed = _BATTLE_ANIMATIONSPEED_DEFAULT;
        } // 無限大なら、デフォルトスピードで。
        else if (_gameSpeed === 0) {
          _speed = _BATTLE_ANIMATIONSPEED_DEFAULT;
        } // 0が入っていたら、デフォルトスピードで。
    ...
    Sprite_Enemy.prototype.update = function () {
      ...
      if (this._enemy) {
        ...
        if (
          ...
          // 変数の値に変な値が入っていないか、随時チェック。
          if (_gameSpeed === Infinity || _gameSpeed === -Infinity) {
            _speed = _BATTLE_ENEMYSPEED_DEFAULT;
          } // 無限大なら、デフォルトスピードで。
          else if (_gameSpeed === 0) {
            _speed = _BATTLE_ENEMYSPEED_DEFAULT;
          } // 0が入っていたら、デフォルトスピードで。
    ...
    Sprite_Weapon.prototype.animationWait = function () {
      ...
      if (
        ...
        // 変数の値に変な値が入っていないか、随時チェック。
        if (_gameSpeed === Infinity || _gameSpeed === -Infinity) {
          _speed = _BATTLE_MOTIONSPEED_DEFAULT;
        } // 無限大なら、デフォルトスピードで。
        else if (_gameSpeed === 0) {
          _speed = _BATTLE_MOTIONSPEED_DEFAULT;
        } // 0が入っていたら、デフォルトスピードで。
    ...
    Sprite_Actor.prototype.updateMove = function () {
      ...
      if (!bitmap || bitmap.isReady()) {
        ...
        if (
          ...
          // 変数の値に変な値が入っていないか、随時チェック。
          if (_gameSpeed === Infinity || _gameSpeed === -Infinity) {
            _speed = _BATTLE_MOVESPEED_DEFAULT;
          } // 無限大なら、デフォルトスピードで。
          else if (_gameSpeed === 0) {
            _speed = _BATTLE_MOVESPEED_DEFAULT;
          } // 0が入っていたら、デフォルトスピードで。
    ...
    Scene_Map.prototype.isFastForward = function () {
      ...
      _isFastForwardButtonPressed =
        ...
        ((_isRapid_BattleButton_ok === true && Input.isLongPressed("ok")) || // 決定ボタン早送りモードが有効＆長押し // または
          (_isRapid_BattleButton_cancel === true &&
            Input.isLongPressed("cancel")) || // キャンセルボタン早送りモードが有効＆長押し // または
          (_isRapid_BattleButton_shift === true &&
            Input.isLongPressed("shift")) || // ダッシュボタン早送りモードが有効＆長押し // または
    ...
    var _Window_Message_processEscapeCharacter =
      Window_Message.prototype.processEscapeCharacter; // 元メソッドにreturnはないよ。
    Window_Message.prototype.processEscapeCharacter = function (
      ...
      var _waitFrameNum =
        Number.parseInt(
          _WaitPeriodDef / getMessageWaitEscapeCharacterSpeed()
        ) || 0;
      ...
      switch (code) {
        case ".":
          if (isSkipModeOn() === false) {
            ...
            if (_gameSpeed !== 10) {
              ...
              if (_waitFrameNum === Infinity || _waitFrameNum === -Infinity)
                _waitFrameNum = 15; // ±Infinityになる理由は、0割りでもしたのかな。とりあえずデフォルト値にします。
        ...
        case "|":
          if (isSkipModeOn() === false) {
            ...
            _waitFrameNum = Number.parseInt(_waitFrameNum) || 0; // NaNや数値にならない場合も、0になります。±Infinityだけ例外処理が必要です。
            if (_waitFrameNum === Infinity || _waitFrameNum === -Infinity)
              _waitFrameNum = 60; // ±Infinityになる理由は、0割りでもしたのかな。とりあえずデフォルト値にします。
            this.startWait(
              ConvertEscapeCharacters(Number.parseInt(_WaitLineDef))
            ); // 独自の方法で、このパラメータ分だけウェイトするよ（元のツクールもともとの待ち時間は無視するよ）
            //}
          }
          break;
        default:
    ...
    var _Window_Message_updateShowFast =
      Window_Message.prototype.updateShowFast;
    ...
    Window_Message.prototype.updateInput = function () {
      // 元のメソッドのreturnは複雑だから、返り値をそのまま使うよ。
    ...
    var _Window_BattleLog_messageSpeed =
      Window_BattleLog.prototype.messageSpeed;
    Window_BattleLog.prototype.messageSpeed = function () {
      // 元メソッドはバトルログ表示後の待ち時間を返すよ。
  ...
  if (_isOnControlBattleEncountAndWinLose === true) {
    // パラメータ「【機能】戦闘を検知するか」がONなら
    ...
    Game_Interpreter.prototype.command301 = function () {
      ...
      if (
        _switchNo != null &&
        _switchNo >= 1 &&
        _switchNo < $dataSystem.switches.length
      ) {
      ...
      if (!$gameParty.inBattle()) {
        if (this._params[0] <= 1) {
          // パラメータの1番目が2「ランダムエンカウント」以外なら、イベント戦闘
      ...
      var _troopId = 0;
      if (this._params[0] === 0) {
        // Direct designation                 // 敵グループID直接指定
        _troopId = this._params[1];
      } else if (this._params[0] === 1) {
        // Designation with a variable // 敵グループを変数名で指定
        _troopId = $gameVariables.value(this._params[1]);
      } else {
        // Same as Random Encounter                               // ランダムエンカウント
        _troopId = $gamePlayer.makeEncounterTroopId();
      }
      // 最後に戦闘した敵グループIDを格納する変数番号の変数を更新するよ。 // パラメータに変な値が入っていないか、随時チェック。
      if (
        _VarNo_LastBattleTroopID != null &&
        _VarNo_LastBattleTroopID >= 1 &&
        _VarNo_LastBattleTroopID < $dataSystem.variables.length
      ) {
        ...
        console.log(
          "minRPG_CustomizeGameSpeed: パラメータ「最後に戦闘した敵グループIDを格納する変数番号」に変な値が入っています。: " +
            _VarNo_LastBattleTroopID
        );
      }

      if (_isDebugMode()) {
        console.log("イベント戦闘発生(敵グループID: " + _troopId + ")");
      }
    ...
    BattleManager.endBattle = function (result) {
      ...
      if (result === 0) {
        ...
        if (this._escaped) {
          ...
          // 逃げていない。つまり、戦闘参加メンバーが０人か、イベントコマンド「戦闘の中断」をした時。
          // 中断した時。元ソースはなにもしない。
        }
      } else if (result === 2) {
    ...
    BattleManager.updateBattleEnd = function () {
      ...
      if (this.isBattleTest()) {
        // バトルテストならそこでシナリオ強制終了（元ソースを呼び出す）
        _BattleManager_updateBattleEnd.apply(this, arguments); // 上書き前のメソッド（元ソース）を呼び出す
      } else if ($gameParty.isAllDead()) {
        // 味方キャラが全員戦闘不能になっていたら。全員マヒとかで全滅っていうがこれに含まれるかは、今のところは未検証。
        // 味方キャラが全員戦闘不能になっていたら、この戦闘が負けた時の分岐があるか、ないかを調べる。
        if (this._canLose) {
          // 負けた時の分岐が在る戦闘（負けても大丈夫なボス戦等）だったら、全滅スイッチはOFFです。戦闘不能のメンバーを蘇らせ、シナリオを継続。
          _BattleManager_updateBattleEnd.apply(this, arguments); // 上書き前のメソッド（元ソース）を呼び出す
        } else {
          // 負けた時の分岐が無い戦闘だったら、
          // デフォルトのソースは以下の一行。
        ...
        // 全滅じゃなく（味方キャラが１人は生きていて）、かつ逃走でも、中断でもない（このメソッドは中断時は呼び出されないはず）。すなわち、戦闘に勝ったとき
        _BattleManager_updateBattleEnd.apply(this, arguments); // 上書き前のメソッド（元ソース）を呼び出す
      }
      this._phase = null;
    };
  ...
  if (_isOnChangeDefault_Migawari === true) {
    // パラメータ「【機能】身代わり条件を変更するか」がONなら、
    // =================================================================================
    ...
    BattleManager.checkSubstitute = function (target) {
      ...
      if (!this._action.isCertainHit()) {
        // 必中でも守るようにしたらダメだよ。回復とか補助効果でも守っちゃう。
        // 条件2. かばわれる側(target)が生きていて、HPが指定％以下だったら、
        if (
          target != null &&
          target.isAlive() &&
          target._hp <= (target.mhp * _Migawawri_Sareru_MaxHPPercent) / 100
        ) {
          // 条件2. かばわれる側(target)のMPが指定％以下だったら、
          if (
            target._mp <=
            (target.mmp * _Migawawri_Sareru_MaxMPPercent) / 100
          ) {
            // デバッグ用。テストプレイ中だけ。実行。
            if (_isDebugMode()) {
              console.log(
                "身代わり条件の変更：\n" +
                  target.name() +
                  "のHP％ " +
                  Math.floor((target._hp / target.mhp) * 100) +
                  " が " +
                  _Migawawri_Sareru_MaxHPPercent +
                  "％以下で、かつMP％ " +
                  Math.floor((target._mp / target.mmp) * 100) +
                  " が " +
                  _Migawawri_Sareru_MaxMPPercent +
                  "％以下です。"
              );
            }
            _isTargetPinch = true;
    ...
    BattleManager.applySubstitute = function (target) {
      ...
      if (
        target != null &&
        target.friendsUnit() != null &&
        target.friendsUnit().substituteBattler() != null &&
            ...
            if (_members[i] != null && _members[i].isSubstitute()) {
              ...
              if (target !== substitute) {
                ...
                if (
                  substitute.isAlive() &&
                  substitute._hp >=
                    (substitute.mhp * _Migawawri_Suru_MinHPPercent) / 100
                ) {
                  // 条件2. かばう側(substitute)のMPが指定％以上だったら、
                  if (
                    substitute._mp >=
                    (substitute.mmp * _Migawawri_Suru_MinMPPercent) / 100
                  ) {
                    // 条件3. 乱数が一定以下だったら、
                    if (Math.random() * 100 <= _Migawawri_Suru_ActPercent) {
                      ...
                      if (_isDamagedCharaActor) {
                        // 敵なのか、味方なのかで、格納する変数を分岐
                        // デバッグ用。テストプレイ中だけ。実行。
                        if (_isDebugMode()) {
                          console.log(
                            "★かばう成功: " +
                              ...
                              "のHP％ " +
                              Math.floor(
                                (substitute._hp / substitute.mhp) * 100
                              ) +
                              " が " +
                              _Migawawri_Suru_MinHPPercent +
                              "％以上で、MP％ " +
                              Math.floor(
                                (substitute._mp / substitute.mmp) * 100
                              ) +
                              " が " +
                              _Migawawri_Suru_MinMPPercent +
                              "％以上で、かつ" +
                              _Migawawri_Suru_ActPercent +
                              "％の確率を満たしたので、\n" +
                              substitute.name() +
                              "が" +
                              target.name() +
                        ...
                        // 変数番号を格納している、パラメータに変な値が入っていないか、随時チェック。
                        if (
                          _VarNo_Migawari_Sareta_AcorID != null &&
                          _VarNo_Migawari_Sareta_AcorID >= 1 &&
                          _VarNo_Migawari_Sareta_AcorID <
                        ...
                        // 変数番号を格納している、パラメータに変な値が入っていないか、随時チェック。
                        if (
                          _VarNo_Migawari_Shita_AcorID != null &&
                          _VarNo_Migawari_Shita_AcorID >= 1 &&
                          _VarNo_Migawari_Shita_AcorID <
                            $dataSystem.variables.length
                        ) {
                          $gameVariables.setValue(
                            _VarNo_Migawari_Shita_AcorID,
                        ...
                        var _commonEventNo =
                          _CommonEventNo_Migawari_Success_Actor;
                        if (
                          _commonEventNo != null &&
                          _commonEventNo >= 1 &&
                          _commonEventNo < $dataCommonEvents.length
                        ) {
                          if (_isDebugMode()) {
                            console.log(
                              " → 味方身代わり成功時コモンイベント " +
                          ...
                          $gameTemp.reserveCommonEvent(
                            $dataCommonEvents[_commonEventNo].list
                          ); // コモンイベントを呼び出します。
                        } else {
                          console.log(
                        ...
                        // 変数番号を格納している、パラメータに変な値が入っていないか、随時チェック。
                        if (
                          _VarNo_Migawari_Sareta_EnemyID != null &&
                          _VarNo_Migawari_Sareta_EnemyID >= 1 &&
                          _VarNo_Migawari_Sareta_EnemyID <
                            $dataSystem.variables.length
                        ) {
                          $gameVariables.setValue(
                            _VarNo_Migawari_Sareta_EnemyID,
                        ...
                        // 変数番号を格納している、パラメータに変な値が入っていないか、随時チェック。
                        if (
                          _VarNo_Migawari_Shita_EnemyID != null &&
                          _VarNo_Migawari_Shita_EnemyID >= 1 &&
                          _VarNo_Migawari_Shita_EnemyID <
                            $dataSystem.variables.length
                        ) {
                          $gameVariables.setValue(
                            _VarNo_Migawari_Shita_EnemyID,
                        ...
                        // 成功時、パラメータで指定したコモンイベント番号を実行する。
                        var _commonEventNo =
                          _CommonEventNo_Migawari_Success_Enemy;
                        if (
                          _commonEventNo != null &&
                          _commonEventNo >= 1 &&
                          _commonEventNo < $dataCommonEvents.length
  ...
  var _Tachi_Game_Battler_performSubstitute =
    Game_Battler.prototype.performSubstitute; // 元メソッドにreturnはないよ。
