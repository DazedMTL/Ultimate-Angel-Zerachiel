...
(function () {
  ...
  var parameters = $plugins.filter(function (p) {
    return p.description.contains("<BattleEffects>");
  })[0].parameters;
  ...
  Olivia.OctoBattle.BattleEffects = {
    ...
    WeakPopupReqRate: Number(
      parameters["Battle Effects Weak Popup Require Rate"] || 1.1
    ),
    WeakCellX: Number(parameters["Battle Effects Weak Popup Cell X"] || 4),
    WeakCellWidth: Number(
      parameters["Battle Effects Weak Popup Cell Width"] || 3
    ),
    WeakCellXFactor: Number(
      parameters["Battle Effects Weak Popup Cell X Factor"] || 0.25
    ),
    WeakCellYFactor: Number(
      parameters["Battle Effects Weak Popup Cell Y Factor"] || 0.6
    ),
    WeakMoveXBase: Number(
      parameters["Battle Effects Weak Popup Move X Base"] || -0.04
    ),
    WeakMoveXRate: Number(
      parameters["Battle Effects Weak Popup Move X Rate"] || 1.1
    ),
    WeakMoveYBase: Number(
      parameters["Battle Effects Weak Popup Move Y Base"] || 0
    ),
    WeakMoveYRate: Number(
      parameters["Battle Effects Weak Popup Move Y Rate"] || 0
    ),
    ...
    BreakCellWidth: Number(
      parameters["Battle Effects Break Popup Cell Width"] || 3
    ),
    BreakCellXFactor: Number(
      parameters["Battle Effects Break Popup Cell X Factor"] || 0.25
    ),
    BreakCellYFactor: Number(
      parameters["Battle Effects Break Popup Cell Y Factor"] || 0.6
    ),
    BreakMoveXBase: Number(
      parameters["Battle Effects Break Popup Move X Base"] || -0.04
    ),
    BreakMoveXRate: Number(
      parameters["Battle Effects Break Popup Move X Rate"] || 1.1
    ),
    BreakMoveYBase: Number(
      parameters["Battle Effects Break Popup Move Y Base"] || 0
    ),
    BreakMoveYRate: Number(
      parameters["Battle Effects Break Popup Move Y Rate"] || 0
    ),
    // Stacking Buff Turns
  ...
  Olivia.OctoBattle.Effects.___BattleManager_initMembers___ =
  ...
  Game_Action.prototype.isItemDivine = function () {
    return (
      this.item().note.match(/<Divine>/i) ||
      this.item().note.match(/<Bypass Target Change>/i)
    );
  };

  Olivia.OctoBattle.Effects.___Game_Action_isForUser___ =
    Game_Action.prototype.isForUser;
  Game_Action.prototype.isForUser = function () {
    if (this.isSkill()) {
      if (
        this.checkItemScope([11]) &&
        this.isSubjectAffectedByNote(/<Skill Target Change: Self to All>/i) &&
        !this.isItemDivine()
      ) {
        return false;
      }
      ...
      if (
        this.checkItemScope([11]) &&
        this.isSubjectAffectedByNote(/<Item Target Change: Self to All>/i) &&
        !this.isItemDivine()
      ) {
        return false;
      }
  ...
  Olivia.OctoBattle.Effects.___Game_Action_isForOne___ =
    Game_Action.prototype.isForOne;
  Game_Action.prototype.isForOne = function () {
    if (this.isSkill()) {
      if (
        this.checkItemScope([8]) &&
        this.isSubjectAffectedByNote(
          /<Skill Target Change Allies: All to One>/i
        ) &&
        !this.isItemDivine()
      ) {
        return true;
      } else if (
        this.checkItemScope([2]) &&
        this.isSubjectAffectedByNote(
          /<Skill Target Change Enemies: All to One>/i
        ) &&
        !this.isItemDivine()
      ) {
        return true;
      } else if (
        this.checkItemScope([7]) &&
        this.isSubjectAffectedByNote(
          /<Skill Target Change Allies: One to All>/i
        ) &&
        !this.isItemDivine()
      ) {
        return false;
      } else if (
        this.checkItemScope([1]) &&
        this.isSubjectAffectedByNote(
          /<Skill Target Change Enemies: One to All>/i
        ) &&
        !this.isItemDivine()
      ) {
        return false;
      ...
      if (
        this.checkItemScope([8]) &&
        this.isSubjectAffectedByNote(
          /<Item Target Change Allies: All to One>/i
        ) &&
        !this.isItemDivine()
      ) {
        return true;
        ...
        this.checkItemScope([2]) &&
        this.isSubjectAffectedByNote(
          /<Item Target Change Enemies: All to One>/i
        ) &&
        !this.isItemDivine()
      ) {
        return true;
        ...
        this.checkItemScope([7]) &&
        this.isSubjectAffectedByNote(
          /<Item Target Change Allies: One to All>/i
        ) &&
        !this.isItemDivine()
      ) {
        return false;
        ...
        this.checkItemScope([1]) &&
        this.isSubjectAffectedByNote(
          /<Item Target Change Enemies: One to All>/i
        ) &&
        !this.isItemDivine()
      ) {
        return false;
  ...
  Olivia.OctoBattle.Effects.___Game_Action_isForAll___ =
    Game_Action.prototype.isForAll;
  ...
  Olivia.OctoBattle.Effects.___Game_Action_needsSelection___ =
  ...
  Game_Action.prototype.needsSelection = function () {
    ...
    return Olivia.OctoBattle.Effects.___Game_Action_needsSelection___.call(
      this
    );
  };

  Olivia.OctoBattle.Effects.___Game_Action_makeDamageValue___ =
    Game_Action.prototype.makeDamageValue;
  Game_Action.prototype.makeDamageValue = function (target, critical) {
    this._isCalculatingDamage = true;
    var value =
      Olivia.OctoBattle.Effects.___Game_Action_makeDamageValue___.call(
        this,
        target,
        critical
      );
  ...
  Olivia.OctoBattle.Effects.___Game_Action_calcElementRate___ =
  ...
  Game_Action.prototype.calcElementRate = function (target) {
    if (!!this._isCalculatingDamage) {
      ...
      for (var i = 0; i < states.length; i++) {
        ...
        if (
          !!state &&
          state.note.match(/<All Element Damage Rate: (\d+)([%％])>/i)
        ) {
          return parseFloat(RegExp.$1) * 0.01;
        }
      }
    }
    return Olivia.OctoBattle.Effects.___Game_Action_calcElementRate___.call(
      this,
      target
    );
  };

  Olivia.OctoBattle.Effects.___Game_Action_executeDamage___ =
    Game_Action.prototype.executeDamage;
  Game_Action.prototype.executeDamage = function (target, value) {
    this.processPopupColorNote(target, value);
    Olivia.OctoBattle.Effects.___Game_Action_executeDamage___.call(
      this,
      target,
      value
    );
  ...
  Game_Action.prototype.processPopupColorNote = function (target, value) {
    if (!!target && value > 0 && this.isHpEffect()) {
      ...
      for (var i = 0; i < states.length; i++) {
        ...
        if (
          !!state &&
          state.note.match(/<Damage Color:[ ]*(\d+(?:\s*,\s*\d+)*)>/i)
        ) {
          target.result().colorSettings = JSON.parse(
            "[" + RegExp.$1.match(/\d+/g) + "]"
          );
        }
      }
  ...
  Game_Action.prototype.processWeakPopup = function (target, value) {
    if (
      ...
      value > 0 &&
      this.isHpEffect() &&
      this.calcElementRate(target) >=
        Olivia.OctoBattle.BattleEffects.WeakPopupReqRate
    ) {
      var states = target.states();
      for (var i = 0; i < states.length; i++) {
        var state = states[i];
  ...
  Olivia.OctoBattle.Effects.___Game_Action_applyItemUserEffect___ =
    Game_Action.prototype.applyItemUserEffect;
  Game_Action.prototype.applyItemUserEffect = function (target) {
    this.registerUserLastActionType();
    Olivia.OctoBattle.Effects.___Game_Action_applyItemUserEffect___.call(
      this,
      target
    );
  ...
  Olivia.OctoBattle.Effects.___Game_ActionResult_clear___ =
  ...
  Olivia.OctoBattle.Effects.___Game_BattlerBase_paySkillCost___ =
    Game_BattlerBase.prototype.paySkillCost;
  Game_BattlerBase.prototype.paySkillCost = function (skill) {
    Olivia.OctoBattle.Effects.___Game_BattlerBase_paySkillCost___.call(
      this,
      skill
    );
  ...
  Game_BattlerBase.prototype.payWeaponDestroy = function (skill) {
    this._destroyWeapon =
      skill.note.match(/<Destroy Weapon>/i) && this.isActor();
  };
  ...
  Olivia.OctoBattle.Effects.___Game_Battler_startAnimation___ =
  ...
  Game_Battler.prototype.startAnimation = function (
    ...
    Olivia.OctoBattle.Effects.___Game_Battler_startAnimation___.call(
      this,
      animationId,
      mirror,
      delay
  ...
  Olivia.OctoBattle.Effects.___Game_Battler_addState___ =
  ...
  Game_Battler.prototype.setupBreakDamagePopup = function (stateId, affected) {
    if (
      ...
      $dataStates[stateId].note.match(/<Break Popup>/i)
    ) {
  ...
  Game_Battler.prototype.setStateMaximumTurns = function (stateId) {
    if (
      this.isStateAffected(stateId) &&
      $dataStates[stateId].note.match(/<Max Turns: (\d+)>/i)
    ) {
      this._stateTurns[stateId] = Math.min(
  ...
  Game_Battler.prototype.isStatePrevented = function (stateId) {
    ...
    for (var i = 0; i < states.length; i++) {
      ...
      if (
        !!state &&
        state.note.match(
          /<State (?:Immune|Immunity):[ ]*(\d+(?:\s*,\s*\d+)*)>/i
        )
      ) {
  ...
  Olivia.OctoBattle.Effects.___Game_Battler_addBuff___ =
    Game_Battler.prototype.addBuff;
  Game_Battler.prototype.addBuff = function (paramId, turns) {
    if (!this.isBuffPrevented(paramId)) {
      var currentTurns = this._buffTurns[paramId] || 0;
      Olivia.OctoBattle.Effects.___Game_Battler_addBuff___.call(
        this,
        paramId,
        turns
      );
  ...
  Game_Battler.prototype.isBuffPrevented = function (buffId) {
    ...
    for (var i = 0; i < states.length; i++) {
      ...
      if (
        !!state &&
        state.note.match(/<Buff (?:Immune|Immunity):[ ]*(\d+(?:\s*,\s*\d+)*)>/i)
      ) {
        var array = JSON.parse("[" + RegExp.$1.match(/\d+/g) + "]");
        prevented = prevented.concat(array);
      }
  ...
  Game_Battler.prototype.setBuffTurnStacking = function (paramId, turns) {
    if (Olivia.OctoBattle.BattleEffects.StackBuffTurns) {
      this._buffTurns[paramId] = Math.min(
        turns,
        Olivia.OctoBattle.BattleEffects.MaxBuffTurns
      );
    }
  };

  Olivia.OctoBattle.Effects.___Game_Battler_addDebuff___ =
    Game_Battler.prototype.addDebuff;
  Game_Battler.prototype.addDebuff = function (paramId, turns) {
    if (!this.isDebuffPrevented(paramId)) {
      var currentTurns = this._buffTurns[paramId] || 0;
      Olivia.OctoBattle.Effects.___Game_Battler_addDebuff___.call(
        this,
        paramId,
        turns
      );
  ...
  Game_Battler.prototype.isDebuffPrevented = function (debuffId) {
    ...
    for (var i = 0; i < states.length; i++) {
      ...
      if (
        !!state &&
        state.note.match(
          /<Debuff (?:Immune|Immunity):[ ]*(\d+(?:\s*,\s*\d+)*)>/i
        )
      ) {
        var array = JSON.parse("[" + RegExp.$1.match(/\d+/g) + "]");
        prevented = prevented.concat(array);
  ...
  Game_Battler.prototype.setDebuffTurnStacking = function (paramId, turns) {
    if (Olivia.OctoBattle.BattleEffects.StackDebuffTurns) {
      this._buffTurns[paramId] = Math.min(
        turns,
        Olivia.OctoBattle.BattleEffects.MaxDebuffTurns
      );
    }
  };

  Olivia.OctoBattle.Effects.___Game_Battler_onAllActionsEnd___ =
  ...
  Game_Battler.prototype.performDestroyWeapon = function () {
    if (!!this._destroyWeapon && this.isActor()) {
      ...
      if (
        ...
        Olivia.OctoBattle.WeaponSwap.Enabled &&
        !!this.getFirstSwapWeapon()
      ) {
  ...
  Game_Battler.prototype.performFollowUpAction = function () {
    if (Imported.YEP_BattleEngineCore) {
      ...
      for (var i = 0; i < states.length; i++) {
        ...
        if (!!state) {
          if (
            this._lastActionHitType === "physical" &&
            state.note.match(/<Physical Follow Up Skill: (\d+)>/i)
          ) {
            var skillId = parseInt(RegExp.$1);
            BattleManager.queueForceAction(this, skillId, -1);
          } else if (
            this._lastActionHitType === "magical" &&
            state.note.match(/<Magical Follow Up Skill: (\d+)>/i)
          ) {
            var skillId = parseInt(RegExp.$1);
            BattleManager.queueForceAction(this, skillId, -1);
          } else if (
            this._lastActionHitType === "certain" &&
            state.note.match(/<Certain Follow Up Skill: (\d+)>/i)
          ) {
            var skillId = parseInt(RegExp.$1);
            BattleManager.queueForceAction(this, skillId, -1);
          } else if (
            this._lastActionHitType !== "certain" &&
            state.note.match(/<Follow Up Skill: (\d+)>/i)
          ) {
            var skillId = parseInt(RegExp.$1);
            BattleManager.queueForceAction(this, skillId, -1);
          }
  ...
  Olivia.OctoBattle.Effects.___Game_BattlerBase_meetsItemConditions___ =
  ...
  Game_BattlerBase.prototype.meetsItemConditions = function (item) {
    ...
    return Olivia.OctoBattle.Effects.___Game_BattlerBase_meetsItemConditions___.call(
      this,
      item
    );
  };
  ...
  Olivia.OctoBattle.Effects.___Game_Troop_expTotal___ =
  ...
  Olivia.OctoBattle.Effects.___Game_Troop_goldTotal___ =
  ...
  if (Imported.YEP_JobPoints) {
    Olivia.OctoBattle.Effects.___Game_Troop_jpTotal___ =
      Game_Troop.prototype.jpTotal;
  ...
  Olivia.OctoBattle.Effects.___Scene_Battle_isAnyInputWindowActive___ =
    Scene_Battle.prototype.isAnyInputWindowActive;
  Scene_Battle.prototype.isAnyInputWindowActive = function () {
    if (!!this._extraSkillWindow && this._extraSkillWindow.active) {
      return true;
    }
    return Olivia.OctoBattle.Effects.___Scene_Battle_isAnyInputWindowActive___.call(
      this
    );
  };

  Olivia.OctoBattle.Effects.___Scene_Battle_updateWindowPositions___ =
    Scene_Battle.prototype.updateWindowPositions;
  Scene_Battle.prototype.updateWindowPositions = function () {
    Olivia.OctoBattle.Effects.___Scene_Battle_updateWindowPositions___.call(
      this
    );
  ...
  Olivia.OctoBattle.Effects.___Scene_Battle_onSkillOk___ =
    Scene_Battle.prototype.onSkillOk;
  Scene_Battle.prototype.onSkillOk = function () {
    if (
      this._skillWindow
        .item()
        .note.match(/<Extra Skill List:[ ]*(\d+(?:\s*,\s*\d+)*)>/i)
    ) {
      var array = JSON.parse("[" + RegExp.$1.match(/\d+/g) + "]");
  ...
  Scene_Battle.prototype.createExtraSkillListWindow = function (array) {
    if (!this._extraSkillWindow) {
      ...
      this._extraSkillWindow.setHandler(
        "cancel",
        this.onExSkillCancel.bind(this)
      );
  ...
  Olivia.OctoBattle.Effects.___Scene_Battle_onActorOk___ =
  ...
  Olivia.OctoBattle.Effects.___Scene_Battle_onActorCancel___ =
  ...
  Olivia.OctoBattle.Effects.___Scene_Battle_onEnemyOk___ =
  ...
  Olivia.OctoBattle.Effects.___Scene_Battle_onEnemyCancel___ =
  ...
  Olivia.OctoBattle.Effects.___Sprite_Battler_setupDamagePopup___ =
  ...
  Sprite_Battler.prototype.setupOctoSpecialEffectDamagePopup = function () {
    if (
      ...
      if (
        !!this._battler._result._breakPopup &&
        Olivia.OctoBattle.BattleEffects.BreakPopupEnabled
      ) {
        this.setupBreakDamagePopup();
      } else if (
        !!this._battler._result._weakPopup &&
        Olivia.OctoBattle.BattleEffects.WeakPopupEnabled
      ) {
  ...
  Sprite_Battler.prototype.setupBreakDamagePopup = function () {
    ...
    sprite.x =
      this.x -
      Math.round(this.width * Olivia.OctoBattle.BattleEffects.BreakCellXFactor);
    sprite.y =
      this.y -
      Math.round(
        this.height * Olivia.OctoBattle.BattleEffects.BreakCellYFactor
      );
    sprite.createBreak();
  ...
  Sprite_Battler.prototype.setupWeakDamagePopup = function () {
    ...
    sprite.x =
      this.x -
      Math.round(this.width * Olivia.OctoBattle.BattleEffects.WeakCellXFactor);
    sprite.y =
      this.y -
      Math.round(this.height * Olivia.OctoBattle.BattleEffects.WeakCellYFactor);
    sprite.createWeak();
    BattleManager._spriteset.addChild(sprite);
    this._battler.clearResult();
  };
  ...
  Olivia.OctoBattle.Effects.___Sprite_Damage_setup___ =
  ...
  Olivia.OctoBattle.Effects.___Sprite_Damage_updateChild___ =
    Sprite_Damage.prototype.updateChild;
  Sprite_Damage.prototype.updateChild = function (sprite) {
    if (sprite._specialEffectPopup) {
      this.updateBreakPopup(sprite);
    } else {
      Olivia.OctoBattle.Effects.___Sprite_Damage_updateChild___.call(
        this,
        sprite
      );
    }
  ...
  Window_BattleSkillExtra.prototype = Object.create(
    Window_BattleSkill.prototype
  );
  Window_BattleSkillExtra.prototype.constructor = Window_BattleSkillExtra;

  Window_BattleSkillExtra.prototype.initialize = function (
    x,
    y,
    width,
    height
  ...
  Olivia.OctoBattle.Effects.___Window_ActorCommand_addItemCommand___ =
    Window_ActorCommand.prototype.addItemCommand;
  Window_ActorCommand.prototype.addItemCommand = function () {
    Olivia.OctoBattle.Effects.___Window_ActorCommand_addItemCommand___.call(
      this
    );
    if (!!this._actor) {
      this.applyItemSeal();
