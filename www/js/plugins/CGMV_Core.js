...
//-----------------------------------------------------------------------------
CGMV_Temp.prototype.approximateTimeValue = function (
  seconds,
  forceApproximation,
  approximateToUnitString
) {
  forceApproximation = forceApproximation || false;
  var value = [];
  if (forceApproximation) {
    value[0] = this.approximateTimeValueToUnit(
      seconds,
      approximateToUnitString
    );
    ...
    // 3060 seconds in an hour, 86400 seconds in a day
...
//-----------------------------------------------------------------------------
CGMV_Temp.prototype.approximateTimeValueToUnit = function (
  seconds,
  unitString
) {
  var value = 0;
...
// 8: DD/MM          (ex: 20/1)
//-----------------------------------------------------------------------------
CGMV_Temp.prototype.createDateText = function (
  day,
  month,
  year,
  format,
...
CGMV_Temp.prototype.getShortMonthName = function (month) {
  ...
  switch (month) {
    ...
    default:
      this.reportError(
        "getShortMonthName: Out of range",
        "CGMV Core",
        "Update CGMV Plugins"
      );
      monthName = "Unknown";
...
CGMV_Temp.prototype.lookupItem = function (type, id) {
  switch (type) {
    ...
    default:
      this.reportError(
        "Item type setup incorrectly",
        "CGMV Core",
        "Check item parameters set up through CGMV plugins"
      );
      return null;
  }
};
...
DataManager.extractSaveContents = function (contents) {
  ...
  contents.cgmv
    ? ($cgmv = contents.cgmv)
    : console.log("Could not load CGMV data!");
};
//-----------------------------------------------------------------------------
...
// Initialize
//-----------------------------------------------------------------------------
CGMV_Window_Scrollable.prototype.initialize = function (
  x,
  y,
  width,
  height,
...
CGMV_Window_Scrollable.prototype.processHandling = function () {
  if (this.isActive()) {
    if (
      ...
      (Input.isRepeated("cancel") || TouchInput.isCancelled())
    ) {
      this.processCancel();
    }
  }
...
CGMV_Window_Scrollable.prototype.updateScroll = function () {
  if (
    this.origin.y + this._windowHeight >= this._neededHeight &&
    this._scrollMode == 0
  ) {
