...
if (Imported.SoR_PartyCommandHUD)
  throw new Error(
    "[SoR_PartyCommandHUD] Do NOT import more than 2 types of <SoR_PartyCommandHUD> series."
  );
...
(function () {
  ...
  var IsActorStatusPositionAdjust =
    Boolean(Param["IsActorStatusPositionAdjust"] === "true") || false;
  var IsActorStatusPositionFixed =
    Boolean(Param["IsActorStatusPositionFixed"] === "true") || false;
  var PartyCommandWindow_Yratio = Number(
    Param["PartyCommandWindow_Y-ratio"] || 1.38
  );
  var PartyCommandWindow_width = Number(
    Param["PartyCommandWindow_width"] || 360
  );

  Window_PartyCommand.prototype.initialize = function () {
    var x = Graphics.boxWidth / 2 - this.windowWidth() / 2;
    var y =
      Graphics.boxHeight / 2 + this.windowHeight() * PartyCommandWindow_Yratio;
  ...
  if (IsActorStatusPositionAdjust) {
    Scene_Battle.prototype.updateWindowPositions = function () {
      ...
      if (IsActorStatusPositionFixed)
        statusX = Graphics.boxWidth / 2 - this._statusWindow.width / 2;
      else {
        if (
