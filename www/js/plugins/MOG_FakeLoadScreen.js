...
Moghunter.fakeLoad_FileName = String(
  Moghunter.parameters["File Name"] || "Picture_"
);
Moghunter.fakeLoad_NumberOfPictures = Number(
...
Moghunter.fakeLoad_WordFloat = String(
  Moghunter.parameters["Word Animation"] || "true"
);
...
Moghunter.fakeLoad_SoundFile = String(
  Moghunter.parameters["Sound File"] || "Magic4"
);
Moghunter.fakeLoad_SoundPitch = Number(
  Moghunter.parameters["Sound Pitch"] || 50
);
...
Game_Interpreter.prototype.pluginCommand = function (command, args) {
  ...
  if (command === "fakeloadbar") {
    if ($gameTemp._fakeloadEnabled) {
      ...
      if (args[5]) {
        $gameTemp._fakeload[3] = String(args[5]);
      }
...
SceneFakeLoadScreen.prototype.create = function () {
  ...
  this.createMeter();
  this.createWord();
  SoundManager.mogPlaySound(
    Moghunter.fakeLoad_SoundFile,
    Moghunter.fakeLoad_SoundPitch,
    100
  );
};
...
SceneFakeLoadScreen.prototype.needSetPicture = function () {
  if (!$gameTemp._fakeload[3]) {
    return false;
  }
  if ($gameTemp._fakeload[3] === "") {
    return false;
  }
  if ($gameTemp._fakeload[3].toLowerCase() == "random") {
    return false;
  }
  return true;
};
...
SceneFakeLoadScreen.prototype.createWord = function () {
  ...
  this._wordData[4] =
    String(Moghunter.fakeLoad_WordFloat) === "true" ? true : false;
...
SceneFakeLoadScreen.prototype.update = function () {
  ...
  if (this._meter.bitmap.isReady()) {
    this.updateMeter();
  }
  if (this._word.bitmap.isReady()) {
    this.updateWord();
  }
  if (this._duration[2]) {
    this.updateFadeEnd();
