...
(function () {
  ...
  var categories = parameters["Categories"]
    .replace(/^\s+|\s+$/gm, "")
    .split(",") || ["Blacksmithing", "Alchemy"];
  var craftingSounds = parameters["CraftingSounds"]
    .replace(/^\s+|\s+$/gm, "")
    .split(",") || ["Sword2", "Ice4"];
  ...
  var ingredientsText = String(
    parameters["Ingredients Text"] || "Ingredients:"
  );
  var currencyText = String(parameters["Currency Text"] || "Currency");
  var itemCraftedText = String(
    parameters["Item Crafted Text"] || "Item crafted:"
  );

  /*----------------------------------------------
   * Game_Interpreter
   *---------------------------------------------*/
  var _szcrsy_Game_Interpreter_pluginCommand =
  ...
  Game_Interpreter.prototype.addRecipes = function (args) {
    ...
    for (var i = 1; i < args.length; i++) {
      ...
      switch (regex[1]) {
        case "i":
          item = $dataItems[parseInt(regex[2])];
          break;
        case "w":
          item = $dataWeapons[parseInt(regex[2])];
          break;
        case "a":
          item = $dataArmors[parseInt(regex[2])];
          break;
      }
      if (item._ingredients == null) continue;
      if (
        book._recipe_book["recipes"].filter(function (obj) {
          return obj == item;
        }).length > 0
      )
        continue;
  ...
  Game_Interpreter.prototype.removeRecipes = function (args) {
    ...
    for (var i = 1; i < args.length; i++) {
      ...
      if (
        ...
        continue;
      book._recipe_book["recipes"].splice(
        book._recipe_book["recipes"].indexOf(item),
        1
      );
    }
  };
    ...
    dbs.forEach(function (db) {
      db.filter(function (obj) {
        ...
        if (item.note.match(/<recipe>[\s\S]*<\/recipe>/gim)) {
          ...
          item.note
            .match(/([iwa]:\s*\d+,\s*\d+|[c]:\s*\d+)/gim)
            .forEach(function (ing_line) {
              var ing = ing_line.match(
                /([iwa]):\s*(\d+),\s*(\d+)|([c]):\s*(\d+)/i
              );
              var ing_db;
              var needed;
              if (ing[1] != null) {
                switch (ing[1]) {
                  case "w":
                    ing_db = $dataWeapons[parseInt(ing[2])];
                    break;
                  case "a":
                    ing_db = $dataArmors[parseInt(ing[2])];
                    break;
                  case "i":
                    ing_db = $dataItems[parseInt(ing[2])];
                    break;
                }
              } else {
                if (ing[4] == "c") {
  ...
  DataManager.makeSaveContents = function () {
    ...
    $dataItems
      .filter(function (item) {
        return item != null && item._recipe_book != null;
      })
      .forEach(function (item) {
        var rb_ids = [];
  ...
  DataManager.extractSaveContents = function (contents) {
    ...
    for (var key in recipe_books) {
      ...
      recipe_books[key].forEach(function (book) {
        ...
        switch (rex[1]) {
          case "i":
            $dataItems[key]._recipe_book["recipes"].push(
              $dataItems[parseInt(rex[2])]
            );
            break;
          case "w":
            $dataItems[key]._recipe_book["recipes"].push(
              $dataWeapons[parseInt(rex[2])]
            );
            break;
          case "a":
            $dataItems[key]._recipe_book["recipes"].push(
              $dataArmors[parseInt(rex[2])]
            );
            break;
        }
      }, this);
  ...
  Scene_CraftingMenu.prototype.create = function () {
    ...
    this._indexWindow = new Window_CraftingItems(
      ...
      288,
      Graphics.height - this._categoryWindow.height,
      this._category
    );
    ...
    this._statusWindow = new Window_CraftingItemStatus(
      this._indexWindow.width,
      this._categoryWindow.height,
      Graphics.width - this._indexWindow.width
    );
    this.addWindow(this._statusWindow);
    this._indexWindow.setStatusWindow(this._statusWindow);
    // Create Item Ingredients window
    this._ingredientsWindow = new Window_CraftingIngredients(
      this._indexWindow.width,
      this._statusWindow.y + this._statusWindow.height,
      500,
      Graphics.height - this._statusWindow.y - this._statusWindow.height
    );
  ...
  Scene_CraftingMenu.prototype.onCraft = function () {
    ...
    $gameMessage.setPositionType(1) - 100;
  ...
  Window_CraftingItems.prototype.setIngredientsWindow = function (
    ingredientsWindow
  ) {
  ...
  Window_CraftingItems.prototype.refresh = function () {
    ...
    $gameParty
      ...
      .filter(function (item) {
        return (
          item._recipe_book != null &&
          cat === parseInt(item._recipe_book["category"])
        );
      })
      .forEach(function (item) {
  ...
  Window_CraftingIngredients.prototype.initialize = function (
    x,
    y,
    width,
    height
  ...
  Window_CraftingIngredients.prototype.refresh = function () {
    ...
    this._item._ingredients.forEach(function (ing) {
      ...
      if (existingCount < needed) this.changeTextColor(this.crisisColor());
      else this.changeTextColor(this.powerUpColor());
      this.drawText(
        existingCount,
        x,
        y,
        width - this.textWidth("/" + needed),
        "right"
      );
      this.resetTextColor();
    }, this);
