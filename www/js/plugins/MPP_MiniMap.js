...
(() => {
  ...
  const param_DisplayPositions = JSON.parse(
    parameters["Display Positions"] || "[]",
    reviverParse
  );
  ...
  const param_VehicleMarkers = JSON.parse(
    parameters["Vehicle Markers"] || "{}"
  );
  ...
  Game_MinimapMarker.prototype.pos = function (mapId, x, y) {
    return this._mapId === mapId && this._x === x && this._y === y;
  };
  ...
  Game_Minimap.prototype.setupZoom = function () {
    this._zoom = this.metadataZoom() || this.paramsZoom() || this._targetZoom;
  ...
  Game_Minimap.prototype.zoomTo = function (zoom) {
    this._zoom =
      zoom || this.metadataZoom() || this.paramsZoom() || this._targetZoom;
    this._targetZoom = this._zoom;
    this.refreshZoom();
    this.updateScroll();
  };
  ...
  Game_Minimap.prototype.removeMarker = function (tag) {
    this._markers = this._markers.filter((marker) => marker.tag() !== tag);
  };

  Game_Minimap.prototype.removeMarkerXy = function (x, y) {
    const mapId = $gameMap.mapId();
    this._markers = this._markers.filter((marker) => !marker.pos(mapId, x, y));
  };
  ...
  Game_Event.prototype.getMinimapMarkerStr = function () {
    if (this._pageIndex >= 0) {
      return this.getCommentMinimapMarkerStr() || this.event().meta.Marker;
    }
    return "";
  };
  ...
  Sprite_MinimapMarker.prototype.paintHighlightedArrow = function (
    x,
    y,
    radian
  ) {
