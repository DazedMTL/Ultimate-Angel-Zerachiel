...
(function () {
  ...
  param.IconIndexWithoutShowTurns = (param.IconIndexWithoutShowTurns || []).map(
    function (index) {
      return parseInt(index);
    }
  );
  param.IconIndexWithoutRing = (param.IconIndexWithoutRing || []).map(function (
    index
  ) {
    return parseInt(index);
  });
  ...
  if (param.ActorRingIcon) {
    var _Sprite_Actor_createStateSprite =
  ...
  var _Sprite_Enemy_updateStateSprite =
  ...
  Sprite_StateIcon.prototype.isRingView = function () {
    ...
    var limit = this._battler.isActor()
      ? param.LineViewLimitActor
      : param.LineViewLimit;
    if (limit === 0) {
      return false;
  ...
  Sprite_StateIconChild.prototype.makeTurnSpriteIfNeed = function () {
    ...
    sprite.bitmap = new Bitmap(
      Sprite_StateIcon._iconWidth,
      Sprite_StateIcon._iconHeight
    );
  ...
  if (param.ShowActorTurnCount) {
    var _Window_BattleStatus_drawActorIcons =
      Window_BattleStatus.prototype.drawActorIcons;
    ...
    Window_BattleStatus.prototype.drawActorIconsTurn = function (actor, x, y) {
      ...
      for (var i = 0; i < this._drawIconIndexList.length; i++) {
        if (
          !param.IconIndexWithoutShowTurns.contains(this._drawIconIndexList[i])
        ) {
    ...
    if (typeof Battle_Hud !== "undefined") {
      ...
      Battle_Hud.prototype.create_states = function () {
        ...
        this._state_icon_turn = new Sprite(
      ...
      Battle_Hud.prototype.refresh_states = function () {
        ...
        if (
          turn &&
          !param.IconIndexWithoutShowTurns.contains(this._states_data[0])
        ) {
          this._state_icon_turn.bitmap.drawText(
            turn,
            0,
            0,
            Window_Base._iconWidth,
