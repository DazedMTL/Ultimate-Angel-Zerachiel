...
(function () {
  ...
  DataManager.bfExps = function (item) {
    ...
    var ary = [];
    if (
      (item && item.meta["陣形経験値設定"]) ||
      item.meta["FormationExpSetting"]
    ) {
  ...
  Game_Actor.prototype.checkFormationEnable = function (cond, id) {
    ...
    if (
      this.traitObjects().some(function (t) {
  ...
  Game_Party.prototype.winExp = function () {
    ...
    $gameTroop.members().forEach(
      function (e) {
        if (e.enemy().meta["陣形経験値"])
          value += parseInt(e.enemy().meta["陣形経験値"], 10);
        if (e.enemy().meta["BattleFormationExp"])
          value += parseInt(e.enemy().meta["BattleFormationExp"], 10);
      }.bind(this)
    );
    $gameParty.battleMembers().forEach(
      function (m) {
        m.traitObjects().forEach(
          function (t) {
            if (t.meta["陣形経験値倍率"] !== undefined)
              value *= Number(t.meta["陣形経験値倍率"]);
            if (t.meta["BattleFormationExpRate"] !== undefined)
              value *= Number(t.meta["BattleFormationExpRate"]);
          }.bind(this)
        );
      }.bind(this)
    );
  ...
  Window_BFFormationList.prototype.initialize = function (y) {
    if ($gameParty.battleFormation() === undefined)
      $gameParty.setBattleFormation(null);
    //this._oy = y;
  ...
  Window_BFFormationList.prototype.maxPageItems = function () {
    ...
    var max =
      (Graphics.boxHeight - hw - this.standardPadding() * 2) /
      this.lineHeight();
  ...
  Window_BFFormationList.prototype.refresh = function () {
    ...
    var mh =
      this.lineHeight() * this.maxPageItems() + this.standardPadding() * 2;
    this.height = Math.min(
      l * this.lineHeight() + this.standardPadding() * 2 + this.offsetHeight(),
      mh
    );
  ...
  Window_BFFormationList.prototype.drawItem = function (index) {
    ...
    if (this._list[index].ext === $gameParty.battleFormation())
      this.changeTextColor("rgba(128,255,128,1.0)");
    ...
    if (exps.length > 0) {
      ...
      var rate =
        next - crt > 0 ? Math.min((exp - crt) / (next - crt), 1.0) : 1.0;
      this.drawFormationLevel(rect, lv, max, item);
      var gc1 =
        item.meta["経験値ゲージカラー1"] ||
        item.meta["ExpGaugeColor1"] ||
        expGaugeColor1;
      var gc2 =
        item.meta["経験値ゲージカラー2"] ||
        item.meta["ExpGaugeColor2"] ||
        expGaugeColor2;
      this.drawGauge(
        rect.x + 172,
        rect.y + 8,
  ...
  Window_BFFormationList.prototype.drawFormationLevel = function (
    rect,
    lv,
    max,
    item
    ...
    var symbol =
      item.meta["LevelSymbol"] || item.meta["レベルシンボル"] || levelSymbol;
    var color = Number(
      item.meta["SymbolColor"] || item.meta["シンボルカラー"] || levelColor
    );
    ...
    if (isNaN(symbol)) {
      ...
      var w = Number(symbol);
      for (var i = 0; i < lv; i++)
        this.contents.fillRect(rect.x + (w + 1) * i, rect.y + 38, w, 16, color);
      for (var i = lv; i < max; i++)
  ...
  Window_BFParameter.prototype.refresh = function () {
    if (showParamWindow && this._commandWindow && this._statusWindow) {
      ...
      for (var i = 0, max = texts.length; i < max; i++) {
        ...
        if (
          text.match(
            /<(?:陣形効果テキスト追加|AddFormationEffectText):([前後FL]):?(?:Lv(\d+))?:(.+)>/
          )
        ) {
      ...
      for (var i = 0, max = c.learnings.length; i < max; i++) {
        ...
        if (
          ...
          l.level <= level &&
          !(
            l.note.includes("<陣形非表示>") ||
            l.note.includes("<HideFormation>")
          )
        ) {
          var text =
            "\\C[" +
      ...
      for (var i = 0, max = c.traits.length; i < max; i++) {
        ...
        if (
          !(trait.code === 23 && trait.dataId === 10) &&
          !(trait.code === 55 && trait.dataId === 0)
        ) {
          ary.push(trait);
        }
      }
      ...
      this.height =
        n * (size + 2) + this.lineHeight() + this.standardPadding() * 2 + 8;
    }
  };
  ...
  Window_BFInfo.prototype.refresh = function () {
    var text = $gameParty.battleFormation()
      ? $gameParty.battleFormation().name
      : basicFormationText;
  ...
  Scene_Menu.prototype.createCommandWindow = function () {
    ...
    this._commandWindow.setHandler(
      "battleFormation",
      this.commandBattleFormation.bind(this)
    );
  };
  ...
  Scene_BattleFormation.prototype.createBackSprite = function () {
    var back1 = new Sprite(
      ImageManager.loadBattleback1($gameMap._battleback1Name)
    );
    back1.move(-92, -58);
    var back2 = new Sprite(
      ImageManager.loadBattleback2($gameMap._battleback2Name)
    );
    back2.move(-92, -58);
    this.addChild(back1);
    this.addChild(back2);
