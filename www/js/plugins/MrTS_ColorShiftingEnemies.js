...
(function () {
  ...
  var paramToneSequence = String(
    parameters["Tone Sequence"] ||
      "[50, 50, 0, 0] [100, 0, 25, 0] [0, 0, 75, 0] [-50, 99, -10, 0] [90, 0, 0, 0]"
  );
  ...
  Sprite_Enemy.prototype.updateDiscoSteps = function () {
    ...
    if (this._currentDiscoStep >= paramChangeTime) {
      ...
      var r = Math.floor(
        ((nTone[0] - cTone[0]) / paramChangeTime) * this._currentDiscoStep
      );
      var g = Math.floor(
        ((nTone[1] - cTone[1]) / paramChangeTime) * this._currentDiscoStep
      );
      var b = Math.floor(
        ((nTone[2] - cTone[2]) / paramChangeTime) * this._currentDiscoStep
      );
      var gray = Math.floor(
        ((nTone[3] - cTone[3]) / paramChangeTime) * this._currentDiscoStep
      );
