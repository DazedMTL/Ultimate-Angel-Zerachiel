...
var parameters = $plugins.filter(function (p) {
  return p.description.contains("<HorrorEffects>");
})[0].parameters;
...
Olivia.HorrorEffects.___Game_System_initialize___ =
...
Game_System.prototype.createHorrorEffect = function (type) {
  ...
  if (type.match(/noise/i) && !this._horrorFilters.noiseFilter) {
    ...
    this._horrorFilters.glitchFilter.sliceMin = Math.ceil(
      Olivia.HorrorEffects.GlitchSlices / 2
    );
    this._horrorFilters.glitchFilter.sliceMax =
      Olivia.HorrorEffects.GlitchSlices;
    this._horrorFilters.glitchFilter.animated = Olivia.HorrorEffects.GlitchAni;
    this._horrorFilters.glitchFilter.aniFrequency =
      Olivia.HorrorEffects.GlitchAniFreq;
    this._horrorFilters.glitchFilter.aniStrength =
      Olivia.HorrorEffects.GlitchAniStr;
    ...
    this._horrorFilters.tvFilter.lineWidth =
      Olivia.HorrorEffects.TVLineThickness;
...
Olivia.HorrorEffects.___Game_Picture_initialize___ =
...
Olivia.HorrorEffects.___Game_BattlerBase_initialize___ =
...
Game_BattlerBase.prototype.setHorrorEffectToValue = function (
  type,
  key,
  value
) {
...
Olivia.HorrorEffects.___Game_CharacterBase_initMembers___ =
...
Game_CharacterBase.prototype.setHorrorEffectToValue = function (
  type,
  key,
  value
) {
...
Game_Player.prototype.refresh = function () {
  ...
  if (!!$gameParty.leader()) {
    this._horrorFilters = JsonEx.makeDeepCopy(
      $gameParty.leader()._horrorFilters
    );
  }
};
...
Game_Player.prototype.setHorrorEffectToValue = function (type, key, value) {
  if (!!$gameParty.leader()) {
    Game_System.prototype.setHorrorEffectToValue.call(
      $gameParty.leader(),
      type,
      key,
      value
...
Olivia.HorrorEffects.___Game_Follower_refresh___ =
...
Sprite.prototype.synchronizeHorrorFiltersWithSource = function () {
  if (
    !!this._horrorFiltersSource &&
    !!this._horrorFiltersSource._horrorFilters
  ) {
...
Sprite.prototype.updateHorrorGlitch = function () {
  if (!!this._horrorFilters.glitchFilter) {
    if (
      this._horrorFiltersGlitchSpecial &&
    ...
    if (
      this._horrorFiltersGlitchSpecial &&
      this._horrorFilters.glitchFilter.refreshRequest
    ) {
      this._horrorFilters.glitchFilter.refreshRequest = false;
...
Olivia.HorrorEffects.___Scene_Title_createBackground___ =
  Scene_Title.prototype.createBackground;
Scene_Title.prototype.createBackground = function () {
  Olivia.HorrorEffects.___Scene_Title_createBackground___.call(this);
  this.applyTitleHorrorEffects(
    this._backSprite1,
    Olivia.HorrorEffects.Title1Settings
  );
  this.applyTitleHorrorEffects(
    this._backSprite2,
    Olivia.HorrorEffects.Title2Settings
  );
};
...
Olivia.HorrorEffects.___Sprite_Character_initialize___ =
...
Olivia.HorrorEffects.___Sprite_Battler_setBattler___ =
...
Olivia.HorrorEffects.___Spriteset_Map_initialize___ =
...
Olivia.HorrorEffects.___Spriteset_Battle_initialize___ =
...
Olivia.HorrorEffects.___Game_Interpreter_pluginCommand___ =
  Game_Interpreter.prototype.pluginCommand;
Game_Interpreter.prototype.pluginCommand = function (command, args) {
  Olivia.HorrorEffects.___Game_Interpreter_pluginCommand___.call(
    this,
    command,
    args
  );
...
Game_Interpreter.prototype.getPluginCommandHorrorEffectsTarget = function (
  args
) {
  var text = args.shift();
...
Game_Interpreter.prototype.processPluginCommandHorrorEffects = function (
  target,
  args
) {
  ...
  if (command.match(/Create/i)) {
    ...
    alert(
      "Invalid HorrorEffect command type: please use Create, Remove, or Change"
    );
    return null;
  }
  var command = args.shift();
    ...
    if (type === 1) {
      ...
      if (type === "noise") {
        if (key.match(/rate/i)) {
          ...
          alert(
            "Invalid HorrorEffect Change format. Please refer to help file"
          );
          return;
        }
        // Glitch
