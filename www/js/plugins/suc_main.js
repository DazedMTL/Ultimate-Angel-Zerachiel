...
var Nnfs = {
	...
	parseDeep: function (params) {
		for (i in params) {
			...
			try {
				params[i] = JSON.parse(param, ((key, value) => {
	...
	drawDigit: function (bitmap, tex, value, x, y, fontSize, yOfs = 0, align = 0) {
		let width = 0;
	...
	replaceFormulaTag: function (desc, action, target, value1, value2) {
		if (desc.includes('%0')) {
			...
			if (item.damage.formula.contains("a.atk")) {
				...
				// 物理属性補正などはあとから計算される
				base_value = this.evalDamageFormulaEx(s, item, item.damage.formula.replace("a.atk", base_value));
				// console.log(base_value, s.paramBase(2) + s.paramPlus(2));
				if (base_value < final_value)
					desc = desc.replace('%0', "\\c[" + params._colorId[0] + "]" + final_value + "\\c[0]");
				else if (final_value < base_value)
					desc = desc.replace('%0', "\\c[" + params._colorId[1] + "]" + final_value + "\\c[0]");
				else
		...
		if (desc.includes('%1')) {
			...
			let buff = (action.item().meta.id != "shield" || value == 1) ? null : 1 < value;
			if (buff === true)
				desc = desc.replace('%1', "\\c[" + params._colorId[0] + "]" + value1 + "\\c[0]");
			else if (buff === false)
				desc = desc.replace('%1', "\\c[" + params._colorId[1] + "]" + value1 + "\\c[0]");
			else
				desc = desc.replace('%1', value1);
		}
		if (desc.includes('%2')) {
	...
	addHpoint: function (hpoint) {
		if ($gameSystem._hPoint == null)
			$gameSystem._hPoint = 0;
		$gameSystem._hPoint = ($gameSystem._hPoint + hpoint).clamp(0, params._tensionPointMax);
		$gameVariables.setValue(params._hPointVarId, $gameSystem._hPoint);
	},
	getSpriteFromBattler: function (battler) {
		if (battler.isActor())
			return BattleManager._spriteset._actorSprites.find(e => e._battler == battler);
		else
			return BattleManager._spriteset._enemySprites.find(e => e._battler == battler);
	},
	isContainsPointFromRect: function (pointX, pointY, sx, sy, width, height, padX, padY) {
		return sx + padX < pointX && pointX < sx + width - padX && sy < pointY + padY && pointY < sy + height - padY;
	},
...
params.CharaImageParam._skinsNormal = params.CharaImageParam._skinsNormal || ["normal", "nude huku pantu", "nude"];
params.CharaImageParam._skinsTransform = params.CharaImageParam._skinsTransform || ["change", "change", "change"];
params._tensionNgIconPos = params._tensionNgIconPos || [0, 0];
for (let i = 0; i < params.CharaImageParam._skinsNormal.length; ++i)
	params.CharaImageParam._skinsNormal[i] = params.CharaImageParam._skinsNormal[i].split(' ');
for (let i = 0; i < params.CharaImageParam._skinsTransform.length; ++i)
	params.CharaImageParam._skinsTransform[i] = params.CharaImageParam._skinsTransform[i].split(' ');
params.CardInfo._fontInfoName = params.CardInfo._fontInfoName || ["NotoSansJP-Bold", "40", "25", "125", "0", "center", "13", "#fff3ca", "5", "rgb(0, 0, 0)"];
params.CardInfo._fontInfoCost = params.CardInfo._fontInfoCost || ["NotoSansJP-Bold", "14", "8", "14", "28", "center", "28", "#ff0000", "4", "rgb(0, 0, 0)"];
params.CardInfo._fontInfoDesc = params.CardInfo._fontInfoDesc || ["NotoSansJP-Light", "14", "110", "-", "-", "-", "12", "#ffffff", "4", "rgb(0, 0, 0)"];
params.DescWindowParam._cardDescWindowWidth = params.DescWindowParam._cardDescWindowWidth || 175;
params.DescWindowParam._cardDescWindowOffset = params.DescWindowParam._cardDescWindowOffset || [80, -120];
params.DescWindowParam._fontInfo = params.DescWindowParam._fontInfo || ["GameFont", "0", "2", "-", "-", "-", "14", "#ffffff", "4", "rgb(0, 0, 0)"];
for (let i = 0; i < params.BattleActorParam._skins.length; ++i)
	params.BattleActorParam._skins[i] = params.BattleActorParam._skins[i].split(' ');
// console.log(params);
...
const point_half = new PIXI.Point(.5, .5);
//==============================
// * Math
//==============================
...
Math.randomRange = function (min, max) {
	if (Array.isArray(min))
		return Math.randomRange.apply(this, min);
	else if (arguments.length == 1)
		return Math.randomRange(0, min);
	return Math.random() * (max - min) + min;
};
Math.randomRangeInt = function (min, max) {
	if (Array.isArray(min))
		return Math.randomRangeInt.apply(this, min);
	else if (arguments.length == 1)
		return Math.randomRangeInt(0, min);
	min = Math.ceil(min);
	max = Math.floor(max);
...
// ** Coroutine
//==============================
function Coroutine() { throw new Error('This is a static class'); }
...
Coroutine.gLerpVec2 = function* (target, frame, sx, sy, dx, dy, way = Nnfs.EasingFunctions.linear) {
...
// *フォントスプライト
//==============================
function Sprite_Font() { this.initialize.apply(this, arguments); }
...
Sprite_Font.prototype.drawTextEx = function () { return Window_Base.prototype.drawTextEx.apply(this, arguments); };
Sprite_Font.prototype.resetFontSettings = function () {/*return Window_Base.prototype.resetFontSettings.apply(this, arguments);*/ };
...
Sprite_Font.prototype.standardFontSize = function () { return Window_Base.prototype.standardFontSize.apply(this, arguments); };
...
Sprite_Font.prototype.convertEscapeCharacters = function () { return Window_Base.prototype.convertEscapeCharacters.apply(this, arguments); };
Sprite_Font.prototype.convertExtraEscapeCharacters = function () { return Window_Base.prototype.convertExtraEscapeCharacters.apply(this, arguments); };
Sprite_Font.prototype.convertWordWrapEscapeCharacters = function () { return Window_Base.prototype.convertWordWrapEscapeCharacters.apply(this, arguments); };
Sprite_Font.prototype.calcTextHeight = function () { return /*this.textHeight ||*/ Window_Base.prototype.calcTextHeight.apply(this, arguments); };
Sprite_Font.prototype.processCharacter = function () { return Window_Base.prototype.processCharacter.apply(this, arguments); };
Sprite_Font.prototype.processNewLine = function () { return Window_Base.prototype.processNewLine.apply(this, arguments); };
Sprite_Font.prototype.processNormalCharacter = function () { return Window_Base.prototype.processNormalCharacter.apply(this, arguments); };
Sprite_Font.prototype.needWrap = function () { return Window_Base.prototype.needWrap.apply(this, arguments); };
Sprite_Font.prototype.checkWordWrap = function () { return Window_Base.prototype.checkWordWrap.apply(this, arguments); }; // needwrapと競合してるような
Sprite_Font.prototype.setWordWrap = function () { return Window_Base.prototype.setWordWrap.apply(this, arguments); };
Sprite_Font.prototype.enableWordWrap = function () { return Window_Base.prototype.enableWordWrap.apply(this, arguments); };
Sprite_Font.prototype.obtainEscapeCode = function () { return Window_Base.prototype.obtainEscapeCode.apply(this, arguments); };
Sprite_Font.prototype.obtainEscapeParam = function () { return Window_Base.prototype.obtainEscapeParam.apply(this, arguments); };
...
Sprite_Font.prototype.textWidth = function () { return Window_Base.prototype.textWidth.apply(this, arguments); };
Sprite_Font.prototype.textColor = function () { return Window_Base.prototype.textColor.apply(BattleManager._skillWindow, arguments); };
Sprite_Font.prototype.changeTextColor = function () { return Window_Base.prototype.changeTextColor.apply(this, arguments); };
Sprite_Font.prototype.processDrawIcon = function () { return Window_Base.prototype.processDrawIcon.apply(this, arguments); };
Sprite_Font.prototype.drawIcon = function () { return Window_Base.prototype.drawIcon.apply(this, arguments); };
Sprite_Font.prototype.makeFontBigger = function () { return Window_Base.prototype.makeFontBigger.apply(this, arguments); };
Sprite_Font.prototype.makeFontSmaller = function () { return Window_Base.prototype.makeFontSmaller.apply(this, arguments); };
Sprite_Font.prototype.isValidFontRangeForPopup = function () { return false; };
...
(function () {
			...
			json.events.forEach(ee => {
				...
				if (ee.note.match('suc_book')) {
					...
					if (ee.pages[0].list[2].parameters.length !== 1) {
						console.log("想定外のsuc_bookを発見 3番目が不正値:", item.mapDisplayName, item.eventId);
						return;
					}
					item.name = ee.pages[0].list[2].parameters[0].replace(/text_indicator : */, "").replace(/:.*/, "");
					if (ee.pages[0].list[3].parameters.length !== 5) {
						console.log("想定外のsuc_bookを発見 4番目が不正値:", item.mapDisplayName, item.eventId);
						return;
					}
	...
	const playVoice = function (name) {
		AudioManager.playVoice({ 'name': name, 'volume': 90, 'pitch': 100 }, false, undefined);
	}
	//==============================
	// *カスタムプラグインコマンド
	//==============================
	...
	Game_Interpreter.prototype.pluginCommand = function (command, args) {
		...
		const f_move = function (args, self, target) {
			...
			if (self != null && target != null) {
				...
				$gameTemp.setCameraProp([-(Graphics.boxWidth / 2 - target.x) + args[4], target.y - Graphics.boxHeight / 2 + args[5], args[3]]);
				if (self._futureSightSprite)
					self._futureSightSprite.visible = false;
			}
			this.wait(t);
		}.bind(this);
		// 敵の前に移動
		if (command === 'move_to_enemy') {
			Makonet.MpiShowSpine.setAnimation(BattleManager._spriteset._actorSprites[0]._spine, params.BattleActorParam._animDash, true);
			Coroutine.pushTask(Coroutine.gWaitForFrames(+args[0], () => Makonet.MpiShowSpine.setAnimation(BattleManager._spriteset._actorSprites[0]._spine, params.BattleActorParam._animIdle, true)));
			f_move(args, BattleManager._spriteset._actorSprites[0], Nnfs.getSpriteFromBattler(BattleManager._targets[0]));
			Nnfs._enableUpdateTargetPosition[0] = false;
		}
		else if (command === 'move_to_actor') {
			const es = BattleManager._spriteset._enemySprites.find(e => e._battler == BattleManager._subject);
		...
		else if (command === 'hpoint_up_in_battle' || command === 'add_tension_in_battle') {
			Nnfs.addHpoint(+args[0]);
		...
		// セルフスイッチ
			...
			if (0 < self_sw_types.length) {
				self_sw_types.forEach(function (selfSwitchType) {
					$gameSelfSwitches.setValue([map_id, ev_id, selfSwitchType.toUpperCase()], on);
				});
			}
			else {
				$gameSelfSwitches.setValue([map_id, ev_id, self_sw_types.toUpperCase()], on);
			}
		}
		// セルフスイッチ検出
			...
			console.log('// 全マップのメモから「' + note_filter + '」で抽出したイベント');
			fs.readdir(base + '/data/', function (err, files) {
				if (err) throw err;
				files.filter(e => /^Map\d/.test(e)).forEach(e => {
					...
					json.events.forEach(ee => {
						...
						if (ee.note.match(note_filter)) {
							if (!title_output && (title_output = true))
								console.log('// ' + map_id + ' ' + json.displayName);
							console.log('$gameSelfSwitches.setValue([' + map_id + ', ' + ee.id + ', \'A\'], ' + is_on + '); // ' + ee.name);
						}
					});
				});
			});
		...
		// 戦闘時立ち絵のアニメーション変更
			...
			BattleManager._skillWindow._charaStand._overrideAnimName = args[0] === 'null' ? null : args[0];
		}
		// 戦闘時立ち絵のアニメーション変更
	...
	const Sprite_Actor_updateTargetPosition = Sprite_Actor.prototype.updateTargetPosition;
	...
	if (params.CardInfo._on) {
		...
		Sprite.prototype.getRectangle = function () {
			return new Rectangle(this.x - this.anchor.x * this.width, this.y - this.anchor.y * this.height, this.width, this.height);
		};
		//==============================
		// *カードバトルシステム:読み込み処理
		//==============================
		...
		const LoadDeck = function () {
			if (params.CardInfo._forceUseDeck)
				return LoadDeckFromFile();
			else
				return LoadDeckFromPossession();
		};
		const LoadDeckFromPossession = function () {
			...
			$gameParty.items()
				...
				.forEach(e => Array.prototype.push.apply(obj.deck, Array($gameParty.numItems(e)).fill(e.id)));
			// console.log("所持アイテムから追加されるカード:", $gameParty.items().filter(e => DataManager.isItem(e) && e.itypeId === 1 && e.meta.cost));
			// 所持装備
			$gameParty.members()[0].equips()
		...
		const LoadCard = function (filename) {
			...
			try {
				...
				console.log("カード画像が見つかりません。", base + "/img/pictures/card_system/" + filename + ".png");
				return ImageManager.loadPicture('card_system/card0');
			}
			return ImageManager.loadPicture("card_system/" + filename);
		};
		...
		Scene_Battle.prototype.createSkillWindow = function () {
			...
			this._skillWindow.setHandler('cancel', function () {
				this._skillWindow._state = Window_BattleCard.STATE_SELECT_OPTION;
				this._skillWindow.hideCommand();
				this._partyCommandWindow.setup();
				Coroutine.overrideTask("charaStandCoroutine", this._skillWindow._charaStand.coroutineHideChara());
			}.bind(this));
			this._skillWindow.setHandler('cursorup', function () {
				this._skillWindow._state = Window_BattleCard.STATE_SELECT_OPTION;
				this._skillWindow.hideCommand();
				this._skillWindow.hide();
		...
		Scene_Battle.prototype.onBattleCardDecided = function () {
			...
			if (null == item) {
				...
				if (Nnfs._commonEventIdTurnEnd) {
					BattleManager._interpreter.setup($dataCommonEvents[Nnfs._commonEventIdTurnEnd].list, 0);
					Coroutine.pushTask(function* () {
						while (BattleManager.isEventRunning())
							yield;
		...
		Scene_Battle.prototype.onSelectAction = function () {
			...
			// カードを選択した場合
				...
				this._skillWindow._lastTargetType = this._skillWindow.getSelectedCardType();
				if (action.needsSelection() || this._skillWindow._lastTargetType !== Window_BattleCard.SELTYPE_NONE) {
		...
		Scene_Battle.prototype.createWindowBattleCardSelectBattler = function () {
			this._windowBattleCardSelectBattler = new Window_BattleCardSelectBattler();
			this._windowBattleCardSelectBattler.setHandler('cancel', function () {
				this._windowBattleCardSelectBattler.hide();
				this.ReactivateBattleCardByCancel();
			}.bind(this));
			this._windowBattleCardSelectBattler.setHandler('cursordown', function () {
				this._windowBattleCardSelectBattler.hide();
				this.ReactivateBattleCardByCancel();
			}.bind(this));
		...
		Scene_Battle.prototype.createWindowBattleCardSelectEnemy = function () {
			...
			this._windowBattleCardSelectEnemy.setHandler('ok', function () {
			...
			this._windowBattleCardSelectEnemy.setHandler('cancel', function () {
				this._windowBattleCardSelectEnemy.hide();
				this.ReactivateBattleCardByCancel();
			}.bind(this));
		...
		Scene_Battle.prototype.createWindowBattleCardSelectCard = function () {
			...
			this._windowBattleCardSelectCard.setHandler('ok', function () {
				this._itemWindow.hide();
				...
				switch (this._windowBattleCardSelectCard._baseCardType) {
					...
					case Window_BattleCard.SELTYPE_CARDX:
						...
						if (this._windowBattleCardSelectCard.index() == 0) {
							action._cardTargets = this._windowBattleCardSelectCard.getSelectedCardStack();
							this.selectNextCommand();
							this._windowBattleCardSelectCard.hide();
						}
						else {
			...
			this._windowBattleCardSelectCard.setHandler('cancel', function () {
				if (this._windowBattleCardSelectCard._baseCardType === Window_BattleCard.SELTYPE_CARDX) {
					if (0 < this._windowBattleCardSelectCard.getSelectedCardStack().length) {
		...
		Scene_Battle.prototype.ReactivateBattleCardByCancel = function () {
			...
			Coroutine.overrideTask("charaStandCoroutine", this._skillWindow._charaStand.coroutineShowChara());
		...
		Scene_Battle.prototype.createPartyCommandWindow = function () {
			...
			this._partyCommandWindow.x = Graphics.boxWidth / 2 - this._partyCommandWindow.windowWidth() / 2;
			this._partyCommandWindow.setHandler('item', this.commandItem.bind(this));
			this._partyCommandWindow.setHandler('escape', this.commandEscape.bind(this));
			this._partyCommandWindow.setHandler('cancel', function () {
				this._partyCommandWindow.close();
				this.ReactivateBattleCardByCancel();
			}.bind(this));
		...
		//==============================
		const Window_BattleItem_prototype_includes = Window_BattleItem.prototype.includes;
		Window_BattleItem.prototype.includes = function (item) {
			return Window_BattleItem_prototype_includes.call(this, item) && !item.meta.itemCategory;
		};
		//==============================
		...
		BattleManager.startTurn = function () {
			...
			BattleManager._spriteset._enemySprites.forEach(e => e._futureSightSprite.resetShield());
		}
		//==============================
		...
		BattleManager.startInput = function () {
			...
			actor.states().forEach(e => {
				if (e.meta.id === "shield")
					shield += Math.round(Nnfs.evalDamageFormulaEx(actor, null, e.meta.formula1));
				else if (e.meta.id === "addcard")
					draw_plus += Math.round(Nnfs.evalDamageFormulaEx(actor, null, e.meta.formula1));
			})
			// シールド値初期化
			if (shield === 0)
				BattleManager._spriteset._actorSprites[0]._futureSightSprite.resetShield();
			else
				actor._shieldValue = shield;
			if (0 < draw_plus)
				BattleManager._drawPlusAtNow += draw_plus;
			// ターン限定ブースト値初期化
			if (actor._boostValueFor1Turn !== 0) {
				BattleManager._spriteset._actorSprites[0].showDamagePopup(-actor._boostValueFor1Turn, 64);
		...
		BattleManager.update = function () {
			...
			if (Nnfs._autoVisibleCardWindow) {
				...
				if ((event_running || $gameSkit._isSkitOn) && this._skillWindow._parentCard.visible) {
					this._skillWindow.hide();
					this._skillWindow.hideCommand();
					this._skillWindow.active = false;
					this._cardWindowHide = true;
				}
				else if (this._cardWindowHide && !this._skillWindow.visible && !$gameSkit._isSkitOn && !event_running) {
					this._skillWindow.show();
		...
		BattleManager.isEventRunning = function () { return this._interpreter.isRunning() || this._isEvent; };
		...
		Game_Action.prototype.executeHpDamage = function (target, value) {
			...
			if (dmg_type === 1) {
				...
				if (is_shield) {
					...
					if (0 < target._shieldValue)
						playSe(params._seShield);
					else
						playSe(params._seShieldBreak);
				}
				...
				// ダメージ条件表情切り替え　ダメージ時
				if (target.isActor()) {
					if (!BattleManager._isLifeLow && target._hp <= target.mhp * (params._LifeLowPct * 0.01)) {
					...
					// }
					playVoice(Array.random(target._isTransform ? params._seVoiceBattleActorDamageTransform : params._seVoiceBattleActorDamage));
				}
				else {
					if (s._atkUp2) {
				...
				// ダメージ条件表情切り替え　回復時
				if (target.isActor()) {
					if (BattleManager._isLifeLow && target.mhp * (params._LifeLowPct * 0.01) < target._hp) {
						BattleManager._isLifeLow = false;
					}
				}
			}
		...
		Game_Battler.prototype.useItem = function (item) {
			...
			if (item.meta.cost) {
				...
				if (this.isActor() && !this.currentAction()._skillInsert) {
					...
					BattleManager._skillWindow.drawDigitMp(this.mp, this.mmp);
					playVoice(Array.random(this._isTransform ? params._seVoiceBattleUseCardTransform : params._seVoiceBattleUseCard));
				}
			}
		};
		//==============================
		...
		Game_Action.prototype.apply = function (target) {
			...
			if (result.isHit()) {
				...
				switch (item.meta.id) {
					// mp増加
					case 'mpup':
						s.setMp(s.mp + Math.round(Nnfs.evalDamageFormulaEx(s, item, item.meta.formula1)));
					...
					case 'shield':
						...
						t._shieldValue += Math.round(Nnfs.evalDamageFormulaEx(t, item, item.meta.formula1));
						t._shieldValue = Math.max(0, t._shieldValue);
						break;
					// ガード+ドロー
					case 'shielddraw':
						s._shieldValue += Math.round(Nnfs.evalDamageFormulaEx(s, item, item.meta.formula1));
						s._shieldValue = Math.max(0, s._shieldValue);
						BattleManager._drawPlusAtNow += Math.max(0, Math.round(Nnfs.evalDamageFormulaEx(s, item, item.meta.formula2)));
						break;
					// 手札の攻撃カード2倍
					case 'atkup2':
						s._atkUp2 = true;
					...
					case 'boost': {
						...
						const add_boost = Math.round(Nnfs.evalDamageFormulaEx(t, item, item.meta.formula1));
						t._boostValue += add_boost;
						Nnfs.getSpriteFromBattler(t).showDamagePopup(add_boost, Nnfs._iconIndexBoost);
						break;
					}
					// 1ターンだけブースト値+
					case 'boost1turn': {
					...
					case 'mpshield':
						s._shieldValue += Math.round(Nnfs.evalDamageFormulaEx(s, item, item.meta.formula1));
						Nnfs.getSpriteFromBattler(s).showDamagePopup(this._card._value1, 81);
						break;
					// x枚ドロー
					case 'draw':
						BattleManager._drawPlusAtNow += Math.max(0, Math.round(Nnfs.evalDamageFormulaEx(s, item, item.meta.formula1)));
						break;
					// 次のターンでMP回復＆x枚ドロー
					case 'readynext':
						BattleManager._mpAdd = Math.round(Nnfs.evalDamageFormulaEx(s, item, item.meta.formula1));
						BattleManager._drawPlusAtFirst += Math.max(0, Math.round(Nnfs.evalDamageFormulaEx(s, item, item.meta.formula2)));
						break;
					// 指定したカードのコスト0
					case 'cost0':
					...
					// 捨て札から好きなカードを手札に加える
					case 'getfromdiscard': {
						const rem_idx = skill_window._deck_info.discards.findIndex(e => e == this._cardTargets[0]._id, this);
						if (0 <= rem_idx) {
						...
						skill_window.drawDigitDiscard(skill_window._deck_info.discards.length);
						break;
					}
					// デッキから好きなカードを手札に加える
					case 'getfromdeck': {
						const rem_idx = skill_window._deck_info.deck.findIndex(e => e == this._cardTargets[0]._id, this);
						if (0 <= rem_idx) {
					...
					case 'resetgauge':
						...
						break;
					case 'addgauge': {
						const v1 = Math.round(Nnfs.evalDamageFormulaEx(s, item, item.meta.formula1));
						target.initTrunk()
					...
					case 'hpointup':
						if (!target._isTransform) {
							const v1 = Math.round(Nnfs.evalDamageFormulaEx(target, item, item.meta.formula1));
							Nnfs.addHpoint(v1);
		...
		Sprite_Battler.prototype.showDamagePopup = function (value, iconIndex) {
			...
			if (iconIndex) {
				...
				var dw = 36 - 4;
				var dh = 36 - 4;
				var n = Math.floor((sprite.bitmap.fontSize / 28) * Window_Base._iconWidth);
		...
		Sprite_Battler.prototype.showPopupStateAttached = function () {
			...
			this._battler._statePopupInfo.forEach(e => {
				...
				if (1 <= cnt) {
					// console.log(e);
					Coroutine.pushTask(Coroutine.gWaitForFrames(10 * cnt, function () {
						this._damages.push(damage);
						this.parent.addChild(damage);
					}.bind(this)));
				}
		...
		Sprite_Battler.prototype.setupDamagePopup = function () {
			if (this._battler.isDamagePopupRequested()) {
				if (this._battler.isSpriteVisible()) {
					...
					const is_hp_damage = result.hpAffected && 0 < result.hpDamage && !result.drain;
					let interrupt = false;
					if (0 < $gameTemp._shieldValue) {
						let sprite = new Sprite_Damage();
						this._futureSightSprite.startShakeShield();
						const r = 30;
						sprite._initX = this._futureSightSprite.x - 90 + Math.randomRange(-r, r);
						...
						for (let i = 0; i < string.length; ++i) {
							const n = +string[i];
							sprite.bitmap.bltImage(sprite._damageBitmap, w * n, h * 2, w, h, i * w, 0);
						}
		...
		// *カードバトルシステム：敵選択ウィンドウ
		//==============================
		function Window_BattleCardSelectEnemy() { this.initialize.apply(this, arguments); }
		Window_BattleCardSelectEnemy.prototype = Object.create(Window_BattleEnemy.prototype);
		Window_BattleCardSelectEnemy.prototype.constructor = Window_BattleCardSelectEnemy;
		...
		Window_BattleCardSelectEnemy.prototype.initialize = function () {
			...
			this._cursor.bitmap.addLoadListener(function () {
		...
		Window_BattleCardSelectEnemy.prototype.scrollDown = function () { Window_BattleCard.prototype.scrollDown.call(this); };
		Window_BattleCardSelectEnemy.prototype.scrollUp = function () { Window_BattleCard.prototype.scrollUp.call(this); };
		...
		Window_BattleCardSelectEnemy.prototype.isCursorMovable = function () { return this.isOpenAndActive(); };
		Window_BattleCardSelectEnemy.prototype.isContentsArea = () => true;
		Window_BattleCardSelectEnemy.prototype.topIndex = () => 0;
		Window_BattleCardSelectEnemy.prototype.maxPageItems = function () { return this.maxItems(); };
		...
		Window_BattleCardSelectEnemy.prototype.isCursorVisible = function () { return false; };
		...
		//==============================
		function Window_BattleCardSelectBattler() { this.initialize.apply(this, arguments); }
		Window_BattleCardSelectBattler.prototype = Object.create(Window_Selectable.prototype);
		Window_BattleCardSelectBattler.prototype.constructor = Window_BattleCardSelectBattler;
		...
		Window_BattleCardSelectBattler.prototype.cursorDown = function () { this.callHandler("cursordown") };
		Window_BattleCardSelectBattler.prototype.cursorUp = function () { };
		Window_BattleCardSelectBattler.prototype.scrollDown = function () { Window_BattleCard.prototype.scrollDown.call(this); };
		Window_BattleCardSelectBattler.prototype.scrollUp = function () { Window_BattleCard.prototype.scrollUp.call(this); };
		...
		Window_BattleCardSelectBattler.prototype.isCursorMovable = function () { return this.isOpenAndActive(); };
		Window_BattleCardSelectBattler.prototype.isContentsArea = () => true;
		Window_BattleCardSelectBattler.prototype.topIndex = () => 0;
		Window_BattleCardSelectBattler.prototype.maxItems = function () { return this._battlers.length; }
		Window_BattleCardSelectBattler.prototype.maxPageItems = function () { return this.maxItems(); }
		...
		Window_BattleCardSelectBattler.prototype.isCursorVisible = function () { return false; };
		...
		//==============================
		function Window_BattleCardSelectCard() { this.initialize.apply(this, arguments); }
		Window_BattleCardSelectCard.prototype = Object.create(Window_Selectable.prototype);
		Window_BattleCardSelectCard.prototype.constructor = Window_BattleCardSelectCard;
		...
		Window_BattleCardSelectCard.prototype.initialize = function () {
			...
			// 選択終了
			this._spriteSelectEnd = new Sprite(ImageManager.loadPicture('card_system/ui_selectend'));
		...
		Window_BattleCardSelectCard.prototype.show = function () {
			...
			switch (this._baseCard.item().meta.id) {
				...
				case 'getfromdiscard':
					...
					break;
				default:
					this._cards = card_window._hands.filter(e => e != this._baseCard && !e.isTransformCard());
					// 変身カードを消す
					const sprite_transform = card_window._hands.find(e => e.isTransformCard());
					if (sprite_transform)
						sprite_transform._y = Graphics.boxHeight + 140;
					break;
			}
			if (deck_or_discard) {
				this._refFromDeckOrDiscards = true;
				card_window._hands.filter(e => e != this._baseCard).forEach(e => e.hide());
				this._baseCard.show();
			}
			this.select(this._baseCardType === Window_BattleCard.SELTYPE_CARD1 ? 0 : 1);
			this._systemCursorVisible = this._baseCardType === Window_BattleCard.SELTYPE_CARDX;
			this._spriteSelectEnd.visible = this._baseCardType === Window_BattleCard.SELTYPE_CARDX;
			if (this._cards.length % 2) {
				this._ofsEven = 0;
				const l = this._cards.length;
				this._cardMedIdx = Array.from(new Array(l)).map((v, i) => i - ~~(l / 2));
			}
			else {
				this._ofsEven = this._baseCard.width / 2;
				const l = this._cards.length + 1;
				this._cardMedIdx = Array.from(new Array(l)).map((v, i) => i - ~~(l / 2)).filter(i => i != 0);
			}
		};
		...
		Window_BattleCardSelectCard.prototype.hide = function (whenCancel) {
			...
			if (this._refFromDeckOrDiscards) {
				...
				if (!whenCancel) {
					const selected = 0 < this._selectedStack.length ? this._selectedStack : [this.getSelectedCard()];
					selected.forEach(e => {
						e._blurMultiplier = 0;
					});
					this._cards.forEach(e => {
		...
		Window_BattleCardSelectCard.prototype.update = function (index) {
			...
			if (this._baseCardType === Window_BattleCard.SELTYPE_CARD1)
				...
				this.updateTypeCardX();
			this._cursor.scale.x = Math.sin(this._cnt += 0.1);
			// 選択終了アニメ
			if (this.index() == 0)
				this._spriteSelectEnd.scale.set(Math.lerp(this._spriteSelectEnd.scale.x, Nnfs._selectEndScaleSize, 0.2));
			else
				this._spriteSelectEnd.scale.set(Math.lerp(this._spriteSelectEnd.scale.x, 1.0, 0.2));
		};
		Window_BattleCardSelectCard.prototype.updateTypeCard1 = function (isMultiple = 0) {
			if (this._baseCard) {
				this._baseCard._x = Graphics.boxWidth / 2 - this._baseCard.width / 2;
				this._baseCard._y = params.CardInfo._pos[1] + params.CardInfo._posOfs3 - 250;
				this._baseCard._r = 0;
			}
		...
		Window_BattleCardSelectCard.prototype.scrollDown = function () { Window_BattleCard.prototype.scrollDown.call(this); };
		Window_BattleCardSelectCard.prototype.scrollUp = function () { Window_BattleCard.prototype.scrollUp.call(this); };
		...
		Window_BattleCardSelectCard.prototype.isCursorMovable = function () { return this.isOpenAndActive(); };
		Window_BattleCardSelectCard.prototype.isContentsArea = () => true;
		Window_BattleCardSelectCard.prototype.topIndex = () => 0;
		Window_BattleCardSelectCard.prototype.maxPageItems = function () { return this.maxItems(); };
		...
		Window_BattleCardSelectCard.prototype.isCursorVisible = function () { return this._systemCursorVisible && this.index() == 0; };
		...
		Window_BattleCardSelectCard.prototype.isCurrentItemEnabled = function () {
			return !(this._systemCursorVisible && this.index() == 0 && this._selectedStack.length == 0);
		};
		...
		Window_BattleCardSelectCard.prototype.getSelectedCardStack = function () { return this._selectedStack; };
		...
		function Sprite_Description() { this.initialize.apply(this, arguments); }
		...
		Sprite_Description.prototype.initialize = function () {
			...
			this.desc.makeFontBitmap(params.DescWindowParam._cardDescWindowWidth, 200);
		...
		Sprite_Description.prototype.refreshState = function (state, subject, x, y) {
			this.x = x;
			this.y = y;
			this.desc.contents.clear();
			let h = 0;
			if (0 < state.iconIndex)
				this.desc.drawTextEx("\\c[1]" + state.name + "\\I[" + state.iconIndex + "]", 2, h);
			else
				this.desc.drawTextEx("\\c[1]" + state.name, 2, h);
			h += 22;
			let msg = state.meta.desc;
			if (msg.includes('%1'))
				msg = msg.replace('%1', Math.round(Nnfs.evalDamageFormulaEx(subject, null, state.meta.formula1)));
			if (msg.includes('%2'))
				msg = msg.replace('%2', Math.round(Nnfs.evalDamageFormulaEx(subject, null, state.meta.formula2)));
		...
		Sprite_Description.prototype.refreshContent = function (contents, x, y) {
			...
			this.y = y;
			this.desc.contents.clear();
			this.desc.drawTextEx("\\c[1]" + contents, +params.DescWindowParam._fontInfo[1], +params.DescWindowParam._fontInfo[2]);
		...
		Sprite_Description.prototype.drawFrame = function (w, h) {
			...
			this.bitmap.blt(skin, p + m, p + 0, q - m * 2, m, ox + m, oy + 0, w - m * 2, m);
			this.bitmap.blt(skin, p + m, p + q - m, q - m * 2, m, ox + m, oy + h - m, w - m * 2, m);
			this.bitmap.blt(skin, p + 0, p + m, m, q - m * 2, ox + 0, oy + m, m, h - m * 2);
			this.bitmap.blt(skin, p + q - m, p + m, m, q - m * 2, ox + w - m, oy + m, m, h - m * 2);
			...
			this.bitmap.blt(skin, p + q - m, p + q - m, m, m, ox + w - m, oy + h - m, m, m);
		};
		//==============================
		// *カードバトルシステム：カードセレクトウィンドウ
		//==============================
		function Window_BattleCard() { this.initialize.apply(this, arguments); }
		...
		Window_BattleCard.prototype.isWaitState = function () { return this._state === Window_BattleCard.STATE_WAIT; };
		Window_BattleCard.prototype.isCardState = function () { return this._state === Window_BattleCard.STATE_SELECT_CARD; };
		Window_BattleCard.prototype.isTargetState = function () { return this._state === Window_BattleCard.STATE_SELECT_TARGET; };
		Window_BattleCard.prototype.isOptionState = function () { return this._state === Window_BattleCard.STATE_SELECT_OPTION; };
		...
		Window_BattleCard.prototype.isLastCardTypeSelectCard = function () {
			...
			return type == Window_BattleCard.SELTYPE_CARD1 || type == Window_BattleCard.SELTYPE_CARDX;
		};
		Window_BattleCard.prototype.initialize = function () {
			...
			this._spriteBg.bitmap.addLoadListener(function (tex) {
			...
			ImageManager.loadPicture('card_system/ui_turn').addLoadListener(tex => {
				...
				this._spriteTurn.bitmap.bltImage(tex, 0, 0, tex.width, tex.height, 0, 0);
				Nnfs.setFont(this._spriteTurn.bitmap, params._turnFont);
				this._spriteTurn.bitmap.drawText(Nnfs.localization.id_battle_turn, 20, 20, this.textWidth(Nnfs.localization.id_battle_turn), this.lineHeight());
			});
			this.addChild(this._spriteTurn);
			// 現在ターン数
			this._spriteTurnCount = new Sprite_Font();
			this._spriteTurnCount.makeFontBitmap(params._turnCountPos[2], params._turnCountPos[3]);
			...
			ImageManager.loadPicture('card_system/ui_turnend').addLoadListener(tex => {
				this._spriteTurnEnd.bitmap = new Bitmap(tex.width, tex.height);
				this._spriteTurnEnd.bitmap.bltImage(tex, 0, 0, tex.width, tex.height, 0, 0);
				Nnfs.setFont(this._spriteTurnEnd.bitmap, params._turnEndFont);
				this._spriteTurnEnd.bitmap.drawText(Nnfs.localization.id_battle_turnend, 0, -4, tex.width, this.lineHeight(), "center");
			});
			this.addChildAt(this._spriteTurnEnd, 0);
			// デッキアイコン
			{
				const offset = 5;
				this._spriteDeck = [new Sprite(ImageManager.loadPicture('card_system/ui_discard')), new Sprite(ImageManager.loadPicture('card_system/ui_deck'))];
				this._spriteDeck[0].bitmap.addLoadListener(function (tex) {
					this._spriteDeck[0].x = tex.width / 2 + offset;
					this._spriteDeck[0].y = Graphics.boxHeight - tex.height / 2 - offset;
					tex.smooth = false;
					this.drawDigitDiscard(0);
				}.bind(this));
				this._spriteDeck[0].anchor.set(0.5);
				this._spriteDeck[1].bitmap.addLoadListener(function (tex) {
					this._spriteDeck[1].x = Graphics.boxWidth - tex.width / 2 - offset;
					this._spriteDeck[1].y = Graphics.boxHeight - tex.height / 2 - offset;
					tex.smooth = false;
				}.bind(this));
			...
			// mp背景
			this._spriteMpBack = new Sprite(ImageManager.loadPicture('card_system/ui_mpback'));
			this._spriteMpBack.x = Graphics.boxWidth - 80;
			this._spriteMpBack.y = Graphics.boxHeight - 162;
			this.addChildAt(this._spriteMpBack, 1);
			// 変身ゲージ
			this._spriteHeart = new Sprite(ImageManager.loadPicture('card_system/ui_heart'));
			...
			if ($gameSwitches.value(params._swIdStopDealTransform)) {
				this._cantTransforming = new Sprite(ImageManager.loadPicture('card_system/ui_cant_transform'));
			...
			if ($gameSystem._hPoint == null)// セーブデータになかった場合
				Nnfs.setHpoint(0);
			// 変身ゲージの数値
			this._spriteHpoint = new Sprite_Font();
			this._spriteHpoint.makeFontBitmap(params._tensionGaugeValuePos[2], params._tensionGaugeValuePos[3]);
		...
		Window_BattleCard.prototype.resetDeck = function () {
			...
			if (this._deck_info == null)
				this._deck_info = LoadDeck();
			...
			this._deck_info.deck = Array.from(default_deck_obj.deck)
				.filter(e => !this._excludeCardIds.includes(e) && !this._transRemoveCards.includes(e) && !once_card_ids.includes(e), this);
			// 追加カード
		...
		Window_BattleCard.prototype.startTurn = function (actor) {
			...
			if (transform_card)
				transform_card._index = 1;
			this.show();
			// カメラ
			$gameTemp.setCameraProp(params.CameraParam._propPlayerTurn);
			// ランプ消す
			if (this._actor._isTransform) {
				if (params._isUntransformTest)
					Nnfs.setHpoint(0);
				else
					Nnfs.addHpoint(-params._transformTurnCountDec);
				this.refreshHPoint();
				// 変身解除
				if ($gameSystem._hPoint === 0) {
					BattleManager._interpreter.setup($dataCommonEvents[params._commonEventIdUntransform].list, 0);
					// 山札と捨札から追加したカードを取り除く
					this._deck_info.deck = this._deck_info.deck.filter(e => !this._transAddCards.includes(e), this);
					this._deck_info.discards = this._deck_info.discards.filter(e => !this._transAddCards.includes(e), this);
					// 山札に除外したカードを追加する?
					// リセット
					this._transAddCards = [];
					this._transRemoveCards = [];
			...
			this.drawFromDeck(params.CardInfo._handCount + (BattleManager._drawPlusAtFirst || 0));
			...
			if ($gameTroop._turnCount == 0)
				...
				playVoice(Array.random(this._actor._isTransform ? params._seVoiceBattleActorTurnTransform : params._seVoiceBattleActorTurn));
			// ターン数を更新
		...
		Window_BattleCard.prototype.update2 = function () {
			...
			if (0 < this._tempDiscard[1]) {
				--this._tempDiscard[1];
				const scale = Math.lerp(1, this._tempDiscard[2], this._tempDiscard[1] / 10);
			...
			// 選択終了アニメ
			if (this.index() == 0)
				this._spriteTurnEnd.scale.set(Math.lerp(this._spriteTurnEnd.scale.x, Nnfs._selectEndScaleSize, 0.2));
			else
				this._spriteTurnEnd.scale.set(Math.lerp(this._spriteTurnEnd.scale.x, 1.0, 0.2));
			// 角度
			let len = this._hands.length;
			if (len === 0)
				return;
			let rot_ofs = 1 < len ? params.CardInfo._rotOfsMax / this._handMedIdx[len - 1] : 0;
			let pos_ofs = params.CardInfo._posOfs;
			let px = Graphics.boxWidth / 2 + params.CardInfo._pos[0], py = params.CardInfo._pos[1];
			...
			for (let i = 0; i < len; ++i) {
				...
				if (is_selected1)
					...
					card._r = Math.deg2rad(medidx * rot_ofs);
				// 位置
				...
				if (is_selected1)
					card._y = py + pos_ofs[1] + params.CardInfo._posOfs3;
				else
					card._y = py + Math.abs(medidx) * pos_ofs[1];
			}
		}

		Window_BattleCard.prototype.coroutineDealCards = function* (hideAllPreProp) {
		...
		Window_BattleCard.prototype.coroutineTurnEnd = function* () {
			...
			for (let i = 0; i < this._hands.length; ++i) {
				if (this._hands[i].isTransformCard()) {
					...
					this._hands[i].discard(true, false);
		...
		Window_BattleCard.prototype.coroutineRestart = function* () {
			// 前のアニメーションが終わるまで待機する
			while (BattleManager.isEventRunning() || this._cacheCards.some(e => e.isBusy()))
				yield Coroutine.gWaitForSeconds(0.5);
			// 敵の予測文字の再計算
			...
			yield this.coroutineDealTransform();
			// 変身カード使用後
			if (this._lastCardItem && this._lastCardItem.meta.id === cardIdTransform) {
				// this._state = Window_BattleCard.STATE_WAIT;
				const items = default_deck_obj.deck;
				// Hカードを全開放
				this._transAddCards = JSON.parse(this._lastCardItem.meta.adddeck).filter(e => !items.includes(e));
		...
		Window_BattleCard.prototype.coroutineDealGainCard = function* () {
			if ($gameTemp.gainCard != null) {
				const new_card_item = $dataItems.find(e => e != null && e.id === $gameTemp.gainCard);
		...
		Window_BattleCard.prototype.coroutineDealTransform = function* () {
			...
			sprite.setup(this._hands.length/*index=0はturnend用*/, $dataItems.find(e => e != null && e.meta.id === cardIdTransform).id);
			...
			i = 0; frame = 60;
			Nnfs.ParticleContainer.makeParticle('Light', this.parent.parent, true, true, true, true, true)
				.setDuration(frame)
				.play();
			Nnfs.ParticleContainer.makeParticle('ShowTransformCard', sprite.parent, true, true, false, true, true)
				.setDuration(frame)
				.setSizeStart(sprite.width, [sprite.height * 0.1, sprite.height])
				.setSizeEnd(sprite.width, [sprite.height * 1, sprite.height * 3])
				.play();
			Nnfs.ParticleContainer.makeParticle('Kira', sprite.parent, true, true, true, true, true)
				.setDuration(frame)
			...
			sprite.anchor.y = 1;
			sprite.setPosition(Graphics.boxWidth / 2, Graphics.boxHeight / 2 + sprite.height / 2);
			while (!sprite.bitmap.isReady())
				yield;
			playSe(params._seCardDealNew);
			while (i < frame) {
				const t = i++ / frame;
				sprite.setFrame(0, sprite_frame.height - sprite_frame.height * t, sprite_frame.width, sprite_frame.height);
				yield;
			}
			i = 0; frame = 15;
			while (i < frame) {
			...
			const p = Nnfs.ParticleContainer.makeParticle('Outline', sprite.parent, true, true, false, true, true)
				.setTag('transform')
				.setColor([128, 0, 255, 150])
				.play();
		...
		Window_BattleCard.prototype.coroutineDealNewCard = function* (newCardItem) {
			...
			let sprite_frame = sprite._frame;
			sprite.parent.visible = true;
			let sy = -150, ey = Graphics.boxHeight / 2;
			sprite.setPosition(Graphics.boxWidth / 2, Graphics.boxHeight / 2 + sprite.height / 2);
			while (!sprite.bitmap.isReady())
				yield;
			playSe(params._seCardDeal);
			while (i < frame) {
				const t = i++ / frame;
				sprite.setPositionY(Math.lerp(sy, ey, Nnfs.EasingFunctions.easeOutQuart(t)));
				yield;
			}
			i = 0; frame = 15;
			while (i < frame) {
		...
		Window_BattleCard.prototype.drawFromDeck = function (count) {
			for (let i = 0; i < count; ++i) {
				...
				this._hands.push(sprite);
				this._parentCard.addChild(sprite.parent);
				if (0 == this._deck_info.deck.length)
					this.resetDeck();
				sprite.setup(this._hands.length/*index=0はturnend用*/, this._deck_info.deck.pop());
			}
			this.makeHandMedIdx();
		};
		...
		Window_BattleCard.prototype.makeHandMedIdx = function () {
			...
			if (len % 2 == 0)
				this._handMedIdx = Array.from(new Array(len + 1)).map((v, i) => i - med).filter(i => i != 0);
			else
				this._handMedIdx = Array.from(new Array(len)).map((v, i) => i - med);
			// 位置オフセットの計算をここでしてしまう
			...
			this._adjust_by_even = len % 2 == 0 ? (len - 1) * params.CardInfo._posOfs[0] / len : 0;
		};
		...
		Window_BattleCard.prototype.posDiscard = function () { return [this._spriteDeck[0].x, this._spriteDeck[0].y]; };
		Window_BattleCard.prototype.posDeck = function () { return [this._spriteDeck[1].x, this._spriteDeck[1].y]; };
		...
		Window_BattleCard.prototype.getSelectedCardType = function () { return this.getSelectedCard().getTargetType(); };
		...
		Window_BattleCard.prototype.isOpenAndActive = function () {
			return this._state === Window_BattleCard.STATE_SELECT_CARD && Window_Selectable.prototype.isOpenAndActive.call(this);
		};

		Window_BattleCard.prototype.isEnabled = function (card) {
			if (this._actor && /*this._actor.canUse(item) && */card != null && card._cost <= this._actor.mp) {
		...
		Window_BattleCard.prototype.isCurrentItemEnabled = function () { return 0 == this.index() || this.isEnabled(this.getSelectedCard()); };
		Window_BattleCard.prototype.maxItems = function () { return this._hands.length + 1; };
		Window_BattleCard.prototype.maxPageItems = function () { return this._hands.length + 1; };
		Window_BattleCard.prototype.maxPageRows = () => 1;
		Window_BattleCard.prototype.maxCols = function () { return this._hands.length + 1; };
		Window_BattleCard.prototype.topIndex = () => 0;
		Window_BattleCard.prototype.isCursorVisible = function () { return 0 == this.index() && this._state === Window_BattleCard.STATE_SELECT_CARD; };
		Window_BattleCard.prototype.isCursorMovable = function () { return this.isOpenAndActive(); };
		...
		Window_BattleCard.prototype.itemRect = function (index) {
			...
			// カード
				...
				return new Rectangle(x - card.anchor.x * card.width, y - card.anchor.y * card.height, card.width, card.height);
			}
		};
		...
		Window_BattleCard.prototype.drawDigitDiscard = function (n) {
			if (this._tempDiscard[0] !== n) {
				...
				const x = this._spriteDeck[0].x, y = this._spriteDeck[0].y;
				this.contents.clearRect(x - 20, y - 24, 50, 40);
				ImageManager.loadPicture('card_system/status_n2').addLoadListener(function (tex) {
		...
		Window_BattleCard.prototype.drawDigitDeck = function (n) {
			if (this._tempDeck[0] !== n) {
				...
				this._tempDeck[1] = 10;
				ImageManager.loadPicture('card_system/status_n2').addLoadListener(function (tex) {
					const x = this._spriteDeck[1].x, y = this._spriteDeck[1].y;
		...
		Window_BattleCard.prototype.drawDigitMp = function (mp, mmp) {
			...
			const x = this._spriteMpBack.x, y = this._spriteMpBack.y;
		...
		Window_BattleCard.prototype.refreshTurnCount = function () {
			...
			this._spriteTurnCount.drawTextEx(($gameTroop._turnCount + 1).toString(), 0, 0);
		};
		...
		Window_BattleCard.prototype.processTouch = function () {
			...
			if (hitIndex < 0) {
				...
				let is_hit = false;
				if (Nnfs.isContainsPointFromRect(x, y, left, up, sprite.width, sprite.height, sprite.width * 0.2, sprite.height * 0.2)) {
					is_hit = true;
					// 初回ヒット
				...
				// enemies hitting
				for (let i = 0, l = BattleManager._spriteset._enemySprites.length; i < l; ++i) {
		...
		Window_BattleCard.prototype.cursorUp = function (wrap) { this.callHandler('cursorup'); };
		...
		Window_BattleCard.prototype.showCardDescWindow = function (spriteCard) {
			...
			if (!state_ids) {
				Nnfs._itemMetaDescCaches[item.id] = state_ids = JSON.parse(item.meta.desc);
				// console.log("cache created:", item.id);
			}
			const px = spriteCard._x < Graphics.boxWidth / 2 ? spriteCard._x + params.DescWindowParam._cardDescWindowOffset[0]
				: spriteCard._x - params.DescWindowParam._cardDescWindowOffset[0] - params.DescWindowParam._cardDescWindowWidth;
		...
		Window_BattleCard.prototype.showBattlerDescWindow = function (battlerSprite) {
			this.closeDescWindow();
			...
			if (battler.isActor()) {
				...
				px = data.meta.stateDescPosX ? (battlerSprite.x + (+data.meta.stateDescPosX)) : (battlerSprite.x - battlerSprite.width * 0.5 - params.DescWindowParam._cardDescWindowWidth);
				py = data.meta.stateDescPosY ? (battlerSprite.y + (+data.meta.stateDescPosY)) : (battlerSprite.y - battlerSprite.height);
			}
			else if (battler.isEnemy) {
				var data = battler.enemy();
				px = data.meta.stateDescPosX ? (battlerSprite.x + (+data.meta.stateDescPosX)) : (battlerSprite.x);
				py = data.meta.stateDescPosY ? (battlerSprite.y + (+data.meta.stateDescPosY)) : (battlerSprite.y - battlerSprite.height);
			}
			let has_state = false;
			...
			if (!has_state) {
				this.makeDescWindowSpriteIfNeeded().refreshContent(Nnfs.localization.id_empty_state, px, py);
			}
		}
		...
		// *カードバトルシステム：キャラクタ立ち絵
		//==============================
		function CharaStand() { this.initialize.apply(this, arguments); }
		...
		CharaStand.prototype.initialize = function () {
			...
			this._spine = new PIXI.spine.Spine(Makonet.MpiShowSpine.spineData[params.CharaImageParam._spineName]);
		...
		CharaStand.prototype.refreshSpine = function (actor) {
			...
			const skins = actor._isTransform ? params.CharaImageParam._skinsTransform : params.CharaImageParam._skinsNormal;
			if (0.6 <= hp_ratio)
				Makonet.MpiShowSpine.setSkinEx(this._spine, skins[0]);
			else if (0.3 <= hp_ratio)
				Makonet.MpiShowSpine.setSkinEx(this._spine, skins[1]);
			else
				Makonet.MpiShowSpine.setSkinEx(this._spine, skins[2]);
			this._spine.skeleton.setSlotsToSetupPose();
			// attachment
			const trunk_ratio = actor.trunk() / 100;
			let anim_and_atcm_set = null;
			if (1 <= trunk_ratio)
				anim_and_atcm_set = actor._isTransform ? params.CharaImageParam._animAndAtcmSetTransform100 : params.CharaImageParam._animAndAtcmSetNormal100;
			else if (0.7 <= trunk_ratio)
				anim_and_atcm_set = actor._isTransform ? params.CharaImageParam._animAndAtcmSetTransform70 : params.CharaImageParam._animAndAtcmSetNormal70;
			else if (0.5 <= trunk_ratio)
				anim_and_atcm_set = actor._isTransform ? params.CharaImageParam._animAndAtcmSetTransform50 : params.CharaImageParam._animAndAtcmSetNormal50;
			else if (0.3 <= trunk_ratio)
				anim_and_atcm_set = actor._isTransform ? params.CharaImageParam._animAndAtcmSetTransform30 : params.CharaImageParam._animAndAtcmSetNormal30;
			else
				anim_and_atcm_set = actor._isTransform ? params.CharaImageParam._animAndAtcmSetTransform0 : params.CharaImageParam._animAndAtcmSetNormal0;
			let r = Math.randomRangeInt(anim_and_atcm_set.length / 2);
			let anim = anim_and_atcm_set[r * 2];
			let atcm = anim_and_atcm_set[r * 2 + 1];
			// animation
			if (this._overrideAnimName)
				anim = this._overrideAnimName;
		...
		CharaStand.prototype.coroutineShowChara = function* () {
			...
			while (i < 13) {
				const t = i / frame;
				this._spine.x = Math.lerp(this._spine.x, params.CharaImageParam._posChara[2], 0.2);
				this._spine.y = Math.lerp(this._spine.y, params.CharaImageParam._posChara[3], 0.2);
				this._spine.alpha = Math.min(this._spine.alpha + .1, 1);
				++i;
				yield;
			}
			// i = 0;
		...
		CharaStand.prototype.coroutineHideChara = function* () {
			...
			while (i < 13) {
				const t = i / frame;
				this._spine.x = Math.lerp(this._spine.x, params.CharaImageParam._posChara[0], 0.2);
				this._spine.y = Math.lerp(this._spine.y, params.CharaImageParam._posChara[1], 0.2);
				this._spine.alpha = Math.max(this._spine.alpha - .1, 0);
				// this._msgBox.opacity -= 20;
				++i;
				yield;
			}
		...
		//   }
		// `;
		function Sprite_CardItem() { this.initialize.apply(this, arguments); }
		Sprite_CardItem.prototype = Object.create(Sprite.prototype);
		Sprite_CardItem.prototype.constructor = Sprite_CardItem;
		Sprite_CardItem.prototype.setPosition = function (x, y = x) { this.parent.x = x; this.parent.y = y; };
		Sprite_CardItem.prototype.setPositionX = function (a) { this.parent.x = a; };
		Sprite_CardItem.prototype.setPositionY = function (a) { this.parent.y = a; };
		Sprite_CardItem.prototype.getPosition = function () { return new PIXI.Point(this.parent.x, this.parent.y); };
		Sprite_CardItem.prototype.getPositionX = function () { return this.parent.x; };
		Sprite_CardItem.prototype.getPositionY = function () { return this.parent.y; };
		Sprite_CardItem.prototype.setScale = function (x, y = x) { this.parent.scale.set(x, y); };
		Sprite_CardItem.prototype.getScale = function () { return this.parent.scale.set(x, y); };
		Sprite_CardItem.prototype.setRotation = function (r) { this.parent.rotation = r; };
		Sprite_CardItem.prototype.getRotation = function (r) { return this.parent.rotation; };
		Sprite_CardItem.prototype.getRectangle = function () {
			return new Rectangle(this.parent.x - this.anchor.x * this.width, this.parent.y - this.anchor.y * this.height, this.width, this.height);
		};
		...
		Sprite_CardItem.prototype.setup = function (index, id) {
			...
			this._value1 = item.meta.formula1 ? Math.round(Nnfs.evalDamageFormulaEx(aa, item, item.meta.formula1)) : 0;
			this._value2 = item.meta.formula2 != null ? Math.round(Nnfs.evalDamageFormulaEx(aa, item, item.meta.formula2)) : 0;
			this._use = true;
			...
			if (this._id !== id || this._resetBitmap) {
				...
				tex.addLoadListener(function () {
					...
					this.bitmap.drawText(item.name, +fontinfo[1], +fontinfo[2], +fontinfo[3], +fontinfo[4], fontinfo[5]);
					// 文字系
		...
		Sprite_CardItem.prototype.update = function () {
			...
			this._blur.blurX = Math.abs(this.parent.x - this._prev.x) * this._blurMultiplier;
			this._blur.blurY = Math.abs(this.parent.y - this._prev.y) * this._blurMultiplier;
			this._prev.x = this.parent.x;
			this._prev.y = this.parent.y;
		};

		Sprite_CardItem.prototype.discard = function (isTurnEnd, isReturnDeck, isOnce, isStartPositionStay) {
			this._isBusy = true;
			Coroutine.pushTask(this.coroutineDiscard(isTurnEnd, isReturnDeck, isOnce, isStartPositionStay));
			if (isTurnEnd || isReturnDeck || (!BattleManager._duplicate || this.isTransformCard())) {
		...
		Sprite_CardItem.prototype.coroutineDiscard = function* (isTurnEnd, isReturnDeck, isOnce, isStartPositionStay) {
			this._activate = false;
			this.setRotation(0);
			let pos = this.getPosition();
			let sx = pos.x, sy = pos.y;
			let ex = Graphics.boxWidth * 0.82, ey = Graphics.boxHeight * 0.8;
			const skill_window = BattleManager._skillWindow;
			...
			// 消える
				...
				const c = Nnfs.ParticleContainer.makeParticle('Blast', skill_window._parentCard, true, true, true, true, true)
				...
				const epos = isReturnDeck ? skill_window.posDeck() : skill_window.posDiscard();
				...
				while (i < frame) {
					...
					this.setPosition(
						Math.lerp(spos.x, xx, t),
						Math.lerp(spos.y, yy, t));
				...
				if (isReturnDeck) {
					...
					skill_window.drawDigitDiscard(BattleManager._skillWindow._deck_info.discards.length);
				}
		...
		Sprite_CardItem.prototype.item = function () { return $dataItems[this._id]; };
		Sprite_CardItem.prototype.isBusy = function () { return this._isBusy; };
		...
		Sprite_CardItem.prototype.getTargetType = function () {
			...
			switch (item.meta.id) {
				...
				default:
					if ([1].includes(item.scope))
						return Window_BattleCard.SELTYPE_ENEMY;
					else
						return Window_BattleCard.SELTYPE_NONE;
			}
		};
		...
		Sprite_CardItem.prototype.refresh = function (target) {
			...
			let desc = Nnfs.replaceFormulaTag(item.description, action, target, this._value1, this._value2);
			this._word.drawTextEx(desc, +fontinfo[1], +fontinfo[2], +fontinfo[3], +fontinfo[4], fontinfo[5]);
			// 特殊効果
			if (item.damage.type === 1) {
				const is_play_atkup = this._particleContainers.find(v => v._tag === 'atkup');
				if (actor._atkUp2 && !is_play_atkup) {
					const p = Nnfs.ParticleContainer.makeParticle('Outline', this.parent, true, true, false, true, true)
						.setTag('atkup')
						.setColor([255, 0, 0, 128])
						.play();
			...
			else if (this._duplicate && !this._particleContainers.some(v => v._tag === 'duplicate')) {
				const p = Nnfs.ParticleContainer.makeParticle('Outline', this.parent, true, true, false, true, true)
					.setTag('duplicate')
		...
		Sprite_CardItem.prototype.refreshCostAndNg = function () {
			...
			bitmap.outlineWidth = +fontinfo[8];
			bitmap.outlineColor = fontinfo[9];
			bitmap.drawText(this.getRealCost(), +fontinfo[1], +fontinfo[2], +fontinfo[3], +fontinfo[4], fontinfo[5]);
			// ng
		...
		Nnfs.ParticleContainer = function ParticleContainer() { this.initialize.apply(this, arguments); };
		Nnfs.ParticleContainer.prototype = Object.create(PIXI.particles.ParticleContainer.prototype);
		Nnfs.ParticleContainer.prototype.constructor = Nnfs.ParticleContainer;
		Nnfs.ParticleContainer.makeParticle = function (id, parent, scale, position, rotation, uvs, alpha) {
			const pc = new Nnfs.ParticleContainer({ scale: scale, position: position, rotation: rotation, uvs: uvs, alpha: alpha, _id: id });
			parent.addChild(pc);
			return pc;
		};
		Nnfs.ParticleContainer.prototype.initialize = function (properties) {
			PIXI.particles.ParticleContainer.call(this, properties._maxSize, properties, undefined, undefined);
			...
			this.setLifetime(config.lifetime.length == null ? config.lifetime : config.lifetime[0], config.lifetime[1]);
			if (config.emit.bursts) {
				this._emitBurst = config.emit.bursts;
			}
			if (config.emit.interval)
				this.setEmitInterval(config.emit.interval.length == null ? config.emit.interval : config.emit.interval[0], config.emit.interval[1]);
			const p = config.position;
			if (p.startX)
				this._positionStart = [p.startX, p.startY != null ? p.startY : p.startX];
			if (p.endX)
				this._positionEnd = [p.endX, p.endY != null ? p.endY : p.endX];
			if (p.start)
				this._positionStart2 = p.start;
			if (p.end)
				this._positionEnd2 = p.end;
			if (config.rotation.start) {
				this._rotationStart = config.rotation.start;
			}
			if (config.rotation.end) {
				this._rotationEnd = config.rotation.end != null ? config.rotation.end : config.rotation.start;
			}
			...
			if (size.startX) {
				this._sizeStart = [size.startX, size.startY != null ? size.startY : size.startX];
			}
			if (size.endX) {
			...
			if (config.opacity.start) {
				...
				this._opacityEnd = config.opacity.end != null ? config.opacity.end : config.opacity.start;
			}
		...
		Nnfs.ParticleContainer.prototype.setTag = function (a) { this._tag = a; return this; };
		Nnfs.ParticleContainer.prototype.setDuration = function (a) { this._duration = a; return this; };
		Nnfs.ParticleContainer.prototype.setEmitBurst = function (...a) { this._emitBurst = a; return this; };
		Nnfs.ParticleContainer.prototype.setEmitInterval = function (a, b = a) { this._emitInterval = [a, b, Math.randomRangeInt(a, b)]; return this; };
		Nnfs.ParticleContainer.prototype.setLifetime = function (a, b = a) { this._lifetime = [a, b]; return this; };
		Nnfs.ParticleContainer.prototype.setLoop = function (a) { this._loop = a; return this; };
		Nnfs.ParticleContainer.prototype.setColor = function (a, b) { this._colorStart = b == null ? a : [a, b]; return this; };
		// Nnfs.ParticleContainer.prototype.setOpacity = function(a, b=a) { this._opacity = [a, b]; return this; };
		Nnfs.ParticleContainer.prototype.setPositionStart = function (a, b = a) { this._positionStart = [a, b]; return this; };
		Nnfs.ParticleContainer.prototype.setPositionEnd = function (a, b = a) { this._positionEnd = [a, b]; return this; };
		Nnfs.ParticleContainer.prototype.setRotationStart = function (a, b = a) { this._rotationStart = [a, b]; return this; };
		Nnfs.ParticleContainer.prototype.setRotationEnd = function (a, b = a) { this._rotationEnd = [a, b]; return this; };
		Nnfs.ParticleContainer.prototype.setScaleStart = function (a, b = a) { this._scaleStart = [a, b]; return this; };
		Nnfs.ParticleContainer.prototype.setScaleStart2 = function (a, b = a) { this._scaleStart2 = [a, b]; return this; };
		Nnfs.ParticleContainer.prototype.setScaleEnd = function (a, b = a) { this._scaleEnd = [a, b]; return this; };
		Nnfs.ParticleContainer.prototype.setSizeStart = function (a, b = a) { this._sizeStart = [a, b]; return this; };
		Nnfs.ParticleContainer.prototype.setSizeEnd = function (a, b = a) { this._sizeEnd = [a, b]; return this; };
		...
		Nnfs.ParticleContainer.prototype.stop = function () { this._isPlay = false; return this; };
		Nnfs.ParticleContainer.prototype.get = function () {
			for (let i = 0, l = this.children.length; i < l; ++i)
				if (!this.children[i]._use)
		...
		Nnfs.ParticleContainer.prototype.makeChildProp = function (p) {
			...
			if (this._positionStart != null) {
				p.x = this._positionStart[0].length == null ? this._positionStart[0] : Math.randomRange(this._positionStart[0][0], this._positionStart[0][1]);
				p.y = this._positionStart[1].length == null ? this._positionStart[1] : Math.randomRange(this._positionStart[1][0], this._positionStart[1][1]);
				if (this._positionEnd != null) {
					prop._sp = new PIXI.Point(p.x, p.y);
					prop._ep = new PIXI.Point(
						this._positionEnd[0].length == null ? this._positionEnd[0] : Math.randomRange(this._positionEnd[0][0], this._positionEnd[0][1]),
						this._positionEnd[1].length == null ? this._positionEnd[1] : Math.randomRange(this._positionEnd[1][0], this._positionEnd[1][1])
					);
				}
			}
			else if (this._positionStart2 != null) {
				p.x = p.y = this._positionStart2.length == null ? this._positionStart2 : Math.randomRange(this._positionStart2[0], this._positionStart2[1]);
				if (this._positionEnd2 != null) {
					prop._sp = new PIXI.Point(p.x, p.y);
					prop._ep = new PIXI.Point();
					prop._ep.set(this._positionEnd2.length == null ? this._positionEnd2 : Math.randomRange(this._positionEnd2[0], this._positionEnd2[1]));
				}
			}
			if (this._rotationStart != null) {
				p.rotation = this._rotationStart.length == null ? this._rotationStart : Math.randomRange(this._rotationStart[0], this._rotationStart[1]);
				if (this._rotationEnd != null) {
					prop._sr = p.rotation;
					prop._er = this._rotationEnd.length == null ? this._rotationEnd : Math.randomRange(this._rotationEnd[0], this._rotationEnd[1]);
				}
			}
			if (this._scaleStart != null) {
				p.scale.x = this._scaleStart[0].length == null ? this._scaleStart[0] : Math.randomRange(this._scaleStart[0][0], this._scaleStart[0][1]);
				p.scale.y = this._scaleStart[1].length == null ? this._scaleStart[1] : Math.randomRange(this._scaleStart[1][0], this._scaleStart[1][1]);
				if (this._scaleEnd != null) {
					prop._ss = new PIXI.Point(p.scale.x, p.scale.y);
					prop._es = new PIXI.Point(
						this._scaleEnd[0].length == null ? this._scaleEnd[0] : Math.randomRange(this._scaleEnd[0][0], this._scaleEnd[0][1]),
						this._scaleEnd[1].length == null ? this._scaleEnd[1] : Math.randomRange(this._scaleEnd[1][0], this._scaleEnd[1][1])
					);
				}
			}
			else if (this._scaleStart2 != null) {
				p.scale.set(this._scaleStart2.length == null ? this._scaleStart2 : Math.randomRange(this._scaleStart2[0], this._scaleStart2[1]));
				if (this._scaleEnd2 != null) {
					prop._ss = new PIXI.Point(p.scale.x, p.scale.y);
					prop._es = new PIXI.Point();
					prop._es.set(this._scaleEnd2.length == null ? this._scaleEnd2 : Math.randomRange(this._scaleEnd2[0], this._scaleEnd2[1]));
				}
			}
			else if (this._sizeStart != null) {
				const w = p.width;
				const h = p.height;
				p.scale.x = this._sizeStart[0].length == null ? this._sizeStart[0] / w : Math.randomRange(this._sizeStart[0][0] / w, this._sizeStart[0][1] / w);
				p.scale.y = this._sizeStart[1].length == null ? this._sizeStart[1] / h : Math.randomRange(this._sizeStart[1][0] / h, this._sizeStart[1][1] / h);
				if (this._sizeEnd != null) {
					prop._ss = new PIXI.Point(p.scale.x, p.scale.y);
					prop._es = new PIXI.Point(
						this._sizeEnd[0].length == null ? this._sizeEnd[0] / w : Math.randomRange(this._sizeEnd[0][0] / w, this._sizeEnd[0][1] / w),
						this._sizeEnd[1].length == null ? this._sizeEnd[1] / h : Math.randomRange(this._sizeEnd[1][0] / h, this._sizeEnd[1][1] / h));
				}
			}
			if (this._colorStart != null) {
				p.setBlendColor(this._colorStart.length == 4 ? this._colorStart : [
					Math.randomRange(this._colorStart[0][0], this._colorStart[1][0]),
					Math.randomRange(this._colorStart[0][1], this._colorStart[1][1]),
					Math.randomRange(this._colorStart[0][2], this._colorStart[1][2]),
					Math.randomRange(this._colorStart[0][3], this._colorStart[1][3])
				]
				);
			}
			if (this._opacityStart) {
				p.opacity = this._opacityStart.length == null ? this._opacityStart : Math.randomRange(this._opacityStart[0], this._opacityStart[1]);
				if (this._opacityEnd != null) {
					prop._so = p.opacity;
					prop._eo = this._opacityEnd.length == null ? this._opacityEnd : Math.randomRange(this._opacityEnd[0], this._opacityEnd[1]);
				}
			}
			if (this._velocityInit) {
				prop._v = new PIXI.Point();
				prop._v.x = this._velocityInit[0].length == null ? this._velocityInit[0] : Math.randomRange(this._velocityInit[0][0], this._velocityInit[0][1]);
				prop._v.y = this._velocityInit[1].length == null ? this._velocityInit[1] : Math.randomRange(this._velocityInit[1][0], this._velocityInit[1][1]);
				prop._vs = new PIXI.Point();
				prop._vs.x = this._velocitySpeed[0].length == null ? this._velocitySpeed[0] : Math.randomRange(this._velocitySpeed[0][0], this._velocitySpeed[0][1]);
				prop._vs.y = this._velocitySpeed[1].length == null ? this._velocitySpeed[1] : Math.randomRange(this._velocitySpeed[1][0], this._velocitySpeed[1][1]);
			}
		...
		// *カードバトルシステム：カード破片
		//==============================
		FragmentParent = function FragmentParent() { this.initialize.apply(this, arguments); };
		...
		FragmentParent.prototype.initialize = function (texture, fragmentNum) {
			Sprite.prototype.initialize.call(this);
			// this.visible = false;
			texture.addLoadListener(function (tex) {
				const w = tex.width;
				const h = tex.height;
				let vertices = new Array(fragmentNum), i, x, y;
				vertices[0] = [0, 0];
				vertices[1] = [w, 0];
				vertices[2] = [0, h];
				vertices[3] = [w, h];
		...
		Fragment = function Fragment() { this.initialize.apply(this, arguments); };
	...
	Game_Temp.prototype.setCameraTarget = function (battlerSprite, ofsx, ofsy, scale) {
	...
	Game_Temp.prototype.getPerlin2d = function (x, y) {
		const i = perlin.simplex2(this._shakeCnt + x, this._shakeCnt + y) * this._shakePower;
	...
	Spriteset_Battle.prototype.update = function () {
		...
		if ($gameTemp._cameraFCount++ < params.CameraParam._calcFrame) {
			// ベーススプライト
			this._baseSprite.x = Math.lerp(this._baseSprite.x, $gameTemp._cameraPos.x + $gameTemp.getPerlin2d(0.00, 0.01), params.CameraParam._moveT);
			this._baseSprite.y = Math.lerp(this._baseSprite.y, $gameTemp._cameraPos.y + $gameTemp.getPerlin2d(0.98, 1.00), params.CameraParam._moveT);
			this._baseSprite.scale.set(Math.lerp(this._baseSprite.scale.x, $gameTemp._cameraScale, params.CameraParam._moveT));
			noise_target = $gameTemp._cameraPos;
			// 遠景
			const b0 = this._battleField.children[0];
			b0.x = Math.lerp(b0.x, params._back1SpritePos[0] + this._baseSprite.x * params.CameraParam._moveWeightBack, params.CameraParam._moveT);
			b0.y = Math.lerp(b0.y, params._back1SpritePos[1] + this._baseSprite.y * params.CameraParam._moveWeightBack, params.CameraParam._moveT);
			b0.scale.set(Math.lerp(b0.scale.x, 1 + ($gameTemp._cameraScale - 1) * params.CameraParam._scaleWeightBack, params.CameraParam._moveT));
			// クランプ処理
			...
			if (this._backSpriteEx != null) {
				...
				ex.x = Math.lerp(ex.x, ex._defaultX + this._baseSprite.x * params.CameraParam._moveWeightFront, params.CameraParam._moveT);
				ex.y = Math.lerp(ex.y, ex._defaultY + this._baseSprite.y * params.CameraParam._moveWeightFront, params.CameraParam._moveT);
				ex.scale.set(Math.lerp(ex.scale.x, 1 + ($gameTemp._cameraScale - 1) * params.CameraParam._scaleWeightFront, params.CameraParam._moveT));
			}
			// 終了時
			this._cameraEnabled = !($gameTemp._cameraEnd && $gameTemp._cameraFCount == params.CameraParam._calcFrame);
		}
		else {
			this._baseSprite.x = $gameTemp._cameraPos.x + $gameTemp.getPerlin2d(0.00, 0.01);
			this._baseSprite.y = $gameTemp._cameraPos.y + $gameTemp.getPerlin2d(1.01, 1.02);
		}
	...
	BattleManager.startAction = function () {
		...
		if (this._subject.isEnemy()) {
			...
			$gameVariables.setValue(params.BattleActorParam._varIdxCurrentSkillId, item.id);
			...
			if (item.meta.commonevent) {
				...
				if (meta.enemy_type != null)
					$gameVariables.setValue(params.BattleActorParam._varIdxCurrentEnemyType, +meta.enemy_type);
	...
	// *背景追加
	//==============================
	const Spriteset_Battle_createLowerLayer = Spriteset_Battle.prototype.createLowerLayer;
	...
	Sprite_Actor.prototype.load = function () {
		// すでにロードしていた場合
		if (!this._isSpine || this._container != null)
			return;
		// コンテナ作成
		...
		// ロード
		this._spine = new PIXI.spine.Spine(Makonet.MpiShowSpine.spineData['actor' + this._actor._actorId]);
		this._spine.scale.set(params.BattleActorParam._scale);
		this._battler._isTransform = false;
		Makonet.MpiShowSpine.setSkinEx(this._spine, params.BattleActorParam._skins[this._battler._isTransform ? 2 : 5]);
		Makonet.MpiShowSpine.setAnimation(this._spine, params.BattleActorParam._animIdle, true);
	...
	Sprite_Actor.prototype.refreshChara = function (force) {
		...
		Makonet.MpiShowSpine.setSkinEx(this._spine, params.BattleActorParam._skins[state_idx]);
		$gameVariables.setValue(params.BattleActorParam._varIdxCurrentActorState, +state_idx);
		if ((this._motion == Sprite_Actor.MOTIONS.walk || this._motion == Sprite_Actor.MOTIONS.wait))
			Makonet.MpiShowSpine.setAnimation(this._spine, params.BattleActorParam._animIdle, true);
		else if (this._motion == Sprite_Actor.MOTIONS.damage)
			Makonet.MpiShowSpine.setAnimation(this._spine, params.BattleActorParam._animDamage, false);
		else if ((this._motion == Sprite_Actor.MOTIONS.spell || this._motion == Sprite_Actor.MOTIONS.item)) {
			Makonet.MpiShowSpine.setAnimation(this._spine, params.BattleActorParam._animUseItem[Math.randomRangeInt(0, params.BattleActorParam._animUseItem.length)], false);
			playSe(params._seUseItem);
		}
		else if (this._motion == Sprite_Actor.MOTIONS.victory) {
			Makonet.MpiShowSpine.setAnimation(BattleManager._spriteset._actorSprites[0]._spine, params.BattleActorParam._animBattleEndWin, false);
			playVoice(Array.random(this._battler._isTransform ? params._seVoiceBattleWinTransform : params._seVoiceBattleWin));
			if (this._battler._isTransform)
				Nnfs.setHpoint(0);
		}
		else if (this._motion == Sprite_Actor.MOTIONS.dead) {
			Makonet.MpiShowSpine.setAnimation(BattleManager._spriteset._actorSprites[0]._spine, params.BattleActorParam._animBattleEndLose, false);
			playVoice(Array.random(this._battler._isTransform ? params._seVoiceBattleLoseTransform : params._seVoiceBattleLose));
			Nnfs.setHpoint(0);
		}
	...
	// *ステートポップアップ
	//==============================
	const Game_BattlerBase_prototype_resetStateCounts = Game_BattlerBase.prototype.resetStateCounts;
