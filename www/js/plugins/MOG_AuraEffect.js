...
Game_Battler.prototype.getAuraData = function () {
  this.notetags().forEach(function (note) {
    ...
    if (note_data[0].toLowerCase() == "particle effect") {
      ...
      this._particlesEffect.blendMode = Math.min(
        Math.max(Number(par[3]), 0),
        2
      );
...
// * create Enemies
//==============================
var _mog_auraEffect_sprtBat_createEnemies =
...
SpriteAura.prototype.refreshAuraSprites = function () {
  if (
    this.battler()._auraEffect.mode >= 0 &&
    this._zIndex == this.battler()._auraEffect.z
  ) {
    this.createAuraSprite();
  }
  if (
    this.battler()._particlesEffect.power > 0 &&
    this._zIndex == this.battler()._particlesEffect.z
  ) {
...
SpriteAura.prototype.updateAuraAnimation1 = function () {
  if (this._auraSprite.animation[0] == 0) {
    ...
    if (this._auraSprite.scale.x >= 1.3) {
      this._auraSprite.scale.x = 1.3;
      this._auraSprite.animation[0] = 1;
    }
  } else {
    this._auraSprite.scale.x -= 0.005;
    if (this._auraSprite.scale.x <= 1.0) {
      this._auraSprite.scale.x = 1.0;
      this._auraSprite.animation[0] = 0;
    }
  }
};
...
SpriteAura.prototype.updateAuraSprite = function () {
  if (
    this._auraSprite.org[0] == -1 &&
    this._auraSprite.bitmap &&
    this._auraSprite.bitmap.isReady()
  ) {
...
SpriteAura.prototype.isEnemyPose = function () {
  ...
  if (!this._sprite._battler.isBPose()) {
    return false;
  }
  return true;
};
...
SpriteAura.prototype.refreshParticles = function (sprite, initial) {
  var w = this.isEnemyPose()
    ? this._sprite.bitmap.width / 4
    : this._sprite.bitmap.width;
  sprite.x = Math.randomInt(w) - w / 2;
  sprite.y =
    Math.randomInt(this._sprite.bitmap.height) - this._sprite.bitmap.height;
...
SpriteAura.prototype.needFadeParticles = function (sprite) {
  if (sprite.x < -sprite.limitX) {
    return true;
  }
  if (sprite.x > sprite.limitX) {
    return true;
  }
  if (sprite.y < -sprite.limitY) {
    return true;
  }
  if (sprite.y > 10) {
    return true;
...
SpriteAura.prototype.updateParticles = function (sprite) {
  if (sprite.needGetData && this._sprite.bitmap.isReady()) {
    this.getDataParticles(sprite);
  }
  sprite.x -= sprite.sx;
  sprite.y -= sprite.sy;
