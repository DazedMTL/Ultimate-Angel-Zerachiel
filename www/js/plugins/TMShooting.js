...
(function () {
  ...
  Game_Bullet.prototype.angleToCharacter = function (character) {
    return Math.atan2(
      character._realY + character._collideH / 2 - this._y,
      character._realX + character._collideW - this._x
    );
  };
  // プレイヤーがいる方向（角度）を取得
  ...
  Game_DummyBullet.prototype.timeBomb = function () {
    if (
      SceneManager._scene &&
      SceneManager._scene.constructor === Scene_Equip
    ) {
      var a = $dataSkills[this._skillId].meta["timeBomb"]
        .split(" ")
        .map(Number);
      var space = (Math.PI * 2) / a[0];
      for (var i = 0; i < a[0]; i++) {
        SceneManager._scene.addBullet(
          this._x,
          this._y,
          200 + i,
          Math.cos(a[1]) * a[2],
