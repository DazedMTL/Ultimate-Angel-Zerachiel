...
const RingCommandMenuPluginName =
...
const RingCommandMenuClassAlias = (() => {
  ...
  const PP = PluginParamsParser.parse(
    PluginManager.parameters(RingCommandMenuPluginName),
    typeDefine
  );
  ...
  class RingCommandData {
    ...
    static fromParam(param, internalIndex) {
      if (param.CommandType === "subMenu") {
        const subCommands = JSON.parse(param.SubMenuCommands)
          .map((strSubMenuCommandParam, i) => {
            if (strSubMenuCommandParam === "") return null;
            const subMenuCommandParam = PluginParamsParser.parse(
              JSON.parse(strSubMenuCommandParam),
              {}
            );
  ...
  Game_Temp.prototype.setRingCommandCommonEventRunner = function (
    commonEventRunner
  ) {
  ...
  class Sprite_RingCommand extends Sprite_ClickableMVMZ {
    ...
    initialize(
      ...
      homeY,
      data,
      opt = { mouseEnterCbk: null, mouseExitCbk: null, clickCbk: null }
    ) {
      super.initialize();
      this._homeX = homeX;
      this._homeY = homeY;
  ...
  class RingCommandSpriteController {
    ...
    startOutCenterAndRotation() {
      this.startInOutAndRotation(
        "outCenterAndRotation",
        this._baseMinFar,
        this._baseMaxFar,
        this._baseMinFar,
        false
    ...
    updateOutCenterAndRotation() {
      if (this._baseSprite.far >= this._maxFar - this._marginFar) {
        ...
        if (!this._baseSprite.isBusy()) {
          const speedPer =
            (this._baseSprite.far - this._minFar) /
            (this._maxFar - this._minFar);
          for (const sprite of this._sprites) {
            sprite.rotateSpeed =
              this._baseRotateSpeed - (this._baseRotateSpeed / 2) * speedPer;
    ...
    updateInCenterAndRotation() {
      if (this._baseSprite.far <= this._minFar + this._marginFar) {
        ...
        if (!this._baseSprite.isBusy()) {
          ...
          this._sprites.forEach((sprite, i) => {
            sprite.rotateSpeed =
              this._baseRotateSpeed / 2 +
              (this._baseRotateSpeed / 2) * speedPer;
            sprite.startRotation(360);
          });
        }
        for (const sprite of this._sprites) {
  ...
  class Sprite_RingCommandLabel extends SpriteMVMZ {
    ...
    initialize() {
      ...
      this.bitmap = new Bitmap(
        Sprite_RingCommandLabel.LABEL_WIDTH,
        Sprite_RingCommandLabel.LABEL_HEIGHT
      );
      if (Utils.RPGMAKER_NAME === "MZ") {
  ...
  class Sprite_RingCommandCursor extends SpriteMVMZ {
    ...
    initialize(ringCommandSpriteWidth, ringCommandSpriteHeight) {
      ...
      const cursorWidth =
        ringCommandSpriteWidth + Sprite_RingCommandCursor.PADDING * 2;
      const cursorHeight =
        ringCommandSpriteHeight + Sprite_RingCommandCursor.PADDING * 2;
    ...
    execCommand(data) {
      ...
      switch (commandType) {
        case "actorData":
          ...
          const subjectData =
            this._holdDataStatus[this._holdDataStatus.length - 2].currentData();
    ...
    evaluateData(data) {
      if (data.isEnabled()) {
        switch (data.commandType) {
          ...
          case "commonEvent":
            $gameTemp
              .ringCommandCommonEventRunner()
              .startCommonEvent(data.commonEventId);
            break;
          default:
            throw new Error(
              `CommandType: ${data.commandType} cannot evaluate.`
            );
        }
      } else {
  ...
  class Scene_RingCommandMenu extends Scene_MenuBase {
    create() {
      ...
      this._spriteset.addRingCommandSpriteCallback(
        "mouseEnterCbk",
        mouseEnterCbk
      );
      this._spriteset.addRingCommandSpriteCallback(
    ...
    createBackground() {
      ...
      const backGroundOpacity =
        PP.BackGroundOpacity == null ? 192 : PP.BackGroundOpacity;
      ...
      for (const data of $ringCommnadDatas) {
        ...
        if (data.subCommands) {
          for (const subData of data.subCommands) {
            this._preloadingImageObjects.push(
              ImageManager.loadPicture(subData.image)
            );
          }
        }
      }
    ...
    isBusy() {
      if (this._ringCommandMenuState === "active")
        return this._spriteset.isRingCommandControllerBusy();
      return false;
    }
    ...
    processStart(isRestart) {
      this._spriteset.createRingCommandSprites(
        $gameTemp.ringCommandManager().datas()
      );
    ...
    processClickRotation() {
      this._ringCommandMenuState = "active";
      const index = $gameTemp
        .ringCommandManager()
        .getRingCommandSpriteIndex(this._holdClickRotationSprite);
      $gameTemp.ringCommandManager().changeIndex(index);
      const execCommandType = $gameTemp
        .ringCommandManager()
        .execCommand(this._holdClickRotationSprite.data);
      this.postExecCommand(execCommandType);
    }
    ...
    ringCommandSpriteMouseExitCbk(sprite) {
      if (this._ringCommandMenuState === "active") {
        const index = $gameTemp.ringCommandManager().index();
        this._spriteset.ringCommandShowCursor(index);
        this._spriteset.ringCommandShowLabel(
          $gameTemp.ringCommandManager().currentData().text
        );
      }
    }
    ...
    ringCommandSpriteClickCbk(sprite) {
      if (this._ringCommandMenuState === "active") {
        ...
        this._spriteset.ringCommandShowCursor(
          $gameTemp.ringCommandManager().index()
        );
      }
    }
  ...
  class Spriteset_RingCommandMenu extends Spriteset_Base {
    ...
    update() {
      super.update();
      if (this._ringCommandSpriteController)
        this._ringCommandSpriteController.update();
    }
    ...
    ringCommandCenterX() {
      return $gamePlayer.screenX() * 1.4;
    }

    ringCommandCenterY() {
      return $gamePlayer.screenY() * 1.4 - 30;
    }
    ...
    createRingCommandSprites(datas) {
      ...
      for (const data of datas) {
        const sprite = new Sprite_RingCommand(
          0,
          0,
          data,
          this._ringCommandSpriteCallbacks
      ...
      this._ringCommandCursor = new Sprite_RingCommandCursor(
        baseRingCommandSprite.width,
        baseRingCommandSprite.height
      );
    ...
    ringCommandShowLabel(label) {
      ...
      const ofsY =
        RING_COMMAND_OFFSET_Y -
        baseRingCommandSprite.height -
        baseRingCommandSprite.far;
    ...
    ringCommandChange(targetIndex) {
      let rotationDeg =
        this._ringCommandSpriteController.calcRelativeIndexDeg(targetIndex);
      ...
      this._ringCommandSpriteController.startChangeCommand(
        targetIndex,
        rotationDeg * sign
      );
    }
