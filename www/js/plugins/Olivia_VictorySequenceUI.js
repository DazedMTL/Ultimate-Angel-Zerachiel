...
(function () {
    ...
    BattleManager.processActorLevelUpPhase = function () {
        setTimeout(SceneManager._scene.hideAllWindows.bind(SceneManager._scene), 150);
        setTimeout(SceneManager._scene.createActorLevelUp.bind(SceneManager._scene), 150);
    };
    ...
    Window_BattleVictoryContinue.prototype.update = function () {
        ...
        if (this._duration > 0) {
            ...
            while (!this.checkLevelUp()) {
                actorStat_index++;
                if (actorStat_index >= $gameParty.battleMembers().length) { break; }
            }
    ...
    Window_BattleVictoryContinue.prototype.checkLevelUp = function () {
        if (!$gameParty.battleMembers()[actorStat_index]) { return false; }
        return $gameParty.battleMembers()[actorStat_index]._level != BattleManager.tempActors[actorStat_index]._level
    }
    // //↓20220501 koke Add
    // }
    // //↑20220501 koke Add
    ...
    Window_ActorLevelUp.prototype.drawBackgroundMajorFadeOut = function () {
        ...
        var height = this.height - y - lowerHeight;
        var width = this.width * 2;
        this.contents.fillRect(0, y, width, height, this.dimColor1(), this.dimColor2());
        this.contents.fillRect(0, this.height - lowerHeight + 2, this.width, lowerHeight - 2, this.dimColor2(), this.dimColor1());
        this.changePaintOpacity(false);
        this.contents.fillRect(0, y - 2, width, 2, this.normalColor());
        this.contents.fillRect(0, this.height - lowerHeight, width, 2, this.normalColor());
        if (this.checkAcquiredSkill()) {//this could be skill window
            var lx = Math.round(this.width * 0.7);
            var ly = y + this.lineHeight() * 2;
            var lw = this.width - lx - ckeIndent;
            //this.contents.fillRect(lx, ly, lw, 2, this.normalColor());
            this.drawBackgroundRewardStrip(lx - 20, ly - this.lineHeight(), lw - 100, 100);
            this.contents.fontSize = cFontSize;
            this.drawText(str2, lx - 20 + ckeIndent * 5, ly - this.lineHeight(), lw - 100, this.lineHeight(), 'left');
            this.createSkillWindow(lx, ly, lw);
        }
    };

    Window_ActorLevelUp.prototype.drawBackgroundRewardStrip = function (x, y, width1, width2) {
        var lh = this.lineHeight();
        var hlh = Math.round(this.lineHeight() / 2);
        x += hlh;
        width1 -= hlh;
    ...
    Window_ActorLevelUp.prototype.checkAcquiredSkill = function () {
        if (!$gameParty.battleMembers()[actorStat_index]) { return false; }
        return $gameParty.battleMembers()[actorStat_index]._skills.length != BattleManager.tempActors[actorStat_index]._skills.length
    }
    ...
    Window_ActorStats.prototype.initialize = function () {
        Window_Base.prototype.initialize.call(this, actorStat_x, actorStat_y, actorStat_w, actorStat_h);
        this.opacity = 0;
        this.createSubWindows();
    }
    ...
    Window_SubStats.prototype.initialize = function (statData) {
        Window_Base.prototype.initialize.call(this, 0, subStat_y, actorStat_w, actorStat_h);
        this.opacity = 0;
        this.value = 0;
        this.valueOld = 0;
    ...
    Window_SubStats.prototype.getData = function (statData) {
        this.title = statData;
        switch (statData) {
            case "Level": this.value = this.actor._level; break;
            case "Max HP": this.value = this.actor.param(0); break;
            case "Max MP": this.value = this.actor.param(1); break;
            case "ATK": this.value = this.actor.param(2); break;
            case "DEF": this.value = this.actor.param(3); break;
            case "MAT": this.value = this.actor.param(4); break;
            case "MDF": this.value = this.actor.param(5); break;
            case "AGI": this.value = this.actor.param(6); break;
            case "CRI": this.value = this.actor.param(7); break;
        }
    }
    ...
    Window_SubStats.prototype.getDataOld = function (statData) {
        this.title = statData;
        switch (statData) {
            case "Level": this.valueOld = BattleManager.tempActors[actorStat_index]._level; break;
            case "Max HP": this.valueOld = BattleManager.tempActors[actorStat_index].param(0); break;
            case "Max MP": this.valueOld = BattleManager.tempActors[actorStat_index].param(1); break;
            case "ATK": this.valueOld = BattleManager.tempActors[actorStat_index].param(2); break;
            case "DEF": this.valueOld = BattleManager.tempActors[actorStat_index].param(3); break;
            case "MAT": this.valueOld = BattleManager.tempActors[actorStat_index].param(4); break;
            case "MDF": this.valueOld = BattleManager.tempActors[actorStat_index].param(5); break;
            case "AGI": this.valueOld = BattleManager.tempActors[actorStat_index].param(6); break;
            case "CRI": this.valueOld = BattleManager.tempActors[actorStat_index].param(7); break;
        }
    }
    ...
    Window_ActorSkills.prototype.initialize = function (lx, ly, lw) {
        Window_Base.prototype.initialize.call(this, lx, ly, lw, this.lineHeight() * 10);
        this.lw = lw;
        this.opacity = 0;
        this.contentsOpacity = 0;
        this.iniSkill = 0;
    ...
    Window_ActorSkills.prototype.createSubWindows = function (lw) {
        ...
        if (skillMax > this._cSkillList.length) { skillMax = this._cSkillList.length; }
        ...
        if (this._cSkillList.length > 8) {
            this.contents.fontSize = cFontSize;
            var cp1 = String(this.iniSkill / 8 + 1); var cp2 = String(Math.floor(this._cSkillList.length / 8) + 1);
            var txt = 'Page ' + cp1 + ' of ' + cp2 + '/ ← →';
            this.contents.drawText(txt, 0, this.lineHeight() * 3.5, this.width, this.height, 'left');
        }
    }
    ...
    Window_ActorSkills.prototype.update = function (lw) {
        ...
        if (Input.isTriggered('right') && this._cSkillList.length > 0) {
            AudioManager.playSe({ name: turnPageSE, volume: 80, pitch: 100, pan: 100 });
            this.iniSkill += 8;
            this.contentsOpacity = 0;
            this.hideChild();
            this.createSubWindows(this.lw);
    ...
    Window_ActorPortrait.prototype.initialize = function () {
        Window_Base.prototype.initialize.call(this, 0, 0, Graphics.boxWidth / 2, Graphics.boxHeight);
        this.opacity = 0;
        this.createSubWindows();
    }
    ...
    Window_ActorPortrait.prototype.createActorImage = function () {
        var porName = $gameParty.battleMembers()[actorStat_index]._levelupface;
        if (porName == '') {
            var porName = 'actor_' + String($gameParty.battleMembers()[actorStat_index]._actorId);
        }
    ...
    Window_ActorPortrait.prototype.drawActorImage = function () {
        this.contents.blt(this.cbm, 0, 0, this.cbm.width, this.cbm.height, -5 * ckeIndent + portraitOffsetX, 2 * ckeIndent + portraitOffsetY, this.cbm.width, this.cbm.height);
        this.contents.textColor = '#ffffff';
        this.contents.fontSize = cFontSize * 3;
        this.drawText('Level Up', ckeIndent * 5, ckeIndent * 5, this.width, this.height, 'left');
    };
