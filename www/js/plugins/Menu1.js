...
(function () {
  ...
  Scene_Item.prototype.createBackground = function () {
    //if(bgBitmapItem){
    this._backgroundSprite = new Sprite();
    this._backgroundSprite.bitmap = ImageManager.loadPicture("item_bg");
    this.addChild(this._backgroundSprite);
    return;
    //}
    // if background file is invalid, it does original process.
  ...
  Window_Gold.prototype.refresh = function () {
    ...
    if ($gameSwitches.value(144)) {
      this.drawIcon(57, 0, 0);
    }
    if ($gameSwitches.value(145)) {
      this.drawIcon(58, 32, 0);
    }
    if ($gameSwitches.value(146)) {
      this.drawIcon(59, 64, 0);
    }
  };
  ...
  Scene_Equip.prototype.createBackground = function () {
    if (bgBitmapEquip) {
      ...
      this._backgroundSprite.bitmap = ImageManager.loadPicture(
        bgBitmapEquip + $gameParty.menuActor()._actorId
      );
      this.addChild(this._backgroundSprite);
      return;
    }
  ...
  Window_EquipStatus.prototype.drawCurrentParam = function (x, y, paramId) {
    ...
    if (paramId <= 7) {
      ...
      var actorparam = Yanfly.Util.toGroup(
        this._actor.elementRate(1) * 100 - 100
      );
      actorparam = Math.floor(actorparam);
    } else if (paramId == 11) {
      var actorparam = Yanfly.Util.toGroup(
        this._actor.elementRate(2) * 100 - 100
      );
      actorparam = Math.floor(actorparam);
    } else if (paramId == 12) {
      var actorparam = Yanfly.Util.toGroup(
        this._actor.elementRate(3) * 100 - 100
      );
      actorparam = Math.floor(actorparam);
    } else if (paramId == 13) {
      var actorparam = Yanfly.Util.toGroup(
        this._actor.elementRate(4) * 100 - 100
      );
      actorparam = Math.floor(actorparam);
    } else if (paramId == 14) {
      var actorparam = Yanfly.Util.toGroup(
        this._actor.elementRate(5) * 100 - 100
      );
      actorparam = Math.floor(actorparam);
    } else if (paramId == 15) {
      var actorparam = Yanfly.Util.toGroup(
        this._actor.elementRate(6) * 100 - 100
      );
      actorparam = Math.floor(actorparam);
    } else if (paramId == 16) {
      var actorparam = Yanfly.Util.toGroup(
        this._actor.elementRate(7) * 100 - 100
      );
      actorparam = Math.floor(actorparam);
    } else if (paramId == 17) {
      var actorparam = Yanfly.Util.toGroup(
        this._actor.elementRate(8) * 100 - 100
      );
      actorparam = Math.floor(actorparam);
    } else if (paramId == 18) {
      var actorparam = Yanfly.Util.toGroup(
        this._actor.stateRate(4) * 100 - 100
      );
      actorparam = Math.floor(actorparam);
    } else if (paramId == 19) {
      var actorparam = Yanfly.Util.toGroup(
        this._actor.stateRate(5) * 100 - 100
      );
      actorparam = Math.floor(actorparam);
    } else if (paramId == 20) {
      var actorparam = Yanfly.Util.toGroup(
        this._actor.stateRate(6) * 100 - 100
      );
      actorparam = Math.floor(actorparam);
    } else if (paramId == 21) {
      var actorparam = Yanfly.Util.toGroup(
        this._actor.stateRate(7) * 100 - 100
      );
      actorparam = Math.floor(actorparam);
    } else if (paramId == 22) {
      var actorparam = Yanfly.Util.toGroup(
        this._actor.stateRate(8) * 100 - 100
      );
      actorparam = Math.floor(actorparam);
    } else if (paramId == 23) {
      var actorparam = Yanfly.Util.toGroup(
        this._actor.stateRate(9) * 100 - 100
      );
      actorparam = Math.floor(actorparam);
    } else if (paramId == 24) {
      var actorparam = Yanfly.Util.toGroup(
        this._actor.stateRate(10) * 100 - 100
      );
      actorparam = Math.floor(actorparam);
    } else if (paramId == 25) {
      var actorparam = Yanfly.Util.toGroup(
        this._actor.stateRate(11) * 100 - 100
      );
      actorparam = Math.floor(actorparam);
    } else if (paramId == 26) {
      var actorparam = Yanfly.Util.toGroup(
        this._actor.battlepower(this._actor)
      );
    }
    ...
    if ($gameVariables._data[402] == "Equip") {
      if (item) {
        ...
        if (item.meta["武器性能"]) {
          ...
          var wea =
            "斬+" +
            Math.round(item.traits[0].value * 100 - 100) +
            " 突+" +
            Math.round(item.traits[1].value * 100 - 100) +
            " 打+" +
            Math.round(item.traits[2].value * 100 - 100);
  ...
  Scene_File.prototype.createBackground = function () {
    this._backgroundSprite = new Sprite();
    this._backgroundSprite.bitmap = ImageManager.loadPicture("save_bg");
    this.addChild(this._backgroundSprite);
    return;
    // if background file is invalid, it does original process.
  ...
  Scene_GameEnd.prototype.createBackground = function () {
    this._backgroundSprite = new Sprite();
    this._backgroundSprite.bitmap = ImageManager.loadPicture("end_bg");
    this.addChild(this._backgroundSprite);
    return;
    // if background file is invalid, it does original process.
  ...
  Scene_Skill.prototype.createBackground = function () {
    ...
    this._backgroundSprite = new Sprite();
    this._backgroundSprite.bitmap = ImageManager.loadPicture(
      "Menu_skill_" + $gameParty.menuActor()._actorId
    );
    this.addChild(this._backgroundSprite);
    return;
    //}
