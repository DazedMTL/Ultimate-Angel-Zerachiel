...
(function () {
  ...
  var param = {};
  //Basic Stteings
  param.varNum_DefaultLV = getParamNumber([
    "VariableNumber_DefaultEnemyLevel",
    "デフォルト敵レベル用の変数番号",
  ]);
  param.varNum_ForcedVictory = getParamNumber([
    "VariableNumber_ForcedVictoryJudgment",
    "強制勝利判定用の変数番号",
  ]);
  param.additionalBattleLog = getParamString([
    "AdditionalBattleLog",
    "バトルログ追加文字",
  ]);
  ...
  NTMO.BGB_Base.forceVictory = function () {
    if (!SceneManager.isCurrentScene_BGB(Scene_Battle)) {
      //Is current scene Scene_Battle ?
      return;
    }
  ...
  NTMO.BGB_Base.performCollapse = function () {
    $gameTroop.members().forEach(function (enemy) {
      ...
      switch (
        param.effectType //Select effect type.
      ) {
        case "EscapeType":
          enemy.escape_BGB();
          break;
  ...
  NTMO.BGB_Base.shouldCutBattleScene = function () {
    ...
    $gameTroop.members().forEach(function (enemy) {
      enemyTotalLevel += enemy.enemy().meta["BGB_LV"]
        ? Number(enemy.enemy().meta["BGB_LV"])
        : $gameVariables.value(param.varNum_DefaultLV);
    });
  ...
  var _BattleManager_displayVictoryMessage =
