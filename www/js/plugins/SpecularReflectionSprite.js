...
(function () {
  ...
  var terrainIds = parameters["TerrainId"].split(",").map(function (n) {
    return Number(n);
  });
  ...
  Game_Event.prototype.isReflect = function () {
    return (
      this.event() &&
      !(this.event().meta["鏡像表示なし"] || this.event().meta["NoReflection"])
    );
  };
  ...
  Sprite_Specular.prototype.updateVisibility = function () {
    ...
    if (
      this._startBattle ||
      this._character.isTransparent() ||
      !this._character.isReflect()
    ) {
      this.visible = false;
      return;
    }
    ...
    var y =
      this._character.y + Math.ceil(this.offsetY() / $gameMap.tileWidth()) + 1;
    ...
    switch (dir) {
      case 2:
        y = reflectTerrain ? y : y - 1;
        break;
      case 4:
        x = reflectTerrain ? x : x + 1;
        break;
      case 6:
        x = reflectTerrain ? x : x - 1;
        break;
  ...
  Sprite_Specular.prototype.offsetY = function () {
    var offsetY = offsetYVariableId
      ? $gameVariables.value(offsetYVariableId)
      : 0;
    offsetY = this._character.isEvent()
      ? this._character.specularOffsetY()
      : offsetY;
    return offsetY;
  };
  ...
  var __SMap_snapForBattleBackground =
