...
Moghunter.eventInd_FontSize = Number(
  Moghunter.parameters["Default Font Size"] || 16
);
...
// * set Indicators
//==============================
Game_CharacterBase.prototype.setIndicators = function (
  fileName,
  animode,
  range,
  d,
...
// * create Lower Layer
//==============================
var _mog_eventIndicators_srmap_createLowerLayer =
...
EventIndicators.prototype.refreshText = function () {
  ...
  var text =
    this.data().mode === 1
      ...
      : Number($gameVariables.value(this.data().id));
  this._text.bitmap.drawText(text, 0, 0, 200, 40, "center");
  if (this.data().mode === 2) {
    this._varValue = $gameVariables.value(this.data().id);
  }
};
...
EventIndicators.prototype.needRefreshText = function () {
  ...
  if (this._varValue != $gameVariables.value(this.data().id)) {
    return true;
  }
  return false;
};
...
EventIndicators.prototype.sensorD = function () {
  return (
    Math.abs($gamePlayer.x - this.character().x) +
    Math.abs($gamePlayer.y - this.character().y)
  );
};
...
EventIndicators.prototype.updateShake = function () {
  this.data().shake[4]++;
  if (this.data().shake[4] < 4) {
    return;
  }
  this.data().shake[4] = 0;
  this.data().shake[2] =
    -this.data().shake[1] / 2 + Math.randomInt(this.data().shake[1]);
  this.data().shake[3] =
    -this.data().shake[1] / 2 + Math.randomInt(this.data().shake[1]);
};
...
EventIndicators.prototype.updateFrames = function () {
  this.data().frames[2]++;
  if (this.data().frames[2] < this.data().frames[3]) {
    return;
  }
  this.refreshBitmap();
  this.data().frames[2] = 0;
  this.data().frames[0]++;
  if (this.data().frames[0] >= this.data().frames[1]) {
