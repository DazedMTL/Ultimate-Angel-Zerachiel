...
Moghunter.skillName_duration = Number(
  Moghunter.parameters["ABS Mode Duration"] || 120
);
...
BattleManager.setSkillName = function () {
  if (!this._subject.currentAction() || !this._subject.currentAction().item()) {
    return;
  }
...
// ** create Spriteset
//==============================
var _mog_actionName_sbattle_createSpriteset =
...
// ** create Lower Layer
//==============================
var _mog_ActionName_sprMap_createLowerLayer =
...
SpriteSkillName.prototype.isVisible = function () {
  ...
  if (Imported.MOG_ChronoEngine) {
    if ($gameSystem.isChronoMode()) {
      if (
        $gameSystem._chronoMode.inTurn &&
        $gameTemp._skillNameDuration[0] > 0
      ) {
        return true;
      }
    } else {
      if ($gameTemp._skillNameDuration[0] > 0) {
        return true;
      }
    }
    return false;
...
SpriteSkillName.prototype.update = function () {
  ...
  if ($gameTemp._skillNameData[0]) {
    this.refreshSkillName();
  }
  if ($gameTemp._skillNameDuration[0] > 0) {
    $gameTemp._skillNameDuration[0]--;
  }
  this.updateVisible();
  this.visible = true;
