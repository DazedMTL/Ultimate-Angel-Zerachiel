...
(function () {
  ...
  param.hiddenGameTitle = getParamBoolean([
    "HiddenGameTitle",
    "ゲームタイトル非表示",
  ]);
  ...
  Window_SavefileList.prototype.drawPartyFaces = function (info, x, y) {
    ...
    info.faces.forEach(function (faceData, index) {
      this.drawFace(
        faceData[0],
        faceData[1],
        x + index * (Window_Base._faceWidth + 4),
        y
      );
    }, this);
  };
  ...
  Window_SavefileList.prototype.drawMapName = function (info, x, y, width) {
    ...
    this.drawText(
      ...
      y,
      width,
      param.hiddenGameTitle ? "left" : "right"
    );
  };

  var _Window_SavefileList_drawContents =
