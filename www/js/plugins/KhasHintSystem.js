...
Khas.Hint.TITLE_ITALIC =
  Khas.Hint.PARAMETERS["Title italic"].toLowerCase() == "on";
Khas.Hint.MESSAGE_BOLD =
  Khas.Hint.PARAMETERS["Message bold"].toLowerCase() == "on";
Khas.Hint.MESSAGE_ITALIC =
  Khas.Hint.PARAMETERS["Message italic"].toLowerCase() == "on";
Khas.Hint.TITLE_COLOR = Khas.Hint.PARAMETERS["Title color"];
Khas.Hint.MESSAGE_COLOR = Khas.Hint.PARAMETERS["Message color"];
Khas.Hint.BACKGROUND_COLOR = parseInt(
  Khas.Hint.PARAMETERS["Background color"].replace(/^#/, ""),
  16
);
Khas.Hint.BACKGROUND_OPACITY =
  Number(Khas.Hint.PARAMETERS["Background opacity"]).clamp(0, 100) / 100.0;
...
// * Game Event
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
Object.defineProperty(Game_Event.prototype, "hintTitle", {
  get: function () {
    return this._hintTitle;
  },
});
Object.defineProperty(Game_Event.prototype, "hintMessage", {
  get: function () {
    return this._hintMessage;
  },
});
Game_Event.prototype.khs_initialize = Game_Event.prototype.initialize;
Game_Event.prototype.khs_khasScanComments =
  Game_Event.prototype.khasScanComments;
...
Game_Event.prototype.khasScanComments = function () {
  ...
  if (this.page()) {
    ...
    if (this._hintTitle && $khasGraphics.hintSystem)
      $khasGraphics.hintSystem.addHint(
        new Khas_Hint(this, this._hintTitle, this._hintMessage)
      );
  }
};
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
Object.defineProperty(Khas_Hint.prototype, "sprite", {
  get: function () {
    return this._sprite;
  },
});
Object.defineProperty(Khas_Hint.prototype, "destroy", {
  get: function () {
    return this._destroy;
  },
});
Object.defineProperty(Khas_Hint.prototype, "event", {
  get: function () {
    return this._event;
  },
});
Khas_Hint.prototype.initialize = function (event, title, message) {
  ...
  var titleStyle = new PIXI.TextStyle({
    fontFamily: Khas.Hint.FONT_NAME,
    fontSize: Khas.Hint.FONT_SIZE,
    fill: Khas.Hint.TITLE_COLOR,
    wordWrap: true,
    wordWrapWidth: Khas.Hint.MAX_WIDTH,
  });
  var messageStyle = new PIXI.TextStyle({
    fontFamily: Khas.Hint.FONT_NAME,
    fontSize: Khas.Hint.FONT_SIZE,
    fill: Khas.Hint.MESSAGE_COLOR,
    wordWrap: true,
    wordWrapWidth: Khas.Hint.MAX_WIDTH,
  });
  ...
  this._width =
    Math.max(hintTitle.width, this._hasMessage ? hintMessage.width : 0) +
    Khas.Hint.MARGIN * 2;
  this._height =
    hintTitle.height +
    (this._hasMessage ? hintMessage.height : 0) +
    Khas.Hint.MARGIN * (this._hasMessage ? 3 : 2);
  var hintBackground = new PIXI.Graphics();
  hintBackground.beginFill(
    Khas.Hint.BACKGROUND_COLOR,
    Khas.Hint.BACKGROUND_OPACITY
  );
...
Khas_Hint.prototype.update = function () {
  if (
    this._title != this._event.hintTitle ||
    this._message != this._event.hintMessage
  )
    this._destroy = true;
  var cx = Math.round(
    ($gameMap.adjustX(this._event._realX) + 0.5) * $gameMap.tileWidth() +
      $gameScreen.shake()
  );
  var cy = Math.round(
    ($gameMap.adjustY(this._event._realY) + 0.5) * $gameMap.tileHeight()
  );
  this.setPos(cx, cy);
...
// * Khas HintSystem
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
function Khas_HintSystem() {
  this.initialize.apply(this, arguments);
}
Object.defineProperty(Khas_HintSystem.prototype, "layer", {
  get: function () {
    return this._layer;
  },
});
...
Khas_HintSystem.prototype.addMapHints = function () {
  ...
  for (var i = 0; i < events.length; i++) {
    var event = events[i];
    if (event.hintTitle)
      this.addHint(new Khas_Hint(event, event.hintTitle, event.hintMessage));
  }
};
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// * Khas Graphics
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
Object.defineProperty(Khas_Graphics.prototype, "hintSystem", {
  get: function () {
