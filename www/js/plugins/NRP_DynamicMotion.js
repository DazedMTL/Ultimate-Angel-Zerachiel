...
(function () {
  ...
  var pDefaultEnemyMotionDuration = toNumber(
    parameters["defaultEnemyMotionDuration"],
    12
  );
  ...
  const pNoMirrorForFriend = toBoolean(
    animatinParameters["noMirrorForFriend"],
    true
  );
  ...
  BaseMotion.prototype.makeRepeatMotion = function (dynamicMotionList, r) {
    ...
    performers.forEach(function (performer, performerIndex) {
      ...
      if (isEvery) {
        this.targets.forEach(function (target, index) {
          ...
          dynamicMotion = this.createDynamicMotion(
            performer,
            target,
            r,
            targetDelay
    ...
    this.totalDuration = Math.max(
      this.totalDuration,
      targetDelay + dynamicMotion.maxDuration
    );
    // この<D-Motion>内の実行時間
    this.baseDuration = Math.max(
      this.baseDuration,
      baseDelaySum + dynamicMotion.maxDuration
    );
   ...
   * ●動的モーションデータを生成する。
   */
  BaseMotion.prototype.createDynamicMotion = function (
    performer,
    target,
    r,
    delay
  ...
  BaseMotion.prototype.isUseDuration = function () {
    if (
      ...
      this.radX != undefined ||
      this.radY != undefined ||
      this.gridEx != undefined ||
      this.gridEy != undefined ||
      this.gx != undefined ||
      this.gy != undefined ||
   ...
   * ●初期化処理
   */
  DynamicMotion.prototype.initialize = function (
    baseMotion,
    performer,
    target,
    r
    ...
    if (baseMotion.motion != undefined) {
      ...
      if (baseMotion.motionFrame != undefined) {
        ...
        // 初期値を取得
        this.motionDuration = baseMotion.getDefaultMotionDuration(
          spriteA,
          this.motion
        );
      }
    ...
    if (
      this.motion &&
      (this.motion == "attack" ||
        (Sprite_Actor.MOTIONS[this.motion] &&
          !Sprite_Actor.MOTIONS[this.motion].loop))
    ) {
      // 合計モーション時間を計算
      // ※モーション時間は３パターンを想定し、*3して計算
      this.motionDurationTotal =
        nvl(this.motionDuration) * baseMotion.motionPatternCount();
  ...
  Sprite_Battler.prototype.updateDynamicMotion = function () {
    ...
    if (
      ...
      !BattleManager._spriteset._isDynamicAnimationReady
    ) {
      return;
    }
   ...
   * 【独自】アタックモーション
   */
  Game_Battler.prototype.performAttackDynamicMotion = function (
    weaponId,
    weaponType
  ) {
    var wtypeId;
  ...
  Sprite.prototype.updateDynamicMove = function () {
    ...
    if (motion && this._movementDuration > 0) {
      ...
      if (motion._evalRadiusX) {
        ...
        motion._addX +=
          eval(motion._evalRadiusX) * Math.cos(eval(motion._evalRadX));
      }
      if (motion._evalRadiusY) {
        ...
        motion._addY +=
          eval(motion._evalRadiusY) * Math.sin(eval(motion._evalRadY));
      }
      ...
      if (
        ...
        this._startAirY != this._targetAirY
      ) {
        var rate = t / et;
        // 始点から終点へと移動する計算式
      ...
      if (arcX) {
        motion._offsetArcX =
          (-arcX / Math.pow(et / 2, 2)) *
            Math.pow(Math.min(t, et) - et / 2, 2) +
          arcX;
      }
      if (arcY) {
        this._airY +=
          (-arcY / Math.pow(et / 2, 2)) *
            Math.pow(Math.min(t, et) - et / 2, 2) +
          arcY;
      }
  ...
  Game_Action.prototype.isDynamicMotion = function () {
    ...
    if (pShortSettingTagName) {
      settingTagNameSet =
        "(?:" + SETTING_TAG_NAME + "|" + pShortSettingTagName + ")";
    }
  ...
  Spriteset_Battle.prototype.isMotionPlaying = function () {
    return this.battlerSprites().some(function (sprite) {
      return (
        sprite._battler &&
        (sprite._battler.isDynamicMotionRequested() || sprite.isMotionPlaying())
      );
    });
  };
  ...
  if (pUsePriority) {
     ...
     * ※上に同関数があるので名前を_2に変更
     */
    var _Game_Battler_performActionEnd_2 =
