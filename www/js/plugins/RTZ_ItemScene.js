...
(function () {
  ...
  DataManager.itemSecondaryCategories = function (item) {
    if (!item) return [];
    if (this.isItemEx(item))
      item = item.baseItemId ? $dataItems[item.baseItemId] : item;
    if (this.isWeaponEx(item))
      item = item.baseItemId ? $dataWeapons[item.baseItemId] : item;
    if (this.isArmorEx(item))
      item = item.baseItemId ? $dataArmors[item.baseItemId] : item;
    ...
    if (this.isItemEx(item) && item.itypeId === 1) {
      ...
      if (item._secondaryCategories.indexOf(weapon) < 0)
        item._secondaryCategories.push(weapon);
      ...
      if (item._secondaryCategories.indexOf(armor) < 0)
        item._secondaryCategories.push(armor);
  ...
  Window_ItemCategory.prototype.initCategories = function () {
    this._categories = [[], [], [], [], []];
    var n = 0;
    if (Imported["LimitPossession"] && $gameParty.reserveItems().length > 0)
      n = 1;
  ...
  Window_ItemCategory.prototype.backSubCategory = function () {
    var max = this.categorySize(this.index());
    if (max === 1) return;
    this._subIndex[this.index()] =
      (this._subIndex[this.index()] + (max - 1)) % max;
    this._itemWindow.setSubCategory(this.currentSubCategory());
    SoundManager.playCursor();
    this.refresh();
  };
  ...
  Window_ItemDescription.prototype.convertWordWrapEscapeCharacters = function (
    text
  ) {
  ...
  Window_ItemDescription.prototype.processNormalCharacter = function (
    textState
  ) {
  ...
  Window_ItemTraits.prototype.drawQuality = function () {
    var x = 123;
    var y = 43;
    var quality = this._item.meta["quality"]
      ? "品質 " + String(this._item.meta["quality"])
      : "  N/A";
