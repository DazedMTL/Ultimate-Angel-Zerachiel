...
function KoTCMinimapStart() {
    ...
    $KoTCMinimapSystem.MapExploredNotificationOn = ($KoTCMinimapSystem.Parameters['Map Exploration Notification'] !== "false");
    ...
    $KoTCMinimapSystem.CurrentMinimapX = Number($KoTCMinimapSystem.Parameters['Minimap X Horizontal Position']) + $KoTCMinimapSystem.CurrentMinimapBorderThickness;
    $KoTCMinimapSystem.CurrentMinimapY = Number($KoTCMinimapSystem.Parameters['Minimap Y Vertical Position']) + $KoTCMinimapSystem.CurrentMinimapBorderThickness;
    $KoTCMinimapSystem.MinimapMode = Number($KoTCMinimapSystem.Parameters['Minimap Mode']);
    $KoTCMinimapSystem.ExplorationMapMode = ($KoTCMinimapSystem.Parameters['Exploration Map Mode'] === "true");
    $KoTCMinimapSystem.ExplorationMapRefreshRate = Number($KoTCMinimapSystem.Parameters['Exploration Refresh Rate']);
    $KoTCMinimapSystem.KoTCDrawParallax = ($KoTCMinimapSystem.Parameters['Draw Parallax as Background'] === "true");
    $KoTCMinimapSystem.MapBookEnabled = ($KoTCMinimapSystem.Parameters['KoTC Map Book'] === "true");
    if ($KoTCMinimapSystem.MapBookEnabled !== false) {
        $KoTCMinimapSystem.AutoAddMapsToMapBook = ($KoTCMinimapSystem.Parameters['Auto Add Maps to Map Book'] === "true")
    }
    ...
    if ($KoTCMinimapSystem.ExplorationMapMode !== false) {
        ...
        //     };
        $KoTCMinimapSystem.MapFullyExploredAudio = $KoTCMinimapSystem.Parameters['Map Fully Explored Sound'];
        $KoTCMinimapSystem.MapFullyExploredAudioVolume = Number($KoTCMinimapSystem.Parameters['Map Fully Explored Sound Volume']);
    };
    // Map Wall Sprite Init

    $KoTCMinimapSystem.MapWallSprite = new Bitmap($KoTCMinimapSystem.CurrentMinimapSize, $KoTCMinimapSystem.CurrentMinimapSize);
    $KoTCMinimapSystem.MapWallSprite = new Sprite($KoTCMinimapSystem.MapWallSprite);
    ...
    if (Utils.RPGMAKER_NAME == "MZ") {
        PluginManager.registerCommand('KoTC Advanced Minimap', "DisableKoTCMinimap", data => {
            KoTCMinimapOff();
        });
        PluginManager.registerCommand('KoTC Advanced Minimap', "EnableKoTCMinimap", data => {
            KoTCMinimapOn();
        });
        PluginManager.registerCommand('KoTC Advanced Minimap', "SetKoTCMinimapWallColor", data => {
            KoTCMinimapSetWallColor(data.Color);
        });
        PluginManager.registerCommand('KoTC Advanced Minimap', "SetKoTCMinimapBackgroundColor", data => {
            KoTCMinimapSetBackgroundColor(data.Color);
        });
        PluginManager.registerCommand('KoTC Advanced Minimap', "SetKoTCMinimapPlayerColor", data => {
            KoTCMinimapSetPlayerColor(data.Color);
        });
        PluginManager.registerCommand('KoTC Advanced Minimap', "SetKoTCMinimapBorderColor", data => {
            KoTCMinimapSetBorderColor(data.Color);
        });
        PluginManager.registerCommand('KoTC Advanced Minimap', "SetKoTCMinimapSize", data => {
            KoTCMinimapSetSize(data.Size);
        });
        PluginManager.registerCommand('KoTC Advanced Minimap', "KoTCOpenMapBook", data => {
            SceneManager.push(Scene_KoTCMapBook);
        });
        PluginManager.registerCommand('KoTC Advanced Minimap', "KoTCMapBookClear", data => {
            clearKoTCMapBook();
        });
        PluginManager.registerCommand('KoTC Advanced Minimap', "KoTCAddMapToMapBook", data => {
            kotcAddMapToBook(data.MapID);
        });
        PluginManager.registerCommand('KoTC Advanced Minimap', "KoTCRemoveMapToMapBook", data => {
            kotcRemoveMapFromBook(data.MapID);
        });
        PluginManager.registerCommand('KoTC Advanced Minimap', "KoTCMinimapFollowPlayerOn", data => {
            KoTCMinimapFollowPlayerOn(data.ZoomScale)
        });
        PluginManager.registerCommand('KoTC Advanced Minimap', "KoTCMinimapFollowPlayerOff", data => {
            KoTCMinimapFollowPlayerOff()
        });
    } else {
    ...
    if ($KoTCMinimapSystem.MinimapMode == 1) {
        ...
        if (!MVNodeFS.checkFile("data/", "ColorStorageRepository.txt")) {
            ...
            setTimeout(() => {
                MVNodeFS.writeFile("data/", "ColorStorageRepository.txt", JSON.stringify($KoTCMinimapSystem.TilesetColorRepository));
...
KoTCMinimapEnable = function () {
    if ($dataMap && !$gamePlayer.isTransferring() && typeof KoTCMapDisabled == 'undefined') {
        ...
        switch ($KoTCMinimapSystem.MinimapMode) {
                ...
                for (; x < width; x++) {
                    ...
                    for (; y < height; y++) {
                        ...
                        switch (true) {
                            ...
                            case $KoTCMinimapSystem.TilesetColorRepository[$gameMap.tileset().name][$gameMap.tileId(xtarget, ytarget, 3)] !== undefined:

                                var tilecolor = $KoTCMinimapSystem.TilesetColorRepository[$gameMap.tileset().name][$gameMap.tileId(xtarget, ytarget, 3)];
                                $KoTCMinimapSystem.MapWallSprite.bitmap.fillRect(x, y, 1, 1, tilecolor);
                                break;

                            case $KoTCMinimapSystem.TilesetColorRepository[$gameMap.tileset().name][$gameMap.tileId(xtarget, ytarget, 1)] !== undefined:
                                var tilecolor = $KoTCMinimapSystem.TilesetColorRepository[$gameMap.tileset().name][$gameMap.tileId(xtarget, ytarget, 1)];
                            ...
                            case $KoTCMinimapSystem.TilesetColorRepository[$gameMap.tileset().name][$gameMap.tileId(xtarget, ytarget, 0)] !== undefined:
                                var tilecolor = $KoTCMinimapSystem.TilesetColorRepository[$gameMap.tileset().name][$gameMap.tileId(xtarget, ytarget, 0)];
                                $KoTCMinimapSystem.MapWallSprite.bitmap.fillRect(x, y, 1, 1, tilecolor);
            ...
            case 2:
                ...
                if ($KoTCMinimapSystem.KoTCMinimapPictureSprite && $KoTCMinimapSystem.SecondaryMinimapSprite.children.includes($KoTCMinimapSystem.KoTCMinimapPictureSprite) && $KoTCMinimapSystem.MapMemory == String($dataMap.data)) {
                    ...
                    $KoTCMinimapSystem.KoTCMinimapPictureInterval = setInterval(() => {
                        if ($dataMap && SceneManager._scene instanceof Scene_Map && SceneManager._scene._spriteset !== undefined) {
                            $KoTCMinimapSystem.MainMinimapSprite.removeChild($KoTCMinimapSystem.KoTCMinimapPictureSprite);
                            clearInterval($KoTCMinimapSystem.KoTCMinimapPictureInterval);
                            $KoTCMinimapSystem.MapMemory = String($dataMap.data);
                            $KoTCMinimapSystem.KoTCMinimapPicture = ShrinkBitmapToMapSize(OrangeKoTCHybrid.getMapshot());
                            $KoTCMinimapSystem.KoTCMinimapPictureSprite = new Sprite($KoTCMinimapSystem.KoTCMinimapPicture);
                            $KoTCMinimapSystem.SecondaryMinimapSprite.addChild($KoTCMinimapSystem.KoTCMinimapPictureSprite);
        ...
        if ($KoTCMinimapSystem.MapBookTimeout) {
            $KoTCMinimapSystem.MapBookTimeout = setTimeout(() => {
                $KoTCMapImageCache[$gameMap._mapId].ImageDataStorage[$KoTCMinimapSystem.CurrentMapIndex] = CreateMapBookPictureImageData()
                $KoTCMapImageCache[$gameMap._mapId].MapFullImageStorage[$KoTCMinimapSystem.CurrentMapIndex] = CreateMapBookPictureImageData(1)
...
function KoTCStartMapInterval(width, height) {
    if ($KoTCMinimapSystem.KoTCDrawParallax !== false && $gameMap.parallaxName() !== "") {
        ...
        parallaxbitmap.addLoadListener(function () {
            if ($KoTCMinimapSystem.ParallaxSprite) {
                $KoTCMinimapSystem.MainMinimapSprite.removeChild($KoTCMinimapSystem.ParallaxSprite)
            }
            $KoTCMinimapSystem.ParallaxSprite = new Sprite(parallaxbitmap);
            $KoTCMinimapSystem.ParallaxSprite.scale.x = $KoTCMinimapSystem.CurrentMinimapSize / $KoTCMinimapSystem.ParallaxSprite.width;
            $KoTCMinimapSystem.ParallaxSprite.scale.y = $KoTCMinimapSystem.CurrentMinimapSize / $KoTCMinimapSystem.ParallaxSprite.height;
            $KoTCMinimapSystem.SecondaryMinimapSprite.addChildAt($KoTCMinimapSystem.ParallaxSprite, 1);

        })
    } else if ($KoTCMinimapSystem.ParallaxSprite && $gameMap.parallaxName() == "") {
        $KoTCMinimapSystem.SecondaryMinimapSprite.removeChild($KoTCMinimapSystem.ParallaxSprite);
    }
    ...
    if ($KoTCMinimapSystem.ExplorationMapMode !== false) {
        if ($KoTCMinimapSystem.MapMemory2 !== String($dataMap.data) && $KoTCMinimapSystem.FogOfWarSprite) {
            $KoTCMinimapSystem.MapMemory2 = String($dataMap.data);
            $KoTCMinimapSystem.SecondaryMinimapSprite.removeChild($KoTCMinimapSystem.FogOfWarSprite);
            var fogofwar = new Bitmap($KoTCMinimapSystem.CurrentMinimapSize, $KoTCMinimapSystem.CurrentMinimapSize)
            fogofwar.fillRect(0, 0, $KoTCMinimapSystem.CurrentMinimapSize, $KoTCMinimapSystem.CurrentMinimapSize, "black");
            var fogofwarsprite = new Sprite(fogofwar);
            $KoTCMinimapSystem.FogOfWarSprite = fogofwarsprite;
            $KoTCMinimapSystem.SecondaryMinimapSprite.addChild($KoTCMinimapSystem.FogOfWarSprite);
        ...
        if ($KoTCMapImageCache[$gameMap._mapId].DataStorage.includes(String($dataMap.data))) {
            //console.log("Stored Map Detected, stored full image data, name is ", $dataMapInfos[$gameMap._mapId].name)

            var index = $KoTCMapImageCache[$gameMap._mapId].DataStorage.indexOf(String($dataMap.data));
            var chosenmapid = $KoTCMapImageCache[$gameMap._mapId].IDStorage[index];
            if ($KoTCMinimapSystem.MapBookEnabled !== false && $KoTCMapImageCache[$gameMap._mapId].MapFullImageStorage[index] == undefined) {

                switch ($KoTCMinimapSystem.MinimapMode) {
                    case 0:
                        $KoTCMapImageCache[$gameMap._mapId].MapFullImageStorage[index] = CreateMapBookPictureImageData(1)
            ...
            $KoTCMapImageCache[$gameMap._mapId].DataStorage.push(String($dataMap.data));
            ...
            if ($KoTCMinimapSystem.MapBookEnabled !== false) {
                $KoTCMapImageCache[$gameMap._mapId].NameStorage.push($dataMapInfos[$gameMap._mapId].name + index);
        ...
        if ($KoTCMinimapSystem.MapBookEnabled !== false) {
            $KoTCMapImageCache[$gameMap._mapId].ImageDataStorage[$KoTCMinimapSystem.CurrentMapIndex] = CreateMapBookPictureImageData(1)
        };
        var notecheckreg = new RegExp(/<KoTC Map Deny>/i);
        if ($KoTCMinimapSystem.AutoAddMapsToMapBook !== false && $KoTCMapImageCache[0][$KoTCMinimapSystem.CurrentMap] == undefined && notecheckreg.exec($dataMap.note) == null) {
...
KoTCUpdateMinimapLocations = function () {
    if ($dataMap && !$gamePlayer.isTransferring() && SceneManager._scene instanceof Scene_Map) {
        ...
        $KoTCMinimapSystem.KoTCMinimapPlayerSprite.x = Math.round($gamePlayer.x / ($dataMap.width / $KoTCMinimapSystem.CurrentMinimapSize)) + $KoTCMinimapSystem.CurrentMinimapBorderThickness;
        $KoTCMinimapSystem.KoTCMinimapPlayerSprite.y = Math.round($gamePlayer.y / ($dataMap.height / $KoTCMinimapSystem.CurrentMinimapSize)) + $KoTCMinimapSystem.CurrentMinimapBorderThickness;
            ...
            if ($gameMap.events()[l] !== undefined) {
                if (!$gameMap.events()[l]._erased) {
                    if ($gameMap.events()[l].page()) {
                        ...
                        $gameMap.events()[l].list().forEach(function (pageindex) {
                    ...
                    switch (true) {
                        case regexcode1.exec($gameMap.events()[l].event().note) !== null || regexcode1.exec($gameMap.events()[l].KoTCMapComment) !== null:
                            // console.log("Event detected",$gameMap.events()[l])
                            if (RegExp.$1 !== "" && Math.round(Math.hypot($gamePlayer.x - $gameMap.events()[l].x, $gamePlayer.y - $gameMap.events()[l].y) <= RegExp.$1) || RegExp.$1 == "") {
                                if ($KoTCMinimapSystem.ArrayofMinimapPictureSprites[l] == null || $KoTCMinimapSystem.ArrayofMinimapPictureSprites[l] == undefined) {
                                    //      //console.log("Setting Sprite for event.", $gameMap.events()[l]._eventId)
                                    var KoTCMapSprite = KoTCGetImageFromPicture($gameMap.events()[l].characterName(), "characters", $gameMap.events()[l].characterIndex(), 100, 100);
                        ...
                        case regexcode2.exec($gameMap.events()[l].event().note) !== null || regexcode2.exec($gameMap.events()[l].KoTCMapComment) !== null:
                        ...
                        case regexcode3.exec($gameMap.events()[l].event().note) !== null || regexcode3.exec($gameMap.events()[l].KoTCMapComment) !== null:
                            if (RegExp.$2 !== "" && Math.round(Math.hypot($gamePlayer.x - $gameMap.events()[l].x, $gamePlayer.y - $gameMap.events()[l].y) <= RegExp.$2) || RegExp.$2 == "") {
                                if ($KoTCMinimapSystem.ArrayofMinimapPictureSprites[l] == null || $KoTCMinimapSystem.ArrayofMinimapPictureSprites[l] == undefined) {
                        ...
                        case regexcode4.exec($gameMap.events()[l].event().note) !== null || regexcode4.exec($gameMap.events()[l].KoTCMapComment) !== null:
                            if (RegExp.$2 !== "" && Math.round(Math.hypot($gamePlayer.x - $gameMap.events()[l].x, $gamePlayer.y - $gameMap.events()[l].y) <= RegExp.$2) || RegExp.$2 == "") {
                                if ($KoTCMinimapSystem.ArrayofMinimapPictureSprites[l] == null || $KoTCMinimapSystem.ArrayofMinimapPictureSprites[l] == undefined) {
        ...
        if ($KoTCMinimapSystem.FogOfWarSprite == undefined || $KoTCMinimapSystem.FogOfWarSprite.width == 1) {
            var fogofwar = new Bitmap($KoTCMinimapSystem.CurrentMinimapSize, $KoTCMinimapSystem.CurrentMinimapSize)
            fogofwar.fillRect(0, 0, $KoTCMinimapSystem.CurrentMinimapSize, $KoTCMinimapSystem.CurrentMinimapSize, "black");
            var fogofwarsprite = new Sprite(fogofwar);
            $KoTCMinimapSystem.FogOfWarSprite = fogofwarsprite;
        };
        if ($KoTCMinimapSystem.ExplorationMapMode !== false && $KoTCMinimapSystem.SecondaryMinimapSprite.children.indexOf($KoTCMinimapSystem.FogOfWarSprite) !== $KoTCMinimapSystem.MainMinimapSprite.children.length - 1 && $KoTCMinimapSystem.FogOfWarSprite) {
            $KoTCMinimapSystem.SecondaryMinimapSprite.addChild($KoTCMinimapSystem.FogOfWarSprite);
...
function PopulateTilesetColors(spacebetweenpixelscans) {
    if (!MVNodeFS.checkFile("data/", "ColorStorageRepository.txt")) {
            ...
            for (; n < $dataTilesets[t].tilesetNames.length; n++) {
                if ($dataTilesets[t].tilesetNames[n] !== "") {
                    ...
                    for (; p < gridmax; p++) {
                        ...
                        var tiletoscan = KoTCGetImageFromPicture($dataTilesets[t].tilesetNames[n], "tilesets", p, 20, 20, undefined, undefined, n);
                        var averagecolor = GetAverageBitmapColor(tiletoscan.bitmap, spacebetweenpixelscans);
                        ...
                        for (; g < tileend; g++) {
                            ...
                            $KoTCMinimapSystem.TilesetColorRepository[$dataTilesets[t].name][g] = averagecolor;
                        };
                    };
                    $KoTCMinimapSystem.TilesetColorRepository[$dataTilesets[t].name] = Object.assign({}, $KoTCMinimapSystem.TilesetColorRepository[$dataTilesets[t].name]);
        ...
        $KoTCMinimapSystem.TilesetColorRepository = JSON.parse(MVNodeFS.readFile("data/", "ColorStorageRepository.txt"));
    };
};

function MapVisionCheck() {
    if ($gameSystem.MapExplorationData !== undefined && $gameSystem.MapExplorationData[$KoTCMinimapSystem.CurrentMap] && $gameSystem.MapExplorationData[$KoTCMinimapSystem.CurrentMap][Math.round($gamePlayer.y)] && $gameSystem.MapExplorationData[$KoTCMinimapSystem.CurrentMap][Math.round($gamePlayer.y)][Math.round($gamePlayer.x)] == 0 && !$gamePlayer.isTransferring() && $dataMap) {
        var radius = 11 / ($dataMap.width / ($KoTCMinimapSystem.CurrentMinimapSize));
        var x = Math.round($gamePlayer.x / ($dataMap.width / $KoTCMinimapSystem.CurrentMinimapSize));
        var y = Math.round($gamePlayer.y / ($dataMap.height / $KoTCMinimapSystem.CurrentMinimapSize));
        $KoTCMinimapSystem.FogOfWarSprite.bitmap.context.save()
        $KoTCMinimapSystem.FogOfWarSprite.bitmap.context.beginPath()
        $KoTCMinimapSystem.FogOfWarSprite.bitmap.context.arc(x, y, radius, 0, 2 * Math.PI, false);
        $KoTCMinimapSystem.FogOfWarSprite.bitmap.context.clip();
        $KoTCMinimapSystem.FogOfWarSprite.bitmap.clearRect(0, 0, $KoTCMinimapSystem.FogOfWarSprite.width, $KoTCMinimapSystem.FogOfWarSprite.height);
        ...
        for (; hh < h; hh++) {
            var ww = 0;
            for (; ww < w; ww++) {
                var rangedifference = Math.round(Math.hypot($gamePlayer.x - ww, $gamePlayer.y - hh));
                if ($gameSystem.MapExplorationData[$KoTCMinimapSystem.CurrentMap] !== undefined && $gameSystem.MapExplorationData[$KoTCMinimapSystem.CurrentMap][hh] !== undefined && rangedifference < 10 && $gameSystem.MapExplorationData[$KoTCMinimapSystem.CurrentMap][hh][ww] !== 2) {
                    $gameSystem.MapExplorationData[$KoTCMinimapSystem.CurrentMap][hh][ww] = 1;

                };
            };

        };
        if ($gameSystem.MapExplorationData[$KoTCMinimapSystem.CurrentMap] !== undefined) {
            $gameSystem.MapExplorationData[$KoTCMinimapSystem.CurrentMap][Math.round($gamePlayer.y)][Math.round($gamePlayer.x)] = 2;

        }
        //  console.log("Map Vision Check", Math.round($gamePlayer.x), Math.round($gamePlayer.y))
        if ($KoTCMapImageCache[0][0][$gameMap._mapId] == undefined || $KoTCMapImageCache[0][0][$gameMap._mapId][$KoTCMinimapSystem.CurrentMapIndex] == undefined) {
            KoTCCountExploredArea($KoTCMinimapSystem.CurrentMapIndex);
            if ($KoTCMinimapSystem.MapExploredNotificationOn == true && $KoTCMapImageCache[$gameMap._mapId].AmountExplored[$KoTCMinimapSystem.CurrentMapIndex] == $KoTCMapImageCache[$gameMap._mapId].AmountToExplore[$KoTCMinimapSystem.CurrentMapIndex]) {
                if ($KoTCMapImageCache[0][0][$gameMap._mapId] == undefined) {
                    $KoTCMapImageCache[0][0][$gameMap._mapId] = {}
                }
                var notificationbit = new Bitmap(Graphics.boxWidth / 2, Graphics.boxHeight / 2);
                var notificationsprite = new Sprite(notificationbit);
                notificationsprite.bitmap.drawText("Map Fully Explored", notificationsprite.width / 2, notificationsprite.height / 2, 600, 64)
                ...
                $KoTCMapImageCache[0][0][$gameMap._mapId][$KoTCMinimapSystem.CurrentMapIndex] = 1;
                AudioManager.playSe({
...
function UpdateMapVision() {
    if (!$gamePlayer.isTransferring() && $dataMap && SceneManager._scene instanceof Scene_Map) {
        if ($KoTCMapImageCache[$gameMap._mapId] == undefined) {
            //console.log("New Map Detected, creating image data storage, name is ", $dataMapInfos[$gameMap._mapId].name)
            $KoTCMapImageCache[$gameMap._mapId] = {
                DataStorage: [String($dataMap.data)],
        ...
        if ($KoTCMapImageCache[$gameMap._mapId].DataStorage.includes(String($dataMap.data))) {
            ...
            //console.log("New Map on Old ID Detected, stored full image data, name is ", $dataMapInfos[$gameMap._mapId].name)
            var index = $KoTCMapImageCache[$gameMap._mapId].MapFullImageStorage.length;
            var chosenmapid = KoTCNewMapID();
            $KoTCMapImageCache[$gameMap._mapId].DataStorage.push(String($dataMap.data));
            $KoTCMapImageCache[$gameMap._mapId].IDStorage.push(chosenmapid);
            $KoTCMapImageCache[$gameMap._mapId].AmountExplored.push(0);
            ...
            if ($KoTCMinimapSystem.MapBookEnabled !== false) {
                ...
                if ($KoTCMapImageCache[$gameMap._mapId].MapFullImageStorage[index] == undefined) {
        ...
        var width = $KoTCMinimapSystem.CurrentMinimapSize + $KoTCMinimapSystem.CurrentMinimapBorderThickness;
        var height = $KoTCMinimapSystem.CurrentMinimapSize + $KoTCMinimapSystem.CurrentMinimapBorderThickness;
        ...
        if ($gameSystem.MapExplorationData[$KoTCMinimapSystem.CurrentMap] == undefined) {
            ...
            for (; hh < h; hh++) {
                ...
                for (; ww < w; ww++) {
                    ...
                    if (rangedifference <= 10 && $gameSystem.MapExplorationData[$KoTCMinimapSystem.CurrentMap][hh][ww] !== 2) {
                        $gameSystem.MapExplorationData[$KoTCMinimapSystem.CurrentMap][hh][ww] = 1;

                    } else {
                        $gameSystem.MapExplorationData[$KoTCMinimapSystem.CurrentMap][hh][ww] = 0;
        ...
        var minimapsize = $KoTCMinimapSystem.CurrentMinimapSize;
        var width = $dataMap.width / ($dataMap.width / (minimapsize)) + $KoTCMinimapSystem.CurrentMinimapBorderThickness;
        var height = $dataMap.height / ($dataMap.height / (minimapsize)) + $KoTCMinimapSystem.CurrentMinimapBorderThickness;
        var radius = 11 / ($dataMap.width / ($KoTCMinimapSystem.CurrentMinimapSize));
        var x = 0;
        for (; x < width; x++) {
            ...
            for (; y < height; y++) {
                ...
                if ($gameSystem.MapExplorationData[$KoTCMinimapSystem.CurrentMap][xtarget] !== undefined) {

                    if ($gameSystem.MapExplorationData[$KoTCMinimapSystem.CurrentMap][xtarget][ytarget] == 2) {
                        $KoTCMinimapSystem.FogOfWarSprite.bitmap.context.save()
                        $KoTCMinimapSystem.FogOfWarSprite.bitmap.context.beginPath()
                        $KoTCMinimapSystem.FogOfWarSprite.bitmap.context.arc(x, y, radius, 0, 2 * Math.PI, false);
                        $KoTCMinimapSystem.FogOfWarSprite.bitmap.context.clip();
...
function KoTCGetImageFromPicture(picture, picturefoldername, gridindex, xlocation, ylocation, gridwidth, gridheight, indexoftileset) {
            ...
            case 5:
                ...
                switch (true) {
                    ...
                    case gridindex > 127:
                        ...
                        var gridtargety = Math.floor(gridindex / gridxamount) * gridheight - 768;
    ...
    CanvasToPaintOn.blt(targetimagebitmap, gridtargetx, gridtargety, gridwidth, gridheight, 1, 1);
...
function ConvertRGBStorageToColor(rgbstorage) {
    var target = "rgba(" + rgbstorage[0] + ', ' + rgbstorage[1] + ', ' + rgbstorage[2] + ", 1)";
    return target;
};
...
function KoTCDrawCircle(size, color) {
    ...
    bitmaptodrawon.drawCircle(Math.round(bitmaptodrawon.width / 2), Math.round(bitmaptodrawon.height / 2), Math.round(bitmaptodrawon.width / 2), color);
...
function KoTCDrawSquare(size, color) {
    var bitmaptodrawon = new Bitmap(size, size);
    bitmaptodrawon.fillRect(Math.round(bitmaptodrawon.width / 2), Math.round(bitmaptodrawon.height / 2), Math.round(bitmaptodrawon.width / 2), Math.round(bitmaptodrawon.height / 2), color);
    var spritetoexport = new Sprite(bitmaptodrawon);
    spritetoexport.anchor.x = 0.5;
    spritetoexport.anchor.y = 0.5;
    return spritetoexport;
    ...
    for (; milliseconds > 0; milliseconds--) {
        (function (milliseconds, timeelapsed) {
            setTimeout(() => {
                ...
                sprite.scale.x = (sprite.scale.x * (milliseconds - 1) + targetscale) / milliseconds
                sprite.scale.y = (sprite.scale.y * (milliseconds - 1) + targetscale) / milliseconds
    ...
    for (; milliseconds > 0; milliseconds--) {
        (function (milliseconds, timeelapsed) {
            setTimeout(() => {
                ...
                sprite.alpha = (sprite.alpha * (milliseconds - 1) + targetalpha) / milliseconds
    ...
    this.children.forEach(function (child) {
        if (child instanceof Sprite_Character) {
            if (child._character !== null) {
                if (child._character instanceof Game_Player || child._character instanceof Game_Follower || child._character instanceof Game_Vehicle)
                    return;
            }

            child.update();

            if (child._characterName === '' && child._tileId === 0)
                return;
...
function ShrinkBitmap(bitmap, divideby) {
    ...
    for (; w < newwidth; w++) {
        var h = 0;
        for (; h < newheight; h++) {
            var targetpixel = bitmap.getPixel(Math.round(w * divideby), Math.round(h * divideby));
...
function ShrinkBitmapToMapSize(bitmap) {
    ...
    var smallcanvas = new Bitmap($KoTCMinimapSystem.CurrentMinimapSize, $KoTCMinimapSystem.CurrentMinimapSize).canvas;
    var smallContext = smallcanvas.getContext("2d");
    smallContext.scale($KoTCMinimapSystem.CurrentMinimapSize / bitmap.width, $KoTCMinimapSystem.CurrentMinimapSize / bitmap.height);
    smallContext.drawImage(bigCanvas, 0, 0);
    var smallImageData = smallContext.getImageData(0, 0, bigCanvas.width, bigCanvas.height);
    var newbitmap = new Bitmap($KoTCMinimapSystem.CurrentMinimapSize, $KoTCMinimapSystem.CurrentMinimapSize)
...
function CreateMapBookPicture(nofog) {
    var newbitmap = new Bitmap($KoTCMinimapSystem.CurrentMinimapSize, $KoTCMinimapSystem.CurrentMinimapSize);
    if ($KoTCMinimapSystem.KoTCDrawParallax !== false) {
        var parabitmap = ShrinkBitmapToMapSize($KoTCMinimapSystem.ParallaxSprite.bitmap);
    ...
    if ($KoTCMinimapSystem.ExplorationMapMode !== false && typeof nofog == 'undefined') {
        var fogofwarbitmap = $KoTCMinimapSystem.FogOfWarSprite.bitmap;
        newbitmap.blt(fogofwarbitmap, 0, 0, fogofwarbitmap.width, fogofwarbitmap.height, 0, 0)

    };
    if ($KoTCMinimapSystem.UseDefaultBorder !== true) {
        var borderbitmap = $KoTCMinimapSystem.MapBorderSprite.bitmap;
        newbitmap.blt(borderbitmap, 0, 0, borderbitmap.width, borderbitmap.height, 0, 0)
    };
    return newbitmap
};
...
function CreateMapBookPictureImageData(nofog) {
    var newbitmap = new Bitmap($KoTCMinimapSystem.CurrentMinimapSize, $KoTCMinimapSystem.CurrentMinimapSize);
    if ($KoTCMinimapSystem.ParallaxSprite !== undefined && $KoTCMinimapSystem.KoTCDrawParallax !== false && $gameMap.parallaxName() !== "") {
        var parabitmap = ShrinkBitmapToMapSize($KoTCMinimapSystem.ParallaxSprite.bitmap);
        newbitmap.blt(parabitmap, 0, 0, parabitmap.width, parabitmap.height, 0, 0)
    };
    switch ($KoTCMinimapSystem.MinimapMode) {
    ...
    var spritearraylength = $KoTCMinimapSystem.ArrayofMinimapPictureSprites.length;
    for (; b < spritearraylength; b++) {
        if ($KoTCMinimapSystem.ArrayofMinimapPictureSprites[b] && $gameMap.events()[b] !== null && bookregcode.exec($gameMap.events()[b].event().note) !== null) {
            ...
            newbitmap.blt(spritebitmap, 0, 0, spritebitmap.width, spritebitmap.height, xtodraw, ytodraw, width, height)
        };
    };
    if ($KoTCMinimapSystem.ExplorationMapMode !== false && typeof nofog == 'undefined') {
        var fogofwarbitmap = $KoTCMinimapSystem.FogOfWarSprite.bitmap;
    ...
    return newbitmap.context.getImageData(0, 0, $KoTCMinimapSystem.CurrentMinimapSize, $KoTCMinimapSystem.CurrentMinimapSize)
};

function KoTCExportMapImages() {
    if (Utils.RPGMAKER_NAME == "MZ") {

        MVNodeFS.writeFile("data/", "MapImageData" + $gameSystem.savefileId() + ".json", JSON.stringify($KoTCMapImageCache));

    } else {
        MVNodeFS.writeFile("data/", "MapImageData" + DataManager.lastAccessedSavefileId() + ".json", JSON.stringify($KoTCMapImageCache));
    }
};
...
function KoTCImportMapImages() {
    ...
    if (MVNodeFS.checkFile("data/", "MapImageData" + savefileId + ".json")) {
        $KoTCMapImageCache = JSON.parse(MVNodeFS.readFile("data/", "MapImageData" + savefileId + ".json"));

        var a = 1;
        var length1 = $KoTCMapImageCache.length;
        for (; a < length1; a++) {
            var b = 0;
            if ($KoTCMapImageCache[a] !== null && $KoTCMapImageCache[a] !== undefined) {
                var length2 = $KoTCMapImageCache[a].MapFullImageStorage.length;
                if ($KoTCMapImageCache[a].MapFullImageStorage[b] !== undefined && $KoTCMapImageCache[a].MapFullImageStorage[b] !== null) {
                    for (; b < length2; b++) {
                        var datatemp1 = Object.values($KoTCMapImageCache[a].MapFullImageStorage[b].data);
                        var datatemp2 = Object.values($KoTCMapImageCache[a].ImageDataStorage[b].data);
                        $KoTCMapImageCache[a].MapFullImageStorage[b] = new ImageData(new Uint8ClampedArray(datatemp1), $KoTCMinimapSystem.CurrentMinimapSize, $KoTCMinimapSystem.CurrentMinimapSize);
                        $KoTCMapImageCache[a].ImageDataStorage[b] = new ImageData(new Uint8ClampedArray(datatemp2), $KoTCMinimapSystem.CurrentMinimapSize, $KoTCMinimapSystem.CurrentMinimapSize);
                    }
...
function KoTCCountExploredArea(mapindex) {
    if ($dataMap) {
        ...
        for (; a < width; a++) {
            ...
            for (; b < height; b++) {
                if ($gameMap.checkPassage(a, b, 0x0F) !== false) {
                    if ($gameSystem.MapExplorationData[$KoTCMinimapSystem.CurrentMap][b][a] == 1 || $gameSystem.MapExplorationData[$KoTCMinimapSystem.CurrentMap][b][a] == 2) {
...
if (Utils.RPGMAKER_NAME == "MZ") {
    ...
    Window_KoTCMapIDIndex.prototype.initialize = function (rect) {
        ...
        Window_Selectable.prototype.initialize.call(this, new Rectangle(rect.x, rect.y, width, height));
        this.refresh();
...
Window_KoTCMapIDIndex.prototype.refresh = function () {
    ...
    for (; i < length; i++) {
        ...
        if (currentmap !== undefined && currentmap !== null && $KoTCMapImageCache[0][currentmap.IDStorage[0]] !== undefined) {
...
if (Utils.RPGMAKER_NAME == "MZ") {
    ...
    Window_KoTCMapBook.prototype.initialize = function (rect) {
        ...
        var borderwidth = $KoTCMinimapSystem.CurrentMinimapSize + ($KoTCMinimapSystem.CurrentMinimapBorderThickness * 4);
        var borderheight = $KoTCMinimapSystem.CurrentMinimapSize + ($KoTCMinimapSystem.CurrentMinimapBorderThickness * 4);
        var borderx = rect.width / 1.22 - $KoTCMinimapSystem.CurrentMinimapBorderThickness;
        var bordery = rect.height / 2 - $KoTCMinimapSystem.CurrentMinimapBorderThickness;
        this._mapwindowsprite = new KoTCMinimapWindow(new Rectangle(borderx, bordery, borderwidth, borderheight));
    ...
    Window_KoTCMapBook.prototype.initialize = function (x, y, width, height) {
        ...
        var borderwidth = $KoTCMinimapSystem.CurrentMinimapSize + ($KoTCMinimapSystem.CurrentMinimapBorderThickness * 4);
        var borderheight = $KoTCMinimapSystem.CurrentMinimapSize + ($KoTCMinimapSystem.CurrentMinimapBorderThickness * 4);
        var borderx = width / 1.22 - $KoTCMinimapSystem.CurrentMinimapBorderThickness;
        var bordery = height / 2 - $KoTCMinimapSystem.CurrentMinimapBorderThickness;
        this._mapwindowsprite = new KoTCMinimapWindow(borderx, bordery, borderwidth, borderheight);
        this.addChildToBack(this._mapwindowsprite)
...
Window_KoTCMapBook.prototype.update = function () {
    ...
    if (this._kotcMapSprite.bitmap) {
        ...
        this._mapwindowsprite.x = this._kotcMapSprite.x - (this._mapwindowsprite.width / 2 * this._mapwindowsprite.scale.x)
        this._mapwindowsprite.y = this._kotcMapSprite.y - (this._mapwindowsprite.height / 2 * this._mapwindowsprite.scale.y)
    }
}
...
Window_KoTCMapBook.prototype.refresh = function () {
    ...
    if (currentmap !== null) {
        ...
        var bitmap;
        var bitmap = new Bitmap($KoTCMinimapSystem.CurrentMinimapSize, $KoTCMinimapSystem.CurrentMinimapSize);
        if ($KoTCMinimapSystem.ExplorationMapMode !== true) {
            bitmap.context.putImageData($KoTCMapImageCache[currentmapid].MapFullImageStorage[currentmapindex], 0, 0)

        } else {
            bitmap.context.putImageData($KoTCMapImageCache[currentmapid].ImageDataStorage[currentmapindex], 0, 0)
        ...
        for (var i = 0; i < 1; i++) { // i was 8
            var tempamountexplored = $KoTCMapImageCache[currentmapid].AmountExplored[currentmapindex];
            var tempamounttoexplore = $KoTCMapImageCache[currentmapid].AmountToExplore[currentmapindex];
            if ($KoTCMinimapSystem.ExplorationMapMode !== false) {
                this.drawTextEx("\\C[17]" + Math.round(tempamountexplored / tempamounttoexplore * 100) + "% Explored", x, y, 160);
        ...
        var mapnoteregex1 = new RegExp(/<KoTC Map Description 1: ([^ \r\n][^!?\.\r\n]+[\w!?\.]+)>/i);
        var mapnoteregex2 = new RegExp(/<KoTC Map Description 2: ([^ \r\n][^!?\.\r\n]+[\w!?\.]+)>/i);
        var mapnoteregex3 = new RegExp(/<KoTC Map Description 3: ([^ \r\n][^!?\.\r\n]+[\w!?\.]+)>/i);
        var mapnoteregex4 = new RegExp(/<KoTC Map Description 4: ([^ \r\n][^!?\.\r\n]+[\w!?\.]+)>/i);
