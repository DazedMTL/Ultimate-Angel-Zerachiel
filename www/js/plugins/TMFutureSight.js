...
(function () {
	...
	const params = {
		fontSizeMsg: parameters.fontSizeMsg ? JSON.parse(parameters.fontSizeMsg) : 15,
		fontSizeShield: +(parameters.fontSizeShield || 28),
		fontSizeBoost: +(parameters.fontSizeBoost || 28),
		fontSizeHp: parameters.fontSizeHp ? JSON.parse(parameters.fontSizeHp) : [28, 28],
		textStateTurn: parameters.textStateTurn ? JSON.parse(parameters.textStateTurn) : [15, 5, 20],
		posFrame: parameters.posFrame ? JSON.parse(parameters.posFrame) : [0, -75, 200, 100],
		posMsgbox: parameters.posMsgbox ? JSON.parse(parameters.posMsgbox) : [0, -300],
		posMsg: parameters.posMsg ? JSON.parse(parameters.posMsg) : [20, 0],
		buffColors: parameters.buffColors ? JSON.parse(parameters.buffColors) : ["#0000ff", "#ff0000"],
	};
	// 事前ロード
	...
	// Game_BattlerBase
	//
	const Game_BattlerBase_prototype_addNewState = Game_BattlerBase.prototype.addNewState;
	...
	const Game_BattlerBase_prototype_updateStateTurns = Game_BattlerBase.prototype.updateStateTurns;
	...
	// Game_Action
	//
	const Game_Action_prototype_applyItemUserEffect = Game_Action.prototype.applyItemUserEffect;
			...
			if (this._actions[i]) {
				...
				if (skill) {
					if (skill.meta.fsText) {
						...
						// }
						const v1 = skill.meta.formula1 ? Nnfs.evalDamageFormulaEx(this, skill, skill.meta.formula1) : 0;
						const v2 = skill.meta.formula2 ? Nnfs.evalDamageFormulaEx(this, skill, skill.meta.formula2) : 0;
						const text = skill.meta.fsText.replace("\\skill", skill.name).replace("\\icon", "\\I[" + skill.iconIndex + "]");
						...
						this._futureSightTexts.push(Nnfs.replaceFormulaTag(text, this._actions[i], $gameParty.members()[0], v1, v2));
						this._variances = tmp;
					}
					else {
						//\{\icon\}\skill
						this._futureSightTexts.push("\\{\\I[" + skill.iconIndex + "]\\}" + skill.name);
					}
				}
			}
		}
	...
	Sprite_Battler.prototype.updateFutureSight = function () {
		if (!this._futureSightSprite && this.parent /*&& this.mainSprite().bitmap.isReady() spine対応でコメント、必要だったのかこれは*/) {
	...
	Sprite_Actor.prototype.mainSprite = function () { return this._mainSprite; }
	Sprite_Enemy.prototype.mainSprite = function () { return this; }
	...
	Sprite_FutureSight.prototype.initialize = function (battlerSprite) {
		...
		this._active = true;
		// シールド
		this._shield = new Sprite(ImageManager.loadPicture('card_system/status_shield'));
		this._shield.visible = false;
		this._shield.anchor.set(0.5);
		this.addChild(this._shield);
		// frame
		this._frame = new Sprite(ImageManager.loadPicture('card_system/status_frame1'));
		...
		this._text.bitmap = new Bitmap(params.posFrame[2], params.posFrame[3] || 60);
		...
		if (battlerSprite._battler.isEnemy()) {
			...
			if (enemy.meta.fsPosX)
				this.x = +enemy.meta.fsPosX;
			if (enemy.meta.fsPosY)
				this.y = +enemy.meta.fsPosY;
		...
		if (this._battlerSprite._battler._initedFutureSightAtLeastOnce) {
			...
			this.drawGaugeHp(this.bitmap, this._hpTemps[2] / this._hpTemps[3], this._hpTemps[0]);
	...
	Sprite_FutureSight.prototype.resetShield = function (value) {
		this._shieldTemps[0] = this._battlerSprite._battler._shieldValue = value || 0;
		this._hpTemps[1] = false;
		this.drawShield(this._text.bitmap, this._shieldTemps[0], params.fontSizeShield, true);
	}

	Sprite_FutureSight.prototype.drawSightText = function (buffAttack, buffShield) {
		ImageManager.loadPicture('card_system/msgbox_enemy').addLoadListener(function (tex) {
			const w = tex.width;
			const h = tex.height;
			...
			let x = params.posMsg[0], y = params.posMsg[1];
	...
	Sprite_FutureSight.prototype.drawHp = function (bitmap, hp, mhp) {
		...
		bitmap1.addLoadListener(function () {
			bitmap2.addLoadListener(function () {
				...
				let w = Nnfs.drawDigit(bitmap, bitmap2, hp, x, y, params.fontSizeHp[0], 0, 1);
				x += w;
	...
	Sprite_FutureSight.prototype.drawShield = function (bitmap, shield, fontSize, clear) {
		const x = 22;
		const y = 70;
		if (clear) {
		...
		ImageManager.loadPicture('card_system/status_n1').addLoadListener(function (tex) {
			Nnfs.drawDigit(bitmap, tex, shield, x, y, fontSize);
		}.bind(this));
	};

	Sprite_FutureSight.prototype.drawBoost = function (bitmap, battler) {
		if (this._boostValue == battler._boostValue)
			return;
		this._boostValue = battler._boostValue;
		if (0 < this._boostValue) {
			const x = 45, y = 65;
			bitmap.clearRect(x, y, Window_Base._iconWidth + 2, Window_Base._iconHeight + 2);
			this.drawSightIcon(bitmap, 96, x, y);
			ImageManager.loadPicture('card_system/status_n1').addLoadListener(function (tex) {
				Nnfs.drawDigit(bitmap, tex, this._boostValue, x + 18, y + 35, params.fontSizeBoost);
			}.bind(this));
		}
	};

	Sprite_FutureSight.prototype.drawStates = function (bitmap, battler, force) {
		if (!force && !battler._requestStateUpdate)
			return;
		...
		battler._states.forEach(id => {
			...
			if (data.autoRemovalTiming != 0) {
				bitmap.drawText(battler._stateTurns[id], x + params.textStateTurn[0], y + params.textStateTurn[1], 16, 36, 'left');
			}
	...
	Sprite_FutureSight.prototype.drawSightIcon = function (bitmap, iconIndex, x, y) {
		...
		bitmap.bltImage(ImageManager.loadSystem('IconSet'), sx, sy, pw, ph, x, y, n, n);
	};
	...
	Sprite_FutureSight.prototype.update = function () {
		...
		if (this._shieldTemps[0] !== battler._shieldValue) {
			...
			if (0 < sign && this._shieldTemps[0] !== battler._shieldValue)
				this.drawShield(this._text.bitmap, this._shieldTemps[0], params.fontSizeShield + 5, true);
			else {
				this.drawShield(this._text.bitmap, this._shieldTemps[0], params.fontSizeShield, true);
				if (this._shieldTemps[0] === 0) {
					this._hpTemps[1] = false;
					if (sign < 0) {
						Nnfs.ParticleContainer.makeParticle('ShieldBreak1', this, true, true, true, true, true)
							.play();
						Nnfs.ParticleContainer.makeParticle('ShieldBreak2', this, true, true, true, true, true)
							.play();
					}
				}
			}
		...
		// }
		// ライフゲージ
		if (this._hpTemps[0] !== this._hpTemps[1] || this._hpTemps[2] !== battler.hp || this._hpTemps[3] !== battler.mhp) {
			this._hpTemps[3] = battler.mhp;
			this._hpTemps[0] = this._hpTemps[1]
			...
			if (1 <= (this._hpTemps[4] += this.digitSpeed(diff, 2))) {
				this._hpTemps[4] = 0;
				this._hpTemps[2] += sign * (80 < Math.abs(diff) ? Math.floor(Math.abs(diff) * 0.05) : 1);
			}
			this.drawGaugeHp(this.bitmap, this._hpTemps[2] / this._hpTemps[3], this._hpTemps[0]);
			this.drawHp(this._text.bitmap, this._hpTemps[2], this._hpTemps[3]);
			if (this._hpTemps[0])
				this._frame.setColorTone([0, 0, 100, 0]);
			else
				this._frame.setColorTone([0, 0, 0, 0]);
		...
		if (battler.isActor()) {
			...
			if (this._trunkTemps[0] !== trunk) {
				...
				// }
				this.drawGaugeTrunk(this.bitmap, this._trunkTemps[0] / battler.trunkMax());
			}
		}
		...
		if (is_enemy && battler._requestFutureSightUpdate) {
			...
			this.drawSightText(battler._futureSightAttackBuff, battler._futureSightShieldBuff);
			// }
