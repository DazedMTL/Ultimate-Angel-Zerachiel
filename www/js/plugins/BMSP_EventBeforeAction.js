...
(function () {
  ...
  BMSP.EventBeforeAction.initEvents = function () {
    for (var i = 1; i < $dataCommonEvents.length; i++) {
      ...
      var match = list[commandIndex].parameters[commandIndex].match(
        /^==行動前イベント:(使用前|使用後)==$/
      );
      if (!match) continue;
      var type = match[1];
  ...
  BMSP.EventBeforeAction.meetsConditions = function (
    conditions,
    action,
    subject,
    targets
  ...
  BMSP.EventBeforeAction.getEventsMetConditions = function (
    eventDataList,
    action,
    subject,
    targets
  ...
  BMSP.EventBeforeAction.getBeforeUseSkillEvents = function (action, subject) {
    var eventDataList = this.getEventsMetConditions(
      this._beforeUseSkillEvents,
      action,
      subject,
      []
    );
    return eventDataList.map(function (eventData) {
      return eventData["id"];
    });
  };

  BMSP.EventBeforeAction.getBeforeInvokeActionEvents = function (
    action,
    subject,
    targets
  ) {
  ...
  BattleManager.processTurn = function () {
    ...
    if (action) {
      ...
      if (action.isValid()) {
        if (
          !(
            BMSP.getObjectId(action) in
            BMSP.EventBeforeAction.flagExecutedEventBeforeUseSkill
          )
        ) {
          BMSP.EventBeforeAction.flagExecutedEventBeforeUseSkill[
            BMSP.getObjectId(action)
          ] = true;
          this.startEventBeforeUseSkill(action, subject);
          return;
        } else {
          delete BMSP.EventBeforeAction.flagExecutedEventBeforeUseSkill[
            BMSP.getObjectId(action)
          ];
        }
      }
  ...
  BattleManager.forceAction = function (battler) {
    ...
    if (BMSP.EventBeforeAction.isEventBeforeActionPhase(this._phase)) {
      BMSP.EventBeforeAction.stackForcedBatterBeforeSetBattler.push(
  ...
  BattleManager.processForcedActionBeforeAction = function () {
    ...
    this._actionForcedBattler =
      BMSP.EventBeforeAction.stackForcedBatterBeforeSetBattler.pop();
  };
  ...
  BattleManager.endAction = function () {
    ...
    switch (phase) {
      ...
      case "eventBeforeForcedInvokeAction":
        ...
        var included_stack = BMSP.EventBeforeAction.stackForceActions.some(
          function (info) {
            return (
              BMSP.getObjectId(info["subject"]) ==
              BMSP.getObjectId(this._subject)
            );
          },
          this
        );
  ...
  BattleManager.updateEventBeforeUseSkill = function () {
    var event_ids =
      BMSP.EventBeforeAction.stackEventList[
        BMSP.EventBeforeAction.stackEventList.length - 1
      ];
  ...
  BattleManager.startEventBeforeForcedUseSkill = function (action, subject) {
    ...
    BMSP.EventBeforeAction.stackForcedBatterBeforeStartEvent.push(
      this._actionForcedBattler
    );
  ...
  BattleManager.endEventBeforeForcedUseSkill = function () {
    ...
    this._actionForcedBattler =
      BMSP.EventBeforeAction.stackForcedBatterBeforeStartEvent.pop();
  ...
  BattleManager.startEventBeforeInvokeAction = function (
    action,
    subject,
    targets
  ) {
    this._phase = "eventBeforeInvokeAction";
    event_ids = BMSP.EventBeforeAction.getBeforeInvokeActionEvents(
      action,
      subject,
      targets
    );
  ...
  BattleManager.startEventBeforeForcedInvokeAction = function (
    action,
    subject,
    targets
  ) {
