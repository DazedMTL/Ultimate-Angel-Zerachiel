...
__BravelyTitleScreen.parameters =
  PluginManager.parameters("BravelyTitleScreen");

/*
 *   Parameters
 */

__BravelyTitleScreen.GameTitleText = String(
  __BravelyTitleScreen.parameters["Game Title"] || "#h Something, #h Went Wrong"
).split(",");
__BravelyTitleScreen.GameTitleText = __BravelyTitleScreen.GameTitleText.map(
  (ele) => {
    return ele.trim();
  }
);
__BravelyTitleScreen.ScrollingBackSprite = String(
  __BravelyTitleScreen.parameters["Background Image"]
).split("/");
__BravelyTitleScreen.ScrollingBackSpriteDirectory = String(
  "img/" + __BravelyTitleScreen.ScrollingBackSprite[0] + "/" ||
    "img/battlebacks1/"
);
__BravelyTitleScreen.ScrollingBackSpriteImg = String(
  __BravelyTitleScreen.ScrollingBackSprite[1] || "DarkSpace"
);
__BravelyTitleScreen.BackSpriteColor = String(
  "0x" + __BravelyTitleScreen.parameters["Background Color"] || "0xffffff"
);
__BravelyTitleScreen.TransitionDuration = Number(
  __BravelyTitleScreen.parameters["Transition Duration"] || 60
);
__BravelyTitleScreen.TitleFontFace = String(
  __BravelyTitleScreen.parameters["Font Face"] || "GameFont"
);
__BravelyTitleScreen.GameTitleTextSize = Number(
  __BravelyTitleScreen.parameters["Headline Fontsize"] || 192
);
__BravelyTitleScreen.GameSubTextSize = Number(
  __BravelyTitleScreen.parameters["Subline Fontsize"] || 52
);
__BravelyTitleScreen.GameTitleOutline = eval(
  __BravelyTitleScreen.parameters["Use Outline"]
);
...
__BravelyTitleScreen.IsImageTitle = eval(
  __BravelyTitleScreen.parameters["Use Image as Title"] || false
);
__BravelyTitleScreen.ImageTitle = String(
  __BravelyTitleScreen.parameters["Title Image Location"]
).split("/");
__BravelyTitleScreen.ImageTitleDirectory = String(
  "img/" + __BravelyTitleScreen.ImageTitle[0] + "/"
);
__BravelyTitleScreen.ImageTitleFilename = String(
  __BravelyTitleScreen.ImageTitle[1]
);
__BravelyTitleScreen.ImageTitleScale = eval(
  __BravelyTitleScreen.parameters["Scale Title Image"] || true
);
__BravelyTitleScreen.ScrollBackground = eval(
  __BravelyTitleScreen.parameters["Sprite Scrolling"] || true
);
__BravelyTitleScreen.ScrollingDirection = Number(
  __BravelyTitleScreen.parameters["Direction"] || 0
);
__BravelyTitleScreen.ScrollingSpeed =
  Number(__BravelyTitleScreen.parameters["Speed"] || 100) / 100;
__BravelyTitleScreen.ScrollingSwingDuration = Number(
  __BravelyTitleScreen.parameters["Swing Duration"] || 240
);
__BravelyTitleScreen.ScrollingSwingDistance = Number(
  __BravelyTitleScreen.parameters["Swing Distance"] || 200
);
__BravelyTitleScreen.UseNativeTilingSprite = eval(
  __BravelyTitleScreen.parameters["Native TilingSprite"] || false
);
...
class Scrolling_BackSprite extends PIXI.extras.TilingSprite {
  ...
  updateScrolling() {
    if (this._motionDirection < 0 || this._motionDirection > 0) {
      this.tilePosition.x +=
        0.5 * __BravelyTitleScreen.ScrollingSpeed * this._motionDirection;
    } else if (this._motionDirection === 0) {
      this._motionCount >= 4 ? (this._motionCount = 0) : null;
      this.tilePosition.x =
        __BravelyTitleScreen.ScrollingSwingDistance *
        Math.sin((Math.PI / 2) * this._motionCount);
      this._motionCount +=
        2 / (__BravelyTitleScreen.ScrollingSwingDuration * 2);
    }
  }
}
...
Scene_Title.prototype.start = function () {
  __BravelyTitleScreen.IsImageTitle
    ? this.createTitleMaskSprite()
    : this.createTitleMask();
...
Scene_Title.prototype.update = function () {
  ...
  if (
    !this.isBusy() &&
    this._commandWindow.isOpen() == false &&
  ...
  if (
    !this.isBusy() &&
    this._commandWindow.isOpen() == true &&
...
Scene_Title.prototype.createBackground = function () {
  ...
  this._backSpriteBlank = new Blank_BackSprite(
    __BravelyTitleScreen.BackSpriteColor
  );
...
Scene_Title.prototype.createGameTitleSprite = function () {
  ...
  this._gameTitleSprite = new Sprite(
    ImageManager.loadBitmap(dir, img, 0, false)
  );
  this._gameTitleSprite2 = new Sprite(
    ImageManager.loadBitmap(dir, img, 0, false)
  );
};
...
Scene_Title.prototype.drawGameTitleSub = function (sprite, text, font) {
  ...
  let y =
    Graphics.height / 4 +
    this._gameTitleHeadSprites.length * __BravelyTitleScreen.GameTitleTextSize -
    __BravelyTitleScreen.GameSubTextSize * 0.5 +
    __BravelyTitleScreen.GameSubTextSize * index;
  let maxWidth = Graphics.width - x * 2;
  Scene_Title.prototype.drawGameTitle(sprite, text, x, y, maxWidth, font);
};
...
Scene_Title.prototype.updateTransition = function () {
  ...
  if (this.isTransitioning()) {
    ...
    this._backSpriteScrolling.mask.alpha =
      1 * Math.sin((Math.PI / 2) * (count / duration));
    if (this._backSpriteScrolling.mask.alpha <= 0.5) {
      let third = duration / 3;
      this._backSpriteBlank.mask.alpha =
        1 - Math.sin((Math.PI / 2) * (count / third));
    }
...
if (Imported.YEP_CoreEngine) {
  Scene_Title.prototype.rescaleTitle = function () {
    if (
      __BravelyTitleScreen.IsImageTitle &&
      __BravelyTitleScreen.ImageTitleScale
    ) {
