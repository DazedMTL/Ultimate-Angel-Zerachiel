...
(function () {
  if (!Graphics.hasWebGL()) {
    console.error(
      "WebGL をサポートしていないため、フィルターを使用できません。"
    );
    return;
  }
  ...
  KMS_WaterEffect = function () {
    ...
    if (Param.autoTone) {
      if (PixiVersion == 2) {
        this._colorFilter.matrix = [
          0.9, 0, 0, 0, 0, 0.9, 0, 0, 0.2, 0.2, 1, 0, 0, 0, 0, 1,
        ];
      } else {
        // For Pixi v4
        this._colorFilter.matrix = [
          0.9, 0, 0, 0, 0, 0, 0.9, 0, 0, 0, 0.2, 0.2, 1, 0, 0, 0, 0, 0, 1, 0,
        ];
      }
    }
    ...
    var margin = 24;
    this._waveSprite.move(
      -margin,
      -margin,
      Graphics.width + margin,
  ...
  KMS_WaterEffect.prototype.loadImage = function () {
    this._dispBitmap = ImageManager.loadBitmap(
      Param.imageDir,
      Param.dispImage,
      0
    );
    this._waveBitmap = ImageManager.loadBitmap(
      Param.imageDir,
      Param.waveImage,
      0
    );
   ...
   * ロード完了後のエフェクト適用処理本体
   */
  Spriteset_Base.prototype.applyWaterMapEffectBody = function (
    target,
    waveContainer
  ) {
  ...
  var _KMS_WaterMapEffect_Spriteset_Base_update =
  ...
  var _KMS_WaterMapEffect_Scene_Map_start = Scene_Map.prototype.start;
  ...
  var _KMS_WaterMapEffect_Scene_Battle_start = Scene_Battle.prototype.start;
