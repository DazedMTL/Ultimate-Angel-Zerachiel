...
(function () {
  ...
  var tpBarTypeRight =
    parameters["TP Bar Shape"].length === 2
      ...
      : false;
  var expBarTypeLeft =
    parameters["EXP Bar Shape"].length === 2
      ? String(parameters["EXP Bar Shape"]).substring(0, 1)
      : false;
  var expBarTypeRight =
    parameters["EXP Bar Shape"].length === 2
      ? String(parameters["EXP Bar Shape"]).substring(1, 2)
      : false;
  ...
  var stopAnimatingOnActorStatus =
    (parameters["Don't Animate In Status"] || "false") === "true";
  var stopAnimatingOnActorMenuStatus =
    (parameters["Don't Animate In Menu Status"] || "false") === "true";
  var stopAnimatingOnActorTarget =
    (parameters["Don't Animate In Actor Targeting"] || "true") === "true";
  ...
  var statusXY = (parameters["Status Icon XY Offset"] || "0")
    .split(", ")
    .map(Number);
  ...
  // Sleek Gauges
  //=============================================================================
  Window_Base.prototype.drawStaticGauge = function (
    x,
    y,
    width,
    rate,
  ...
  Window_Base.prototype.drawAnimatedGauge = function (
    x,
    y,
    width,
    rate,
    ...
    if (this._gauges[gkey]) {
      ...
      this._gauges[gkey] = new Special_Gauge(
        x,
        y,
        width,
        rate,
  ...
  Window_MenuStatus.prototype.drawActorSimpleStatus = function (
    ...
    if (statusXY.length === 2) {
      ...
      this.drawActorIcons(
        actor,
        x + statusXY[0],
        y + lineHeight * 2 + statusXY[1]
      );
      this.drawActorClass(actor, x2, y);
      this.drawActorHp(actor, x2, y + lineHeight * 1, width2);
      this.drawActorMp(actor, x2, y + lineHeight * 2, width2);
    } else {
      Window_Base.prototype.drawActorSimpleStatus.call(
        this,
        actor,
        x,
        y,
    ...
    if (Imported.YEP_CoreEngine) {
      ...
      this.drawActorEXP(
        ...
        x,
        y + this.lineHeight() * 2,
        Math.min(200, width - 200 - this.textPadding())
      );
    }
  };
  ...
  Special_Gauge.prototype.update = function () {
    ...
    if (Math.abs(this._curVal - this._setVal) < this._speed)
  ...
  Special_Gauge.prototype.shouldntAnimate = function () {
    if (
      stopAnimatingOnActorTarget &&
      this._window instanceof Window_BattleActor
    )
      return true;
    if (stopAnimatingOnActorStatus && this._window instanceof Window_Status)
      return true;
    if (
      stopAnimatingOnActorMenuStatus &&
      this._window instanceof Window_MenuStatus
    )
      return true;
    return false;
  };
  ...
  // Pretty Gauges
  //=============================================================================
  Bitmap.prototype.drawTrap = function (
    x,
    y,
    width,
    height,
  ...
  if (showEnemyHP) {
    ...
    Window_EnemyHPBars.prototype.clearGauges = function (actor, x, y, width) {
      ...
      if (
        this._gauges[
          this.makeTGaugeKey(x + tinyGaugeXOffset, y + 1 + tinyGaugeYOffset)
        ]
      )
        height += defaultTinyHeight + 1;
      if (
        this._gauges[
          this.makeTGaugeKey(
            x + tinyGaugeXOffset,
            y + 1 + tinyGaugeYOffset + defaultTinyHeight + 2
          )
        ]
      )
        height += defaultTinyHeight + 2;
    ...
    Window_EnemyHPBars.prototype.drawActorHp = function (actor, x, y, width) {
      ...
      this._gauges[this.makeGaugeKey(x, y)].setTextVisibility(
        showEHPHP,
        showEHPText
      );
      this._gauges[this.makeGaugeKey(x, y)].setExtra(
    ...
    Window_EnemyHPBars.prototype.drawActorMp = function (actor, x, y, width) {
      this.drawTinyGauge(
        x + tinyGaugeXOffset,
        y + 1 + tinyGaugeYOffset,
        width + tinyWidthAdjust,
        actor.mmp !== 0 ? actor.mpRate() : 0,
        this.mpGaugeColor1(),
        this.mpGaugeColor2(),
        "mp"
      );
      this._gauges[
        this.makeTGaugeKey(x + tinyGaugeXOffset, y + 1 + tinyGaugeYOffset)
      ].setExtra(TextManager.mpA, actor.mp, actor.mmp);
    };
    ...
    Window_EnemyHPBars.prototype.drawActorTp = function (actor, x, y, width) {
      ...
      this._gauges[
        this.makeTGaugeKey(x + tinyGaugeXOffset, y + 1 + tinyGaugeYOffset)
      ].setExtra(TextManager.tpA, actor.tp, actor.maxTp());
    };

    Window_EnemyHPBars.prototype.drawEnemyGauges = function (
      actor,
      x,
      y,
      width
      ...
      if (
        this._gauges &&
        this._gauges[this.makeGaugeKey(x, y)] &&
        (this._gauges[this.makeGaugeKey(x, y)]._curVal === 0 ||
          this._gauges[this.makeGaugeKey(x, y)]._curRate === 0) &&
        actor.hp === 0
      )
        return;

      barTypeLeft =
        actor.enemy().meta.BarTypeLeft || hpBarTypeLeft || barTypeLeft;
      barTypeRight =
        actor.enemy().meta.BarTypeRight || hpBarTypeRight || barTypeRight;
      ...
      if (
        shouldDrawEnemyMP &&
        (drawEnemyMPWhenNoMP || actor.mmp > 0) &&
        !actor.enemy().meta.HideEnemyMPBar
      ) {
      ...
      if (
        (shouldDrawEnemyTP && !actor.enemy().meta.HideEnemyTPBar) ||
        (!shouldDrawEnemyTP && actor.enemy().meta.ShowEnemyTPBar)
      ) {
    ...
    Window_EnemyHPBars.prototype.drawTinyGauge = function (
      x,
      y,
      width,
      rate,
      ...
      if (this._gauges[gkey]) {
        ...
        this._gauges[gkey] = new Line_Gauge(
          x,
          y,
          width,
          rate,
    ...
    Window_EnemyHPBars.prototype.update = function () {
      ...
      for (var i = 0; i < enemies.length; i++) {
        ...
        if (!this._enemySprites[i].height || !this._enemySprites[i].width)
          continue;

        x =
          enemies[i].screenX() -
          (width + this.textPadding()) / 2 +
          EHPXOffset +
          (parseInt(enemies[i].enemy().meta.HPBarXOffset) || 0);
        y =
          enemies[i].screenY() -
          this.lineHeight() +
          EHPYOffset +
          (parseInt(enemies[i].enemy().meta.HPBarYOffset) || 0);
    ...
    Line_Gauge.prototype.drawGauge = function () {
      var fill_w = Math.round(
        (this._width - 2 - defaultHeight) * this._curRate
      );
      var fill_ww = Math.round((this._width - 2) * this._maxRate);
      var gy = this._y + this._window.lineHeight() - this._height;

      this._window.contents.fillRect(
        this._x - 1,
        gy,
        this._width - defaultHeight,
        this._height + 2,
        gaugeOutColor
      );
      this._window.contents.fillRect(
        this._x,
        gy + 1,
        this._width - 2 - defaultHeight,
        this._height,
        this._window.gaugeBackColor()
      );
      this._window.contents.gradientFillRect(
        this._x,
        gy + 1,
        fill_w,
        this._height,
    ...
    if (showOver) {
      var alias_Scene_Battle_createSpriteset =
        Scene_Battle.prototype.createSpriteset;
      Scene_Battle.prototype.createSpriteset = function () {
        alias_Scene_Battle_createSpriteset.call(this);

        this._enemyHPBarWindow = new Window_EnemyHPBars(
          this._spriteset._enemySprites
        );
        this.addChild(this._enemyHPBarWindow);
      };
    } else {
      var alias_Spriteset_Battle_createEnemies =
    ...
    Sprite_Enemy.prototype.updateStateSprite = function () {
      this._stateIconSprite.y = -Math.round(
        (this.bitmap.height + EHPStateYOffset) * 0.9
      );
      if (this._stateIconSprite.y < 20 - this.y) {
        this._stateIconSprite.y = 20 - this.y;
      }
      this._stateIconSprite.y +=
        parseInt(this._enemy.enemy().meta.HPBarYOffset) || 0;
      this._stateIconSprite.x =
        EHPStateXOffset +
        (parseInt(this._enemy.enemy().meta.HPBarXOffset) || 0);
    };
  } // End Enemy HP bars
})();
