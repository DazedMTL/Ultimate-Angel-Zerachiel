...
(function () {
  ...
  Game_Screen.prototype.update = function () {
    ...
    if (this._rechildsBattlers) {
      this._rechildsBattlers--;
    }
    if (this._rechildsAnimation) {
      this._rechildsAnimation--;
    }
    if (this._rechildsDamage) {
      this._rechildsDamage--;
    }
  };
  ...
  Scene_Map.prototype.update = function () {
    ...
    if ($gamePlayer._followKeke) {
      $gamePlayer.followerFollowTop();
    }
  ...
  Game_CharacterBase.prototype.convertMapX = function (x) {
    ...
    if (
      ...
      mapX <
        $gameMap._displayX - ($gameMap.width() - $gameMap.screenTileX()) / 2
    ) {
  ...
  Game_CharacterBase.prototype.convertMapY = function (y) {
    var mapY =
      (y + this.jumpHeight() + this.shiftY() - $gameMap.tileHeight()) /
      $gameMap.tileHeight();
    if (
      $gameMap.isLoopVertical() &&
      mapY <
        $gameMap._displayY - ($gameMap.height() - $gameMap.screenTileY()) / 2
    ) {
  ...
  Game_CharacterBase.prototype.setTileImage = function (
    characterName,
    characterIndex
  ) {
  ...
  Game_CharacterBase.prototype.getPreParameters = function () {
    ...
    if (this._vectorX.length > 10) {
      this._vectorX.pop();
    }
    if (this._vectorY.length > 10) {
      this._vectorY.pop();
    }
    this._preX = this._X;
    this._preY = this._Y;
  ...
  Sprite_Battler.prototype.rechild = function () {
    if (!$gameScreen._rechildsBattlers) {
      return;
    }
    if (!this.parent) {
      return;
  ...
  Sprite_Animation.prototype.rechild = function () {
    if (!$gameScreen._rechildsAnimation) {
      return;
    }
    if (this._back) {
      return;
  ...
  // 先頭追尾の処理
  Game_CharacterBase.prototype.processFollowTop = function (
    x,
    y,
    i,
    space,
    ...
    if (topX < preTopX) {
      ...
      speedX =
        offsetX < 0
          ? Math.max(offsetX, -oriSpeedX)
          : Math.min(offsetX, oriSpeedX);
      if (!(type <= 2 && speedX > 0)) {
        result.moveX = speedX;
      }
    }
    ...
    if (topY < preTopY) {
      ...
      speedY =
        offsetY < 0
          ? Math.max(offsetY, -oriSpeedY)
          : Math.min(offsetY, oriSpeedY);
      if (!(type <= 2 && speedY > 0)) {
        result.moveY = speedY;
      }
    }
    ...
    if (Math.abs(goX) > Math.abs(goY)) {
      ...
      offsetY =
        offsetY < 0
          ? -Math.min(speed, Math.abs(offsetY))
          : Math.min(speed, Math.abs(offsetY));
      ...
      offsetX =
        offsetX < 0
          ? -Math.min(speed, Math.abs(offsetX))
          : Math.min(speed, Math.abs(offsetX));
      result.moveX += offsetX;
    }
    // ランダムタイプの場合
    if (type == 2) {
      if (75 - Math.randomInt(100) > 0) {
        result.moveX /= 2;
      }
      if (75 - Math.randomInt(100) > 0) {
        result.moveY /= 2;
