...
(function () {
	...
	function convertEscapeCharacters(text) {
		...
		text = text.replace(/\x1bS\[(\d+)\](.*?):(.*?)(?:\x1bS|$)/gi, function () {
	...
	// Bitmap
	//=============================================================================
	Bitmap.prototype.drawTextEx = function (text, x, y, maxWidth, lineHeight, align, iconSize, iconMargin) {
		...
		if (align && align !== 'left') {
			var totalWidth = this.measureTextWidthEx(text, iconSize, iconMargin, elems);
			if (align === 'right') {
		...
		var anyWindow = SceneManager._scene._windowLayer ? SceneManager._scene._windowLayer.children[0] : null;
		if (!anyWindow) {
			...
			if (elem[0] === '{') {
				...
				switch (escapeChara) {
					case 'C':
						...
						break;
					case 'I':
						this.drawIcon(Number(code), x, y + Math.floor((lineHeight - iconSize) / 2), iconSize, iconSize);
	...
	Bitmap.prototype.measureTextWidthEx = function (text, iconSize, iconMargin, elems) {
	...
	Game_CustomWindows.prototype.commandShow = function (args) {
		...
		if (isNaN(args[1])) {
			...
			switch (pos) {
				case 'leftup': case 'upleft': case '左上':
					x = 0;
					y = 0;
					break;
				case 'rightup': case 'upright': case '右上':
					x = Graphics.boxWidth - setting.windowWidth();
					y = 0;
					break;
				case 'leftdown': case 'downleft': case '左下':
					x = 0;
					y = Graphics.boxHeight - setting.windowHeight();
					break;
				case 'rightdown': case 'downright': case '右下':
					x = Graphics.boxWidth - setting.windowWidth();
					y = Graphics.boxHeight - setting.windowHeight();
					break;
			}
	...
	Game_CustomWindows.prototype.createParts = function (partsId, type, width, height, format) {
	...
	Game_CustomWindows.prototype.commandSetParts = function (args) {
		...
		if (!parts) {
			throwError('パーツID' + partsId + 'が存在しません。パーツ作成コマンドを事前に実行して下さい。');
			return;
		}
	...
	// Game_WindowSetting
	//=============================================================================
	Game_WindowSetting.prototype.initialize = function (id, width, height, underPicture) {
		this._id = id;
		this.clear();
	...
	/* parts
	===================================*/
	Game_WindowSetting.prototype.registerParts = function (partsId, x, y, format) {
		if (this._showing) {
			throw new Error('パーツ作成コマンドはウィンドウ表示前に行えます');
		}
		var partsData = $gameCustomWindows.parts(partsId);
		if (!partsData) {
			throw new Error('パーツID:' + partsId + 'が存在しません。パーツ作成コマンドを事前に実行して下さい。');
		}
	...
	Game_WindowSetting.prototype.updatePartsSprite = function (args) {
		...
		if (index < 0) {
			throwError('パーツ' + target + 'が存在しません。パーツ作成コマンドを事前に実行して下さい。');
			return;
		}
	...
	Scene_Map.prototype.addTrpCustomWindow = function (id) {
		...
		if (window.isUnderPicture()) {
			...
			var index = parent.children.indexOf(this._diceContainer || parent._pictureContainer);
			if (index >= 0) {
	...
	Window_TrpCustom.prototype.refreshParts = function (index) {
		...
		var sprite = null;
		var useSprite = params ? (params.sprite || params['スプライト'] || false) : false;
		var tx, ty;
		if (useSprite) {
			...
			if (!sprite) {
				...
				if (params.underContents || params['コンテンツの下']) {
					this.addChildAt(sprite, this.children.indexOf(this._windowContentsSprite));
				} else {
		...
		if (type === PARTS_TYPE.image) {
			this.refreshImageParts(index, sprite, contents, tx, ty, format, width, height, params);
		} else if (type === PARTS_TYPE.text) {
			this.refreshTextParts(index, sprite, contents, tx, ty, format, width, height, params);
		}
	};

	Window_TrpCustom.prototype.registerDrawRects = function (index, x, y, width, height) {
	...
	Window_TrpCustom.prototype.refreshTextParts = function (index, sprite, contents, x, y, format, width, height, params) {
		...
		contents.outlineWidth = params.outlineWidth || params['アウトライン幅'] || 4;
		contents.outlineColor = params.outlineColor || params['アウトライン色'] || 'rgba(0,0,0,0.5)';
		contents.drawTextEx(format, x, y, width, height, align);
	};

	Window_TrpCustom.prototype.refreshImageParts = function (index, sprite, contents, x, y, format, width, height, params) {
	...
	/* observers
	===================================*/
	Window_TrpCustom.prototype.registerVariableObserver = function (variableId, partsIndex) {
		var cache = $gameVariables.value(variableId);
		this._variableObservers.push([variableId, partsIndex, cache]);
	};
	Window_TrpCustom.prototype.registerSwitchObserver = function (switchId, partsIndex) {
