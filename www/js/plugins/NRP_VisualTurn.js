...
(function () {
  ...
  const pCharacterDirection = setDefault(
    parameters["characterDirection"],
    "down"
  );
  const pActorBackImage = parameters["actorBackImage"];
  const pselectedHighlightType = setDefault(
    parameters["selectedHighlightType"],
    "image"
  );
  ...
  const pVisualIdArray = setDefault(
  ...
  const pDisplayForSkillSelect = toBoolean(
   ...
   * ●バトラー毎の順序表示を行う。
   */
  Window_BattleCtb.prototype.drawCtbBattler = function (
    battler,
    x,
    y,
    opacity
