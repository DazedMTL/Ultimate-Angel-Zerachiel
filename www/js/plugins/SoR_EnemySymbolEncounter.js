...
(function () {
  ...
  var AutoDisable_DefaultEncounter =
    Boolean(Param["AutoDisable_DefaultEncounter"] === "true") || false;
  var FollowerAttack = Number(Param["FollowerAttack"]) || 0;
  var InvinsibleDuration_AB =
    Number(Param["InvincibleTime_AfterBattle"]) || 300;
  ...
  var IsMenuSubCommandMap =
    Boolean(Param["Use_MenuSubCommandMap"] === "true") || false;

  var InvincibleStyle_Flash =
    Boolean(Param["InvincibleStyle_Flash"] === "true") || false;
  var InvincibleStyle_Opaque =
    Boolean(Param["InvincibleStyle_Opaque"] === "true") || false;
  ...
  Game_Event.prototype.EncountDir = function () {
    ...
    if ($gameTemp.collisionFollower != null)
      player = $gameTemp.collisionFollower;
  ...
  Scene_Battle.prototype.terminate = function () {
    ...
    if ($gameTemp.isEncountedonMap) {
      ...
      if ($gameTemp.encflag == 1 && BGMsurprised != "") {
        //surprised bgm
  ...
  Spriteset_Map.prototype.updateCharacterForSymbolEnc = function () {
    ...
    if (IsEnableInvincible()) {
      ...
      if ($gameTemp.invincibleAfterEnc >= 0)
        $gamePlayer.UpdateInvinsibleEffectsAfterSE();
      else if ($gameTemp.invincibleAfterEnc != -1)
        $gamePlayer.FinishInvinsibleEffectsAfterSE();
    }
  };
  ...
  Game_Player.prototype.performTransfer = function () {
    ...
    if (IsEnableInvincible()) {
      if (!this.IsJustMovedFromOrigMap()) {
        if ($gameTemp.invincibleAfterEnc && $gameTemp.invincibleAfterEnc >= 0)
          $gamePlayer.FinishInvinsibleEffectsAfterSE(false);
      }
    }
  ...
  Game_Player.prototype.SetupInvinsibleEffectsAfterSE = function () {
    ...
    if (InvincibleStyle_Blend != 0)
      $gamePlayer.setBlendMode(InvincibleStyle_Blend);
  ...
  Game_Player.prototype.UpdateInvinsibleEffectsAfterSE = function () {
    ...
    if (InvincibleStyle_Flash) {
      let flash;
      if ($gameTemp.invincibleAfterEnc >= InvinsibleDuration_AB * 0.5)
        flash = 12;
      else if ($gameTemp.invincibleAfterEnc >= InvinsibleDuration_AB * 0.25)
        flash = 9;
      else if ($gameTemp.invincibleAfterEnc >= InvinsibleDuration_AB * 0.1)
        flash = 7;
      else flash = 5;

      $gameTemp._flash_count = ($gameTemp._flash_count + 1) % flash;
      if ($gameTemp._flash_count <= 1)
        $gamePlayer.SetInvinsibleAfterSymbolEnc(true);
      else $gamePlayer.SetInvinsibleAfterSymbolEnc(false);

      if ($gameTemp.invincibleAfterEnc == 0)
        $gamePlayer.SetInvinsibleAfterSymbolEnc(false);
    }
    if (InvincibleStyle_Opaque && $gameTemp.invincibleAfterEnc == 0)
      $gamePlayer.FinishInvinsibleEffectsAfterSE(false);
  };
  ...
  Game_Player.prototype.SetInvinsibleAfterSymbolEnc = function (val) {
    ...
    if (InvincibleStyle_Flash) {
      const v =
        val === true ? (InvincibleStyle_Opaque === true ? 127 : 0) : 255;
  ...
  Game_Event.prototype.SetSymbolTags = function (com) {
    if (com.match(/(?:distance):[ ]*(.*)/i))
      this.see_distance = parseInt(RegExp.$1);
    else if (com.match(/(?:angle):[ ]*(.*)/i))
      this.see_angle = (parseInt(RegExp.$1) * Math.PI) / 180 + 0.00001;
    else if (com.match(/(?:speed):[ ]*(.*)/i))
      this.chase_speed = parseInt(RegExp.$1);
    else if (com.match(/(?:duration):[ ]*(.*)/i))
      this.chase_duration = parseInt(RegExp.$1);
    else if (com.match(/(?:wait):[ ]*(.*)/i))
      this.wait_afterchase = parseInt(RegExp.$1);
    else if (com.match(/(?:return)[ ]*/i)) this.Isreturn_afterwait = true;
    else if (com.match(/(?:ignore)/i)) this.Is_symbolIgnore = true;
    else if (com.match(/(?:bush_rate):[ ]*(.*)/i))
      this.bushrate = parseFloat(RegExp.$1);
    else if (com.match(/(?:alert_time):[ ]*(.*)/i))
      this.detectedAlertDuration = parseInt(RegExp.$1);
    else if (com.match(/(?:route_search):[ ]*(.*)/i))
      this.updateRoute_interval = parseInt(RegExp.$1);
    else if (com.match(/(?:chase_vehicle)[ ]*/i)) this.Ischase_onVehicle = true;
    else if (com.match(/(?:common_event):[ ]*(.*)/i))
      this.call_CommonEventID = parseInt(RegExp.$1);
  };
  ...
  Game_Event.prototype.update = function () {
    ...
    if (this.event().meta.EnemySymbol) {
      if ($gameTemp.invincibleAfterEnc > -1) {
        ...
        if (this.IsDetectPlayer && !this.IsChasePlayer && !this.IsFleePlayer)
          this.DetectedPlayer();
      }
    }
  };
  ...
  function Bresenham(a1, a2) {
    ...
    while (1) {
      if (
        !$gameMap.isPassable(cx, cy, dir_x) ||
        !$gameMap.isPassable(cx, cy, dir_y) ||
        $gameMap.regionId(cx, cy) == SymbolKeepOut_RegionID
      ) {
        flag = false;
        break;
      }
  ...
  function Bresenham90(a1, a2) {
    ...
    for (var cy = a1.y; ; cy += incre) {
      if (
        !$gameMap.isPassable(cx, cy, a1.dir) ||
        $gameMap.regionId(cx, cy) == SymbolKeepOut_RegionID
      ) {
        flag = false;
        break;
  ...
  Game_Event.prototype.DetectedPlayer = function () {
    if (!this.IsDetectBallon) {
      ...
      //balloon
      if (BalloonID_PlayerDetected != -1)
        this.requestBalloon(BalloonID_PlayerDetected);
      if (SoundDetected != "") AudioManager.playSe(SE_PlayerDetected);
      //common ev
      if (this.call_CommonEventID != 0)
        $gameTemp.reserveCommonEvent(this.call_CommonEventID);
    }
  ...
  Game_Event.prototype.PlayerChaser = function (state) {
    ...
    if (this.chase_time >= 0) {
      ...
      if (
        ...
        this.trails_n >= this.updateRoute_interval
      ) {
        this.trails = this.AstarDir(
          $gamePlayer.x,
          $gamePlayer.y,
          this.see_distance *
            ($gameMap.isBush($gamePlayer.x, $gamePlayer.y) == true
              ? this.bushrate
              : 1)
        ); ///d
        this.trails_n = 0;
    ...
    if (this.chase_time < 0 && isChaseFeasible && d <= this.see_distance / 3)
      this.chase_time = 10;
  ...
  function calcAngle(e, p) {
    ...
    var theta = Math.acos(
      (vec.z * va.x + vec.c * va.y) / Math.sqrt(va.x * va.x + va.y * va.y)
    );
  ...
  Game_CharacterBase.prototype.IsEnablePassAdjcentCells = function (
    x,
    y,
    x2,
    y2,
    ...
    if (FollowerAttack % 3 == 0 && this.isCollidedWithFollowerCharacters(x, y))
      return false;
  ...
  Game_Player.prototype.updateVehicleGetOn = function () {
    ...
    if ($gameTemp.invincibleAfterEnc && $gameTemp.invincibleAfterEnc != -1)
      $gamePlayer.FinishInvinsibleEffectsAfterSE();
  ...
  if (FollowerAttack % 3 == 1) {
    var SoR_ESE_GE_checkEventTriggerTouch =
    ...
    var SoR_ESE_GE_isCollidedWithPlayerCharacters =
