...
// * Initialize
//==============================
var _mog_chaParticles_gchar_initMembers =
...
Game_Event.prototype.checkCharParticles = function () {
  ...
  if (oldR && !this._particles[0]) {
    this._particles = [true, [], true, -1, 0, 0, 0, 0, 0, ""];
  }
};
...
Sprite_Character.prototype.update = function () {
  ...
  if (this.canUpdateParticles()) {
    this.updateParticlesEffect();
  }
};
...
Sprite_Character.prototype.canUpdateParticles = function () {
  ...
  if (!this._character._particles[0]) {
    return false;
  }
  if (!this.bitmap) {
    return false;
...
Sprite_Character.prototype.updateParticlesEffect = function () {
  ...
  if (!this._particlesSprites || this._character._particles[2]) {
    this.createParticlesSprites();
  }
...
Sprite_Character.prototype.createParticlesSprites = function () {
  ...
  if (this._character._particles[3] < 0) {
    this.disposeParticles();
    return;
  }
  this._particlesSprites = [];
  this._particlesImage = ImageManager.loadCharParticles(
    this._character._particles[9]
  );
...
Sprite_Character.prototype.loadParticlesData = function (i) {
  ...
  this._particlesSprites[i].rotation =
    this._character._particles[1][i].rotation;
  ...
  this._particlesSprites[i].blendMode =
    this._character._particles[1][i].blendMode;
};
...
Sprite_Character.prototype.updateParticlesSprites = function (sprite, index) {
  ...
  if (this.charPartNeedReset(sprite)) {
    this.setParticlesAnimation(sprite);
  }
};
...
Sprite_Character.prototype.charPartNeedReset = function (sprite) {
  if (Imported.MOG_CharShatterEffects && this._character._shatter[0]) {
    return false;
  }
  if (sprite.opacity <= 0) {
    return true;
...
Scene_Map.prototype.terminate = function () {
  if (this._spriteset) {
    this._spriteset.recordParticlesData();
  }
...
Spriteset_Map.prototype.recordParticlesData = function () {
  for (var i = 0; i < this._characterSprites.length; i++) {
    ...
    if (sprite._particlesSprites) {
      ...
      sprite._character._particles = [false, [], false, 0, 0, 0, 0, 0, 0, ""];
    }
  }
};
