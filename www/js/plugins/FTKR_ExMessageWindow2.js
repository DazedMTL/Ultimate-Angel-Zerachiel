...
FTKR.EMW.exwindowNum = Number(
  FTKR.EMW.parameters["Create ExWindow Number"] || 0
);
FTKR.EMW.scene = {
...
Game_Interpreter.prototype.pluginCommand = function (command, args) {
  ...
  switch (command) {
    ...
    case "CANMOVE":
      var windowId = Number(args[0] || 0);
      if (windowId >= 0)
        $gameMessageEx.window(windowId).enabledCanMovePlayer(args[1]);
      break;
    case "行動禁止":
    case "NOTMOVE":
      var windowId = Number(args[0] || 0);
      if (windowId >= 0)
        $gameMessageEx.window(windowId).disabledCanMovePlayer(args[1]);
      break;
    case "テキスト更新":
    case "UPDATE_TEXT":
      var windowId = Number(args[0] || 0);
...
// \EMP  \DMP
//------------------------------------------------------------------------
FTKR.EMW.Window_Message_processEscapeCharacter =
...
Window_Message.prototype.processEscapeCharacter = function (code, textState) {
  switch (code) {
    ...
    case "DMP":
      ...
      break;
    default:
      FTKR.EMW.Window_Message_processEscapeCharacter.call(
        this,
        code,
        textState
      );
...
Window_Message.prototype.updateConditions = function () {
  return (
    !this.isOpening() && !this.isClosing() && !this._gameMessage.isLastText()
  );
};
...
FTKR.EMW.Window_Message_terminateMessage =
...
// ウィンドウの位置サイズ変更
//------------------------------------------------------------------------
FTKR.EMW.Window_Message_updatePlacement =
...
Game_Interpreter.prototype.command101 = function () {
  ...
  if (!windowId) {
    ...
    if (!$gameMessageEx.window(windowId).isEmwEventBusy()) {
      ...
      $gameMessageEx
        .window(windowId)
        .setFaceImage(this._params[0], this._params[1]);
  ...
  if (Imported.YEP_MessageCore) {
    while (this.isContinueMessageString()) {
      ...
      if (this._list[this._index].code === 401) {
        $gameMessageEx
          .window(windowId)
          .addText(this.currentCommand().parameters[0]);
      }
      if (
        $gameMessageEx.window(windowId)._texts.length >=
...
FTKR.EMW.Game_Interpreter_setupChoices =
...
Game_Interpreter.prototype.setupChoices = function (params) {
  ...
  if (!windowId) {
    ...
    $gameMessageEx.window(windowId).setChoiceCallback(
      function (n) {
...
FTKR.EMW.Game_Interpreter_setupNumInput =
...
FTKR.EMW.Game_Interpreter_setupItemChoice =
...
Window_MessageEx.prototype.doesContinue = function () {
  return (
    this._gameMessage.hasText() &&
    !this._gameMessage.scrollMode() &&
...
Window_MessageEx.prototype.drawMessageFace = function () {
  this.drawFace(
    this._gameMessage.faceName(),
    this._gameMessage.faceIndex(),
    0,
    0
  );
...
Scene_Map.prototype.createMessageExWindow = function (windowId) {
  ...
  $gameMessageEx.window(windowId)._window_MessageEx =
    this._messageExWindows[windowId];
  FTKR.EMW.scene.startTerminate
    ? $gameMessageEx.window(windowId).terminate()
...
//=============================================================================
FTKR.EMW.Scene_Battle_createMessagewindow =
...
if (Imported.YEP_MessageCore) {
  ...
  FTKR.EMW.Window_MessageEx_createSubWindows =
    Window_MessageEx.prototype.createSubWindows;
  Window_MessageEx.prototype.createSubWindows = function () {
  ...
  FTKR.EMW.Window_MessageEx_startMessage =
  ...
  // Window_ChoiceListEx
  //------------------------------------------------------------------------
  FTKR.EMW.Window_ChoiceListEx_updatePlacement =
