...
// SoR License (C) 2020 蒼竜, End of Supports for RMMV
 ...
 * バージョン情報
 * -----------------------------------------------------------
 * v1.02 (2020/07/27)       文字列描画幅とウィンドウ幅の関係を調整
...
/*:
 * @plugindesc <GabWindow Type-Right>
 ...
 * Version info.
 * -----------------------------------------------------------
 * v1.02 (Jul 27, 2020)      Fix the parameter related to window size and gab text width
...
SoR.GabWindow_baseDuration = Number(
  SoR.GabParam["GabWindow_baseDuration"] || 160
);
SoR.GabWindow_keepDuration = Number(
  SoR.GabParam["GabWindow_keepDuration"] || 400
);
SoR.Multiple_GabSpaces = Number(SoR.GabParam["Multiple_GabSpaces"] || 62);
SoR.GabWindowYpadd_originalSkin = Number(
  SoR.GabParam["GabWindowYpadd_originalSkin"] || 0
);

SoR.GabWindowHeightPadd_ForBattle = Number(
  SoR.GabParam["GabWindowHeightPadd_ForBattle"] || -196
);
  ...
  if (this._scene instanceof Scene_Map) {
    for (var i = 0; i < this._scene.gab_shown.length; i++) {
      ...
      if (SoR.WindowStyle == 2)
        this._scene.SoR_GabField.removeChild(obj.bg_img);
...
SoR_GabWindow.prototype.setupGab = function (scene) {
  ...
  if (scene == "Scene_Battle")
    this.scene_ypadd = SoR.GabWindowHeightPadd_ForBattle;
...
SoR_GabWindow.prototype.shiftY = function (tmp_y) {
  if (
    this.y !=
    this.Basey + this.scene_ypadd - tmp_y * SoR.Multiple_GabSpaces
  ) {
    this.diffY =
      this.Basey + this.scene_ypadd - tmp_y * SoR.Multiple_GabSpaces - this.y;
  } else this.diffY = 0;
...
SoR_GabWindow.prototype.update = function () {
  ...
  this.x =
    Graphics.boxWidth -
    this.drawLength -
    SoR.GabWindowWidthPadd +
    this.enter_effect;
  this.y += Math.floor(this.diffY / 2.25);
  if (this.face)
    this.face.x = Graphics.boxWidth - SoR.FaceXPosition + this.enter_effect;
  if (this.face)
    this.face.y = this.y + SoR.FaceYPosition + Math.floor(this.diffY / 2.25);
};
...
SoR_GabWindow.prototype.CalcDuration = function () {
  ...
  var time = (x1 + x2) / base;
  return time > 1.0
    ? Math.floor(SoR.GabWindow_baseDuration * time)
    : SoR.GabWindow_baseDuration;
};
...
SoR_GabWindow.prototype.GetFaceSprite = function (id) {
  ...
  if (
    SoR.ActorFaceRect_L <= -1 ||
    SoR.ActorFaceRect_L > Window_Base._faceHeight
  )
    low = ph;
