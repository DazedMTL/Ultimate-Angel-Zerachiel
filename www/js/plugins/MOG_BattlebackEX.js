...
Game_Interpreter.prototype.pluginCommand = function (command, args) {
  ...
  if (command === "bb_ex") {
    var index = Math.min(Math.max(args[1], 1), 100);
    index -= 1;
    ...
    $gameSystem._bbex_data[index][4] = Math.min(
      Math.max(Number(args[11]), 0),
      2
    );
    if (args[13]) {
  ...
  if (command === "bb_ex_clear") {
    $gameSystem._bbex_data = [];
  }
  return true;
};
...
//=============================================================================
var _alias_mog_sprt_createBattleField =
...
Spriteset_Battle.prototype.bbexSetup = function () {
  ...
  for (var i = 0; i < $gameSystem._bbex_data.length; i++) {
    if (i === 0 && !$gameSystem._bbex_data[0]) {
      this._bbData.push($gameSystem._bbex_data[0]);
    }
    if (i === 1 && !$gameSystem._bbex_data[1]) {
      this._bbData.push($gameSystem._bbex_data[1]);
    }
    if ($gameSystem._bbex_data[i]) {
      this._bbData.push($gameSystem._bbex_data[i]);
    }
  }
...
// * createLower Layer
//==============================
var _alias_mog_bbex_sprtbat_createLowerLayer =
...
Spriteset_Battle.prototype.addBattleback = function (sprite, index) {
  ...
  if (this._bbData[index]) {
    sprite.blendMode = this._bbData[index][4];
  }
};
...
// * updateBattleback
//==============================
var _alias_mog_bbex_updateBattleback =
...
Spriteset_Battle.prototype.updateBattlebackEffects = function () {
  if (!this.isbbEXVisible(this._back1Sprite)) {
    this.updateBBEXVisible(this._back1Sprite);
  }
  if (!this.isbbEXVisible(this._back2Sprite)) {
    this.updateBBEXVisible(this._back2Sprite);
  }
  if (this._bbData[0]) {
    this.updateScroll(this._back1Sprite, 0);
  }
  if (this._bbData[1]) {
    this.updateScroll(this._back2Sprite, 1);
  }
  for (var i = 2; i < this._bbData.length; i++) {
    if (!this.isbbEXVisible(this._backSpriteEx[i])) {
      this.updateBBEXVisible(this._backSpriteEx[i]);
    }
