...
(function () {
  ...
  var getBoolean = function (bString) {
    if (bString.toLowerCase() === "off") return false;
    return true;
  };
  var getNumber = function (nString) {
  ...
  SceneManager._useGWTrans = getParamBoolean([
    "Use Custom Transition",
    "画面フェードに適用",
  ]);
  SceneManager._GWImg = getParamString([
    "Transition Fade Image",
    "画面フェード用画像",
  ]);
  SceneManager._GWInImg = getParamString([
    "Transition FadeIn Image",
    "画面フェードイン用画像",
  ]);
  SceneManager._GWOutImg = getParamString([
    "Transition FadeOut Image",
    "画面フェードアウト用画像",
  ]);
  SceneManager.GWSoftness = getParamNumber([
    "Transition Softness",
    "画面フェードの柔らかさ",
  ]);
  SceneManager._fadeDuration = parseInt(
    getParamNumber(["Transition Duration", "画面フェード時間"])
  );
  ...
  var applyFadeGWFilter = function (imgName) {
    var GWsprite = new Sprite(ImageManager.loadTransitions(imgName)),
      f = new PIXI.filters.GradientWipeFilter(
        GWsprite,
        SceneManager.GWSoftness
      );
  ...
  Scene_Base.prototype.startFadeOut = function (duration, white) {
    ...
    if (SceneManager.useGWTrans() && SceneManager.GWOutImg()) {
      ...
      this._GWSpeed =
        (255 + 1 + this._GWFadeFilter.softness) / this._fadeDuration;
    }
  };

  var _Scene_Base_updateFade = Scene_Base.prototype.updateFade;
  Scene_Base.prototype.updateFade = function () {
    if (!SceneManager.useGWTrans())
  ...
  Game_Screen.prototype.initialize = function () {
    ...
    this._fadeDuration = parseInt(
      getParamNumber(["Transition Duration", "画面フェード時間"])
    );
    this._useGWTrans = getParamBoolean([
      "Use Custom Transition",
      "画面フェードに適用",
    ]);
    this._GWImg = getParamString([
      "Transition Fade Image",
      "画面フェード用画像",
    ]);
    this._GWInImg = getParamString([
      "Transition FadeIn Image",
      "画面フェードイン用画像",
    ]);
    this._GWOutImg = getParamString([
      "Transition FadeOut Image",
      "画面フェードアウト用画像",
    ]);
  };

  Game_Screen.prototype.useGWTrans = function () {
    return this._useGWTrans;
  };
  ...
  var _Spriteset_Base_updateScreenSprites =
    Spriteset_Base.prototype.updateScreenSprites;
  Spriteset_Base.prototype.updateScreenSprites = function () {
    ...
    if ($gameScreen._needFilter) {
      this.applyFadeGWFilter(
        $gameScreen._needFilter > 0
          ? $gameScreen.GWInImg()
          : $gameScreen.GWOutImg()
      );
    ...
    if ($gameScreen._fadeOutDuration || $gameScreen._fadeInDuration) {
      ...
      f.time =
        ((255 + 1 + f.softness) * (255 - $gameScreen.brightness())) / 255;
    } else {
      this.removeFadeGWFilter();
    }
  };
  ...
  Window_Base.prototype.initGW = function () {
    ...
    if (typeof this._GWopenness === "undefined")
  ...
  Game_System.prototype.initialize = function () {
    ...
    var setting = {};
    setting.useGW = getParamBoolean([
      "Use Custom Message Fade",
      "メッセージウィンドウに適用",
    ]);
    setting.imgName = getParamString([
      "Message Fade Image",
      "メッセージフェード用画像",
    ]);
    setting.fadeSpeed =
      256 /
      (getParamNumber(["Message Fade Duration", "メッセージフェード時間"]) ||
        8);
...
PIXI.filters.GradientWipeFilter = function (
  gradientSprite,
  softness,
  reverse,
  time
...
PIXI.filters.GradientWipeFilter.prototype = Object.create(
  PIXI.Filter.prototype
);
PIXI.filters.GradientWipeFilter.prototype.constructor =
  PIXI.filters.GradientWipeFilter;

PIXI.filters.GradientWipeFilter.prototype.apply = function (
  filterManager,
  input,
  output,
  clear
