...
(function () {
  ...
  FTKR.STS = {
    ...
    resetWhenForgottenSkill: Number(
      parameters["Reset When Forgotten Skill"] || 0
    ),
  ...
  if (!Window_Base.prototype.reserveFaceImages) {
    console.error(
      "プロジェクトのコアスクリプトを最新版(v1.5.0以降)にアップデートしてください。"
    );
    console.error(
      "Update core scripts of your project to the latest version (v1.5.0 or later)."
    );
    return;
  }
  ...
  if (filename !== "FTKR_SkillTreeSystem.js") {
    console.error(
      "スキルツリープラグインのファイル名が間違っています。「FTKR_SkillTreeSystem.js」に直してください。"
    );
    console.error(
      'The file name of SkillTree-plugin is incorrect. Change to "FTKR_SkillTreeSystem.js".'
    );
    return;
  }
    ...
    if (stsdata) {
      ...
      for (var i = 0; i < datas.length; i++) {
        ...
        if (data.match(case1) || data.match(case1j)) {
          ...
          obj.sts.forgetSkillIds = JSON.parse(
            "[" + RegExp.$1.match(/\d+/g) + "]"
          );
          ...
          var forget = RegExp.$3;
          var skillId2 =
            forget && forget.match(/-d[ ]*(\d+)/i) ? Number(RegExp.$1) : 0;
  ...
  Game_Actor.prototype.learnSkill = function (skillId) {
    ...
    if (this.stsSkill(skillId).sts.learnSkillIds) {
      ...
      var forgetSkillId =
        this.stsSkill(skillId).sts.learnSkillIds[count].forget;
      if (countSkillId) {
  ...
  Game_Actor.prototype.forgetSkill = function (skillId) {
    ...
    if (this.stsSkill(skillId).sts.learnSkillIds) {
      this.stsSkill(skillId).sts.learnSkillIds.forEach(function (
        forgetSkillId
      ) {
  ...
  Game_Actor.prototype.isStsMaxCount = function (skillId) {
    return (
      this.stsSkill(skillId) &&
      this.stsCount(skillId) >= this.stsSkill(skillId).sts.maxCount
    );
  };
  ...
  Game_Actor.prototype.addStsUsedCsp = function (classId, skillId, value) {
    this.setStsUsedCsp(
      classId,
      skillId,
      this.stsUsedCsp(classId, skillId) + value
    );
  };
  ...
  Game_Actor.prototype.initStsUsedItem = function (skillId, itemId) {
    ...
    if (itemId && !this._stsUsedItem[skillId][itemId])
      this._stsUsedItem[skillId][itemId] = 0;
  };
  ...
  Game_Actor.prototype.addStsUsedItem = function (skillId, itemId, value) {
    ...
    this.setStsUsedItem(
      skillId,
      itemId,
      this.stsUsedItems(skillId)[itemId] + value
    );
  };

  Game_Actor.prototype.initStsUsedWeapon = function (skillId, itemId) {
    if (!this._stsUsedWeapon) this._stsUsedWeapon = [];
    if (skillId && !this._stsUsedWeapon[skillId])
      this._stsUsedWeapon[skillId] = [];
    if (itemId && !this._stsUsedWeapon[skillId][itemId])
      this._stsUsedWeapon[skillId][itemId] = 0;
  };
  ...
  Game_Actor.prototype.addStsUsedWeapon = function (skillId, itemId, value) {
    ...
    this.setStsUsedWeapon(
      skillId,
      itemId,
      this.stsUsedWeapons(skillId)[itemId] + value
    );
  };

  Game_Actor.prototype.initStsUsedArmor = function (skillId, itemId) {
    if (!this._stsUsedArmor) this._stsUsedArmor = [];
    if (skillId && !this._stsUsedArmor[skillId])
      this._stsUsedArmor[skillId] = [];
    if (itemId && !this._stsUsedArmor[skillId][itemId])
      this._stsUsedArmor[skillId][itemId] = 0;
  };
  ...
  Game_Actor.prototype.addStsUsedArmor = function (skillId, itemId, value) {
    ...
    this.setStsUsedArmor(
      skillId,
      itemId,
      this.stsUsedArmors(skillId)[itemId] + value
    );
  };
  ...
  Game_Actor.prototype.initStsUsedVar = function (skillId, varId) {
    ...
    if (varId && !this._stsUsedVar[skillId][varId])
      this._stsUsedVar[skillId][varId] = 0;
  };
  ...
  Game_Actor.prototype.addStsUsedVar = function (skillId, varId, value) {
    ...
    this.setStsUsedVar(
      skillId,
      varId,
      this.stsUsedVars(skillId)[varId] + value
    );
  };
  ...
  Game_Actor.prototype.getCsp = function (classId, value) {
    ...
    if (this.maxCsp(classId)) {
      this._stsCsp[classId] = Math.min(
        this._stsCsp[classId],
        this.maxCsp(classId)
      );
    }
  };
  ...
  Game_Actor.prototype.maxAsp = function () {
    return (
      Math.max(this.actor().sts.msp, $dataClasses[this._classId].sts.msp) ||
      FTKR.STS.sp.defaultMaxSp
    );
  };
  ...
  Game_Actor.prototype.stsSp = function () {
    return FTKR.STS.sp.enableClassSp
      ? this.stsCsp(this._classId)
      : this.stsAsp();
  };
  ...
  if (Imported.FTKR_CSS) {
    ...
    Window_Base.prototype.drawCssActorStatusBase = function (
      ...
      return _STS_Window_Bas_drawCssActorStatusBase.call(
        this,
        index,
        actor,
        x,
  ...
  Window_Base.prototype.drawStsDescTitle = function (
    format,
    x,
    y,
    width,
  ...
  var _STS_Window_Base_convertEscapeCharacters =
  ...
  var _STS_Window_Base_processEscapeCharacter =
    Window_Base.prototype.processEscapeCharacter;
  Window_Base.prototype.processEscapeCharacter = function (code, textState) {
    switch (code) {
      case "LW":
        this.processDrawStsWidth(
          this.obtainEscapeStsParam(textState),
          textState
        );
        break;
      default:
  ...
  Window_TreeType.prototype.update = function () {
    ...
    if (tTypeId && this._preskillWindow)
      this._preskillWindow.setTtypeId(tTypeId);
  };
  ...
  Window_SkillTree.prototype.drawStsFrame = function (
    x,
    y,
    width,
    height,
  ...
  Window_SkillTree.prototype.drawSkillText = function (
    ...
    this.drawFormatTextEx(sts.format, x + stx, y + sts.offsetY, [
      this.ftItemName(skill),
    ]);
  };

  //スキルの習得回数を表示
  Window_SkillTree.prototype.drawSkillCountRect = function (
    skill,
    data,
    rect,
    color
  ...
  Window_SkillTree.prototype.drawSkillCount = function (
    ...
    var count = !actor.isStsLearnedSkill(skill.id)
      ? 0
      : actor.stsCount(skill.id);
    if (FTKR.STS.enableSkillCount) {
      if (cfl.enabled) {
        var fcolor = this.setFrameColor(data);
        if (Imported.FTKR_DCF) {
  ...
  Window_SkillTree.prototype.select = function (index) {
    Window_Selectable.prototype.select.call(this, index);
    if (Imported.FTKR_DCF && FTKR.STS.sFrame.enabled)
  ...
  Window_Base.prototype.drawStsCost = function (cost, x, y, width, skillId) {
    ...
    var value =
      this._actor.isStsMaxCount(skillId) && FTKR.STS.cost.maxFormat
        ? FTKR.STS.cost.maxFormat
        : num[1];
  ...
  Window_StsActorStatus.prototype.drawActorLevel = function (
    actor,
    x,
    y,
    width
