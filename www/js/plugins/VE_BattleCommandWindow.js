...
(function () {
  ...
  VictorEngine.BattleCommandWindow.requiredPlugin =
  ...
  PluginManager.requiredPlugin = function (name, required, version) {
    if (!VictorEngine.BasicModule) {
      ...
      VictorEngine.BattleCommandWindow.requiredPlugin.call(
        this,
        name,
        required,
        version
...
(function () {
  ...
  if (Imported["VE - Basic Module"]) {
    ...
    VictorEngine.Parameters.BattleCommandWindow.FaceSufix = String(
      parameters["Face Sufix"]
    ).trim();
    VictorEngine.Parameters.BattleCommandWindow.FaceWidth = String(
      parameters["Face Width"]
    ).trim();
    VictorEngine.Parameters.BattleCommandWindow.FaceHeight = String(
      parameters["Face Height"]
    ).trim();
    VictorEngine.Parameters.BattleCommandWindow.FaceOffsetX = String(
      parameters["Face Offset X"]
    ).trim();
    VictorEngine.Parameters.BattleCommandWindow.FaceOffsetY = String(
      parameters["Face Offset Y"]
    ).trim();
    VictorEngine.Parameters.BattleCommandWindow.PictureOffsetX =
      Number(parameters["Picture Offset X"]) || 0;
    VictorEngine.Parameters.BattleCommandWindow.PictureOffsetY =
      Number(parameters["Picture Offset Y"]) || 0;
    VictorEngine.Parameters.BattleCommandWindow.SkipCommand = eval(
      parameters["Skip Party Command"]
    );
    VictorEngine.Parameters.BattleCommandWindow.ShowFaces = eval(
      parameters["Show Faces"]
    );
    VictorEngine.Parameters.BattleCommandWindow.EscapeCommand = eval(
      parameters["Escape Command"]
    );
    var list = ["Actor", "Party"];
    for (var i = 0; i < list.length; i++) {
      var type = list[i];
      VictorEngine.Parameters.BattleCommandWindow[type + " Lines"] = String(
        parameters[type + " Window Lines"]
      ).trim();
      VictorEngine.Parameters.BattleCommandWindow[type + " Columns"] = String(
        parameters[type + " Window Columns"]
      ).trim();
      VictorEngine.Parameters.BattleCommandWindow[type + " Offset X"] = String(
        parameters[type + " Offset X"]
      ).trim();
      VictorEngine.Parameters.BattleCommandWindow[type + " Offset Y"] = String(
        parameters[type + " Offset Y"]
      ).trim();
      VictorEngine.Parameters.BattleCommandWindow[type + " Width"] = String(
        parameters[type + " Width"]
      ).trim();
      VictorEngine.Parameters.BattleCommandWindow[type + " Height"] = String(
        parameters[type + " Height"]
      ).trim();
      VictorEngine.Parameters.BattleCommandWindow[type + " Back Opacity"] =
        String(parameters[type + " Back Opacity"]).trim();
      VictorEngine.Parameters.BattleCommandWindow[type + " Frame Opacity"] =
        String(parameters[type + " Frame Opacity"]).trim();
      VictorEngine.Parameters.BattleCommandWindow[type + " Background"] =
        String(parameters[type + " Background"]).trim();
      VictorEngine.Parameters.BattleCommandWindow[type + " Text X"] = String(
        parameters[type + " Text X"]
      ).trim();
      VictorEngine.Parameters.BattleCommandWindow[type + " Text Y"] = String(
        parameters[type + " Text Y"]
      ).trim();
      VictorEngine.Parameters.BattleCommandWindow[type + " Content X"] = String(
        parameters[type + " Content X"]
      ).trim();
      VictorEngine.Parameters.BattleCommandWindow[type + " Content Y"] = String(
        parameters[type + " Content Y"]
      ).trim();
      VictorEngine.Parameters.BattleCommandWindow[type + " Content Back"] =
        String(parameters[type + " Content Back"]).trim();
      VictorEngine.Parameters.BattleCommandWindow[type + " Content Back X"] =
        String(parameters[type + " Content Back X"]).trim();
      VictorEngine.Parameters.BattleCommandWindow[type + " Content Back Y"] =
        String(parameters[type + " Content Back Y"]).trim();
      VictorEngine.Parameters.BattleCommandWindow[type + " Content Font"] =
        String(parameters[type + " Content Font"]).trim();
      VictorEngine.Parameters.BattleCommandWindow[type + " Content Size"] =
        String(parameters[type + " Content Size"]).trim();
      VictorEngine.Parameters.BattleCommandWindow[type + " Content Color"] =
        String(parameters[type + " Content Color"]).trim();
      VictorEngine.Parameters.BattleCommandWindow[type + " Active X"] = String(
        parameters[type + " Active X"]
      ).trim();
      VictorEngine.Parameters.BattleCommandWindow[type + " Active Y"] = String(
        parameters[type + " Active Y"]
      ).trim();
      VictorEngine.Parameters.BattleCommandWindow[type + " Active Back"] =
        String(parameters[type + " Active Back"]).trim();
      VictorEngine.Parameters.BattleCommandWindow[type + " Active Back X"] =
        String(parameters[type + " Active Back X"]).trim();
      VictorEngine.Parameters.BattleCommandWindow[type + " Active Back Y"] =
        String(parameters[type + " Active Back Y"]).trim();
      VictorEngine.Parameters.BattleCommandWindow[type + " Active Font"] =
        String(parameters[type + " Active Font"]).trim();
      VictorEngine.Parameters.BattleCommandWindow[type + " Active Size"] =
        String(parameters[type + " Active Size"]).trim();
      VictorEngine.Parameters.BattleCommandWindow[type + " Active Color"] =
        String(parameters[type + " Active Color"]).trim();
      VictorEngine.Parameters.BattleCommandWindow[type + " Text Align"] =
        String(parameters[type + " Text Align"]).trim();
      VictorEngine.Parameters.BattleCommandWindow[type + " Back X"] =
        Number(parameters[type + " Back X"]) || 0;
      VictorEngine.Parameters.BattleCommandWindow[type + " Back Y"] =
        Number(parameters[type + " Back Y"]) || 0;
      VictorEngine.Parameters.BattleCommandWindow[type + " Horizontal"] = eval(
        parameters[type + " Horizontal Centralize"]
      );
      VictorEngine.Parameters.BattleCommandWindow[type + " Vertical"] = eval(
        parameters[type + " Vertical Centralize"]
      );
      VictorEngine.Parameters.BattleCommandWindow[type + " Layer"] = eval(
        parameters[type + " Window Layer"]
      );
    }
  }
  ...
  VictorEngine.BattleCommandWindow.loadNotetagsValues =
  ...
  Game_Action.prototype.isValid = function () {
    return (
      this.isEscape() || VictorEngine.BattleCommandWindow.isValid.call(this)
    );
  };
  ...
  VictorEngine.BattleCommandWindow.clearActions =
  ...
  VictorEngine.BattleCommandWindow.pluginCommand =
  ...
  VictorEngine.BattleCommandWindow.endAction =
  ...
  VictorEngine.BattleCommandWindow.startPartyCommandSelection =
...
Window_CustomPartyCommand.prototype = Object.create(
  Window_PartyCommand.prototype
);
...
(function () {
  ...
  Window_CustomPartyCommand.prototype.itemRect = function (index) {
    ...
    var row =
      cols === 1 ? Math.floor(index / cols) : Math.floor(index / cols) % rows;
  ...
  Window_CustomPartyCommand.prototype.drawTextArea = function (rect, index) {
    ...
    if (this.isActive(index)) {
      var color = eval(
        this.getFontColor(this.selected().color || this.content().color)
      );
...
Window_CustomActorCommand.prototype = Object.create(
...
(function () {
  ...
  Window_CustomActorCommand.prototype.updatePicture = function () {
    if (this._picture && this._actor && this._actor.commandPicture()) {
      if (!this._picture.bitmap || this._pictureActor !== this._actor) {
        ...
        this._picture.bitmap = ImageManager.loadPicture(
          this._actor.commandPicture()
        );
        this._picture.bitmap.addLoadListener(
          this.updatePictureFrame.bind(this)
        );
      }
  ...
  Window_CustomActorCommand.prototype.drawBackArea = function (
    rect,
    index,
    actor
  ) {
  ...
  Window_CustomActorCommand.prototype.drawTextArea = function (
    rect,
    index,
    actor
  ) {
