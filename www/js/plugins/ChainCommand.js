...
(function () {
  ...
  var soundFX = {
    keypress: {
      name: params["Keypress SFX"],
      pan: params["Keypress Pan"],
      pitch: params["Keypress Pitch"],
      volume: params["Keypress Volume"],
    },
    victory: {
      name: params["Success SFX"],
      pan: params["Success Pan"],
      pitch: params["Success Pitch"],
      volume: params["Success Volume"],
    },
    defeat: {
      name: params["Error SFX"],
      pan: params["Error Pan"],
      pitch: params["Error Pitch"],
      volume: params["Error Volume"],
    },
  };
  ...
  Scene_ChainCommand.prototype.createBackground = function () {
    ...
    this.addChild(
      new Sprite(ImageManager.loadBitmap("img/chaincmd/", ChainCommand.bgImage))
    );
  };

  Scene_ChainCommand.prototype.createTimerBar = function () {
    var back = new Sprite(
      ImageManager.loadBitmap("img/chaincmd/", "bar_empty")
    );
    back.x = backBar.x;
    back.y = backBar.y;
  ...
  function parseSequence(string) {
    ...
    if (str.startsWith("random:")) {
      ...
      while (a.length < x)
        a.push(validKeys[Math.floor(Math.random() * validKeys.length)]);
    } else {
      a = str.split(",");
  ...
  Scene_ChainCommand.prototype.createSequence = function () {
    ...
    if (isNaN(ChainCommand.time))
      ChainCommand.time = this.sequence.length * timePerKey;
    ...
    for (var i = 0; i < this.sequence.length; i++) {
      this.buttons[i] = new Sprite(
        ImageManager.loadBitmap("img/chaincmd/", this.sequence[i])
      );
      this.buttons[i].x = x;
      this.buttons[i].y = y;
    ...
    this._cursor = new Sprite(
      ImageManager.loadBitmap("img/chaincmd/", "cursor")
    );
  ...
  Scene_ChainCommand.prototype.hasFailed = function () {
    ...
    this.addChild(
      new Sprite(ImageManager.loadBitmap("img/chaincmd/", "failure"))
    );
    this.ended = true;
  };
  ...
  Scene_ChainCommand.prototype.hasSucceeded = function () {
    ...
    this.addChild(
      new Sprite(ImageManager.loadBitmap("img/chaincmd/", "success"))
    );
    this.ended = true;
  };
  ...
  ChainCommand.remember = function (trigger, time, switch1, sequence) {
    if (this.getFromMemory(trigger) < 0) {
      ...
      console.log(
        "ChainCommand could not register '" +
          trigger +
          "' because it is already registered to a sequence!"
      );
    }
  };
  ...
  // pluginCommand
  //
  var CCmd_Game_Interpreter_pluginCommand =
