...
(function (global) {
  ...
  var upstream_Window_AchievementList_initialize =
    Achievement.Window_AchievementList.prototype.initialize;
  Achievement.Window_AchievementList.prototype.initialize = function (
    x,
    y,
    width,
    height
    ...
    if (Achievement.settings.listCancel) {
      h =
        Math.ceil((this.maxItems() - 1) / AchievementTile.settings.colsSize) +
        1;
    } else {
    ...
    this.width =
      w * this.itemWidth() + (w - 1) * this.spacing() + this.padding * 2;
  ...
  Achievement.Window_AchievementList.prototype.itemRect = function (index) {
    ...
    if (Achievement.settings.listCancel && index === maxItems - 1) {
      ...
      rect.y =
        Math.ceil((maxItems - 1) / maxCols) * rect.height - this._scrollY;
    } else {
  ...
  Achievement.Window_AchievementList.prototype.translucentOpacity =
    function () {
      return 64;
    };
  ...
  Achievement.Window_AchievementList.prototype.setHelpWindowItem = function (
    item
  ) {
    ...
    if (item) {
      this._helpWindow.setItem({
        description: item.unlocked
          ? "\\c[" +
            AchievementTile.settings.titleColorID +
            "]" +
            item.title +
            "\\c[0]\n" +
            item.description
          : "\\c[" +
            AchievementTile.settings.disableColorID +
            "]" +
            item.title +
            "\n" +
            item.description,
  ...
  var upstream_Window_AchievementList_updateCursor =
    Achievement.Window_AchievementList.prototype.updateCursor;
